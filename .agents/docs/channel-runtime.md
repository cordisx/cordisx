...
| --- | --- | --- | --- |
| Feishu/Lark app bot | Official Open Platform supports bot apps, message APIs, events, webhook dispatch, and SDK WebSocket long connection. The official Node SDK states long connection needs outbound internet but no public IP/domain; event handling must complete within the platform timeout and long connection does not cover callback subscriptions. | Developer app, bot capability, App ID/Secret, granted scopes, event subscription, published/approved version. Webhook mode needs a public callback; long connection does not. | implemented launcher-side websocket adapter (`@larksuiteoapi/node-sdk` 1.73.0): direct-message text ingress and outbox reply are normalized through the durable core. A real account smoke remains opt-in evidence, not implied by mocked SDK tests. |
...
| Adapter connections | Tenant-qualified ref, official adapter kind, enabled state, compatible transport mode, callback alias where required, and optional opaque `secretRef`. | Same non-secret fields plus `secretState`; no reference string. | parser/projection and Feishu/Lark websocket adapter are implemented; missing or unavailable private credentials project only `missing`/`unavailable` and do not stop the launcher. |
...
| Credential state | Source config accepts only `keychain:` or `host-secret:` references; inline/plaintext fields fail closed. | The generic Host descriptor carries only secret field paths/set state; the Channel body projection carries `missing`, `ready`, or `unavailable`; neither carries the reference value. | launcher resolver supports `host-secret:env/NAME` and macOS generic-keychain references. It returns an ephemeral value only to the official SDK and never writes, logs, errors, or projects it; credential creation UI remains unavailable. |
...
3. **Node host/core (`cordisx`)**: versioned Host config parser/redacted
   ...
   credential creation UI, and full configured restart remain separate follow-ups.
4. **Platform adapters (`cordisx`)**: Feishu/Lark websocket direct-message
   adapter is implemented with deterministic mocked wire tests and no real
   credential in fixtures. WeCom remains a separate official-SDK/protocol PR.
