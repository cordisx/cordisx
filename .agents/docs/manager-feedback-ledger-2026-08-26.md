...
| Host Form layout, action icon semantics, and official-control containment              | `packages/cli/src/renderer/host-ui/HostForm.tsx`, `packages/cli/src/renderer/host-ui/HostFormValidation.ts`, `packages/cli/src/renderer/icons.ts` | `tests/host-react-form-behavior.test.tsx`, `tests/schemastery-ui.test.ts`, `tests/plugin-config-runtime.integration.test.ts`           |
| Array editor interactions and ordering                                                 | `packages/cli/src/renderer/host-ui/HostForm.tsx`                                                                                                  | `tests/host-react-form-behavior.test.tsx` plus production-renderer pointer and keyboard smoke                                          |
...
| FORM-02 | Comments 11–12                                                        | planned     | Fix numeric-control overflow at the shared primitive boundary. Every official InputNumber, its internal wrapper, buttons, and value input must respect the Host control seat's available inline size. Do not add field-specific width patches; verify multiple number fields and narrow layouts.                                                                                                                                                                                             |
...
shared Host/TDesign control boundary: React component, control seat or internal
...
`host-ui/HostForm.tsx` has one Form owner; the final integrated smoke consumes their
