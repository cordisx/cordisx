...
events, or business command results. Denial, switching, ancestor suppression,
generation replacement, and disposal remove its projected properties and
commands; unloading revokes the lease. Adapter effects are separate from this
state machine. For `composer.reasoning-intensity`, replace/legacy rendering
