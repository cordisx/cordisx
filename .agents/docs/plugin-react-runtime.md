...
Status: implemented and verified on the current candidate. Production-bundle,
immutable-package, lifecycle, fresh scaffold, and isolated browser Playground
evidence is recorded below. Formal merge remains a separate gate.
...
adapt other view frameworks, or provide version negotiation. A plugin importing
`react`, `react-dom`, `tdesign-react`, or `tdesign-icons-react` into its renderer
artifact fails the CordisX build.
...
The launcher compiles every plugin with automatic JSX and replaces
...
| Owning-repository delivery | unimplemented | exact-head PR CI, formal merge, and remote-main readback |

The table describes this candidate only. It must not be used to claim a formal
release before the remaining gates pass.
