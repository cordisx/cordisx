...
import { JsonPackageManifestV2Resolver, PLUGIN_PACKAGE_SCHEMA_V5, PLUGIN_PACKAGE_SCHEMA_V6 } from '../packages/cli/src/launcher/packages/manifest.js'
import { loadStagedPluginPackage, removeStagedPluginPackage, stageResolvedPluginPackage } from '../packages/cli/src/launcher/plugin-package.js'
import { entityTreeDigest } from '../packages/cli/src/launcher/entity-directory.js'

const entitySchema = 'https://raw.githubusercontent.com/cordisx/cordisx-protocol/main/schemas/entity-file.v1.schema.json'
...
async function fixture(packageVersion: 5 | 6): Promise<string> {
  const root = await mkdtemp(path.join(os.tmpdir(), 'cordisx-package-v6-')); roots.add(root)
  ...
  const entity = {
    $schema: entitySchema, contract: 'cordisx.entity-file/v1', schemaVersion: 1, agentId: 'lead', name: 'Lead',
  ...
  const runtime = {
    $schema: CORDISX_PLUGIN_MANIFEST_SCHEMA_V6, schemaVersion: 6, id: 'chatroom-v6', services: [],
    capabilities: [{
  ...
  await writeFile(path.join(root, 'cordisx-package.json'), `${JSON.stringify({
    ...
    id: runtime.id, version: '1.0.0', entry: './src/index.js',
    ...
    entityTemplates: [{
      agentId: 'lead', entityPath: './entities/lead/entity.json',
...
describe('plugin-package/v6 Host boundary', () => {
  const resolver = () => new JsonPackageManifestV2Resolver({
    runtimeValidators: {
      [CORDISX_PLUGIN_MANIFEST_SCHEMA_V6]: value => normalizePluginManifestV6(value, 'chatroom-v6', new CapabilityRiskCatalog()),
    },
  })
