...
describe('public Connector client plugin integration', () => {
  it('injects a principal-bound client into a plugin, keeps native Codex unavailable, and revokes it with the owner', async () => {
    ...
    const dom = new JSDOM('<html><body><div class="sidebar-header"><button id="workspace-switcher">Codex</button></div></body></html>', {
    ...
    for (let attempt = 0; attempt < 30 && dom.window.document.documentElement.dataset.connectorPublicClientMounted !== 'true'; attempt += 1) {
    ...
    expect(discovery).toMatchObject({ status: 'accepted', snapshot: { registrations: [{ availability: 'unavailable', unavailableCode: 'unsupported' }] } })
    ...
    await expect(client?.discover()).resolves.toMatchObject({ status: 'unavailable', authorization: { code: 'principal-unavailable' } })
