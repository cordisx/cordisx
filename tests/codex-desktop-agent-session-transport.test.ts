...
const user = (id: string, value: string): UserMessage => ({
  id, role: 'user', content: [{ type: 'text', text: value }], source: { kind: 'user' },
})
...
describe('Codex Desktop Agent/Session transport', () => {
  it('uses the existing bridge for commands, notifications, approvals, queueing, and first-terminal delivery', async () => {
      ...
      sendMessageFromView: async (value: unknown) => {
        ...
        const result = method === 'thread/start'
          ...
          : method === 'turn/start'
            ...
            : {}
        queueMicrotask(() => view.message({
          type: 'mcp-response', hostId: 'local', message: { id: request.id, result },
        }))
      },
    }
    install('window', view)
    ...
    expect(await transport.create({ sessionId: 'session-1', options: { model: 'gpt-test' } })).toMatchObject({
      status: 'accepted', detail: { kind: 'host', ref: 'codex-thread:native-thread-1' },
    })
    expect(await transport.submit({ sessionId: 'session-1', message: user('m-1', 'first'), target: 'next-turn', wakeup: true })).toBe('accepted')
    view.message({ type: 'mcp-notification', hostId: 'local', message: {
      method: 'turn/started', params: { threadId: 'native-thread-1', turn: { id: 'native-turn-1', status: 'inProgress' } },
    } })
    expect(await transport.submit({ sessionId: 'session-1', message: user('m-2', 'queued'), target: 'next-turn', wakeup: true })).toBe('accepted')
    expect(turnStarts).toBe(1)
    expect(await transport.submit({ sessionId: 'session-1', message: user('m-3', 'steer'), target: 'next-step', wakeup: true })).toBe('accepted')
    expect(await transport.submit({ sessionId: 'session-1', message: user('m-4', 'inject'), target: 'next-step', wakeup: false })).toBe('accepted')
    expect(sent.filter(item => (item.request as Record<string, unknown> | undefined)?.method === 'turn/steer')).toHaveLength(1)
    expect(sent.filter(item => (item.request as Record<string, unknown> | undefined)?.method === 'thread/inject_items')).toHaveLength(1)

    view.message({ type: 'mcp-notification', hostId: 'local', message: {
      method: 'item/agentMessage/delta', params: { threadId: 'native-thread-1', turnId: 'native-turn-1', itemId: 'assistant-1', delta: 'hello' },
    } })
    view.message({ type: 'mcp-notification', hostId: 'local', message: {
      method: 'item/completed', params: { threadId: 'native-thread-1', turnId: 'native-turn-1', item: { type: 'agentMessage', id: 'assistant-1', text: 'hello' } },
    } })
    view.message({ type: 'mcp-notification', hostId: 'local', message: {
      method: 'item/started', params: { threadId: 'native-thread-1', turnId: 'native-turn-1', item: { type: 'commandExecution', id: 'tool-1', command: 'pwd', status: 'inProgress' } },
    } })
    view.message({ type: 'mcp-notification', hostId: 'local', message: {
      method: 'item/completed', params: { threadId: 'native-thread-1', turnId: 'native-turn-1', item: { type: 'commandExecution', id: 'tool-1', command: 'pwd', status: 'completed', aggregatedOutput: '/tmp' } },
    } })

    const approvalEvent = view.message({ type: 'mcp-request', hostId: 'local', request: {
    ...
    view.message({ type: 'mcp-request', hostId: 'local', request: {
      id: 'foreign-approval', method: 'item/commandExecution/requestApproval',
    ...
    const completed = { type: 'mcp-notification', hostId: 'local', message: {
      method: 'turn/completed', params: { threadId: 'native-thread-1', turn: { id: 'native-turn-1', status: 'completed' } },
    } }
    ...
    view.message({ type: 'mcp-notification', hostId: 'local', message: {
      method: 'turn/started', params: { threadId: 'native-thread-1', turn: { id: 'native-turn-2', status: 'inProgress' } },
    } })
    view.message({ type: 'mcp-notification', hostId: 'local', message: {
      method: 'turn/completed', params: { threadId: 'native-thread-1', turn: { id: 'native-turn-2', status: 'completed' } },
    } })
    await settle()

    expect(events).toEqual([
      'turn/start', 'step/start', 'assistant/chunk', 'assistant/message', 'tool/call', 'tool/result',
      'step/end', 'turn/end', 'turn/start', 'step/start', 'step/end', 'turn/end',
    ])
    expect(statuses).toEqual(['running', 'idle'])
    expect(new Set(claims)).toEqual(new Set(['m-1', 'm-2', 'm-3', 'm-4']))

    let replacements = 0
    transport.onReplacement(() => { replacements += 1 })
