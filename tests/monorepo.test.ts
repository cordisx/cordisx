...
describe('npm workspace boundary', () => {
  it('keeps orchestration private and the existing CLI in its owning workspace', async () => {
    ...
    expect(root.cordisxSources).toEqual(cli.cordisxSources)
    expect(cli.dependencies).not.toHaveProperty('@cordisx/channel')
    ...
    expect(cli.dependencies).toMatchObject({ ajv: '8.20.0', 'ajv-formats': '3.0.1' })
    expect(cli.devDependencies).not.toHaveProperty('ajv')
    ...
    await expect(access(path.join(repositoryRoot, 'examples/plugins/settings-tab-demo/index.ts'))).resolves
      .toBeUndefined()
    await expect(access(path.join(repositoryRoot, 'examples/plugins/settings-tab-demo/README.md'))).resolves
      .toBeUndefined()
