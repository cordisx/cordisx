...
function connector(): CordisXHostConnector {
  return {
    ...
    execute: vi.fn(async value => {
      if (value.type === 'conversation.open') {
        return {
          ok: true as const,
          value: { kind: 'opened' as const, conversation: 'conversation-opaque', run: 'run-opaque' },
        }
      }
      if (value.type === 'message.send') {
        return {
          ok: true as const,
          value: { kind: 'sent' as const, conversation: value.conversation, run: 'run-after-send' },
        }
      }
      if (value.type === 'run.stop') {
        return {
          ok: true as const,
          value: { kind: 'stopped' as const, conversation: value.conversation, run: value.run },
        }
      }
...
describe('Host Connector broker', () => {
  it('Host-stamps registrations, preserves opaque handles, publishes ordered events, and redacts its snapshot', async () => {
    ...
    expect(registered).toMatchObject({
      ...
      value: {
        registration: {
          registrationId: 'cxconnector:host-issued-registration',
          connectorId: 'fixture.connector',
          generation: 1,
        },
      },
    ...
    const subscription = broker.subscribe(registration, -1, event => {
      events.push(event)
    })
    expect(subscription.ok).toBe(true)

    await expect(broker.command(command(registration, 'open', { type: 'conversation.open', open: { mode: 'create' } })))
      .resolves.toMatchObject({ ok: true, value: { conversation: 'conversation-opaque' } })
    await broker.command(command(registration, 'send', {
      type: 'message.send',
      conversation: 'conversation-opaque',
      message: { messageId: 'message-opaque', direction: 'outbound', parts: [{ kind: 'text', text: 'hello' }] },
    }))
    await broker.command(
      command(registration, 'stop', { type: 'run.stop', conversation: 'conversation-opaque', run: 'run-opaque' }),
    )
    await broker.command(
      command(registration, 'close', { type: 'conversation.close', conversation: 'conversation-opaque' }),
    )
    ...
    expect(broker.snapshot()).toMatchObject({
      ...
      registrations: [{ state: 'active', eventCount: 6 }],
    })
  ...
  it('fences replacement/disposal by registration generation and cannot accept raw command fields', async () => {
    ...
    broker.subscribe(first.value.registration, -1, event => {
      observed.push(event)
    })
    ...
    await expect(
      broker.command(command(first.value.registration, 'old', { type: 'conversation.open', open: { mode: 'create' } })),
    ).resolves.toMatchObject({ ok: false, error: { code: 'registration-disposed' } })
    const raw = {
      ...command(second.value.registration, 'bad', { type: 'conversation.open', open: { mode: 'create' } }),
      rawBridge: 'forbidden',
    } as CordisXConnectorCommand
  ...
  it('applies Host permission before dispatch and keeps every current Agent native operation typed unavailable', async () => {
    const denied = new CordisXConnectorBroker({
      authorize: vi.fn(async () => ({
        ok: false as const,
        error: { code: 'permission-denied' as const, message: 'denied' },
      })),
    })
    ...
    await expect(
      denied.command(
        command(deniedRegistration.value.registration, 'denied', {
    ...
    const commands: readonly CordisXConnectorCommand[] = [
      ...
      command(registration, 'send', {
        type: 'message.send',
        conversation: 'not-issued',
    ...
    for (const item of commands) {
      await expect(broker.command(item)).resolves.toMatchObject({
        ok: false,
  ...
  it('injects only a Host-bound public client, returns typed authorization outcomes, and keeps the wire subscription serializable', async () => {
    ...
    expect(Object.keys(client)).not.toEqual(
      expect.arrayContaining(['caller', 'principal', 'identity', 'bridge', 'transport']),
    )
    const discovery = await client.discover()
    expect(discovery).toMatchObject({
      status: 'accepted',
      type: 'discover',
      snapshot: { registrations: [{ registration }] },
    })
    ...
    const opened = await client.execute(
      command(registration, 'public-open', { type: 'conversation.open', open: { mode: 'create' } }),
    )
    expect(opened).toMatchObject({
      status: 'accepted',
      execution: { kind: 'conversation.opened', conversation: 'conversation-opaque' },
    })
    const subscribed = await client.subscribe(registration, -1)
    expect(subscribed.result).toMatchObject({
      status: 'accepted',
      type: 'subscribe',
      subscription: { registration, afterSequence: -1, snapshotSequence: 1 },
    })
    ...
    const deniedResult = await denied.execute(
      command(registration, 'denied-public', { type: 'conversation.open', open: { mode: 'create' } }),
    )
  ...
  it('fences public runs to their conversation and reports the builtin Codex connector as unavailable without inventing a write path', async () => {
    ...
    await expect(
      client.execute(command(registration, 'native-open', { type: 'conversation.open', open: { mode: 'create' } })),
    )
    ...
    const fakeClient = fakeBroker.bind({
      active: () => true,
      authorize: async capability => ({ capability, state: 'allowed' as const, code: 'allowed' as const }),
    })
    await fakeClient.execute(
      command(fake.value.registration, 'open-for-run', { type: 'conversation.open', open: { mode: 'create' } }),
    )
    const wrongRun = await fakeClient.execute(
      command(fake.value.registration, 'wrong-run', {
        type: 'run.stop',
        conversation: 'conversation-opaque',
        run: 'run-after-send',
      }),
  ...
  it('serializes replay and live delivery across concurrent and reentrant producers, and fences unsubscribe and replacement', async () => {
    ...
    if (!('handle' in subscribed)) throw new Error('subscription should be accepted')
    reenter = async () =>
      await broker.command(command(registration, 'nested', { type: 'conversation.open', open: { mode: 'create' } }))
    const outer = broker.command(
      command(registration, 'outer', { type: 'conversation.open', open: { mode: 'create' } }),
    )
    const parallel = broker.command(
      command(registration, 'parallel', { type: 'conversation.open', open: { mode: 'create' } }),
    )
    ...
    expect(terminal).toMatchObject({
      ...
      value: { events: [{ type: 'connector.disposed', disposeReason: 'generation-replaced', sequence: 4 }] },
    })
  ...
  it('suppresses prequeued replay pages on explicit unsubscribe', async () => {
    ...
    await client.execute(
      command(registered.value.registration, 'replay-open', { type: 'conversation.open', open: { mode: 'create' } }),
    )
  ...
  it('suppresses prequeued replay pages on owner client dispose', async () => {
    ...
    const client = boundClient(broker)
    await client.execute(
      command(registered.value.registration, 'replay-owner-open', {
        type: 'conversation.open',
        open: { mode: 'create' },
      }),
    )
  ...
  it('suppresses prequeued live pages on explicit unsubscribe', async () => {
    ...
    const client = boundClient(broker)
    await client.execute(
      command(registered.value.registration, 'live-open', { type: 'conversation.open', open: { mode: 'create' } }),
    )
    const handle = acceptedHandle(await client.subscribe(registered.value.registration, 1))
  ...
  it('suppresses prequeued live pages on owner client dispose', async () => {
    ...
    const client = boundClient(broker)
    await client.execute(
      command(registered.value.registration, 'live-owner-open', {
        type: 'conversation.open',
        open: { mode: 'create' },
      }),
    )
  ...
  it('keeps cancellation a no-op after cleanup and rejects a post-cancel queue mutation', async () => {
    ...
    const client = boundClient(broker)
    await client.execute(
      command(registered.value.registration, 'mutation-open', { type: 'conversation.open', open: { mode: 'create' } }),
    )
    const handle = acceptedHandle(await client.subscribe(registered.value.registration, -1))
