...
const requester = {
  agentId: 'cx-session.reviewer',
  sessionId: 'cx-session.reviewer',
  agentGeneration: 2,
  definition: { agentId: 'reviewer', revision: 'r1' },
} as const
const authority = {
...
const scope = {
  authorityRequester: {
    ...
    requester: { kind: 'host-route-param' as const, routeId: 'room-session-detail', param: 'sessionId' as const },
  },
}
...
describe('plugin-manifest/v8 correlated approval authority', () => {
  ...
  it('mints a Lead authority lease only from the exact active Reviewer route and fences route replacement', async () => {
    ...
    const lease = await authorityScope.mintApprovalAuthorityLease(owner, {
      routingId: 'route-1',
    ...
    expect(authorityScope.approvalAuthorityLeaseActive(owner, lease, requester, { ...authority, agentGeneration: 4 }))
      .toBe(false)
    active = {
      owner,
      routeId: 'room-session-detail',
      instanceId: 'reviewer-route-next',
      params: { sessionId: requester.sessionId },
    }
  ...
  it('authorizes a v8 authority lease only through the current exact requester route', async () => {
    ...
    const reviewerRoute = {
      ...
      routeId: 'room-session-detail',
      routeInstanceId: 'main:reviewer-route',
    ...
    let active: AgentActiveRoute = {
      ...
      instanceId: reviewerRoute.routeInstanceId,
      params: reviewerRoute.params,
    }
    const scopeAuthority = new AgentRouteSessionScopeAuthority({
      activeRoute: () => active,
      routes: candidate =>
        candidate.pluginId === agentOwner.pluginId && candidate.generation === agentOwner.generation
    ...
    await expect(broker.authorizeDevelopmentAgentRuntime(developmentAuthority, {
      ...
      scopeSource: {
        kind: 'host-route',
        routeInstanceId: reviewerRoute.routeInstanceId,
        routeId: reviewerRoute.routeId,
        path: reviewerRoute.path,
        params: reviewerRoute.params,
      },
      connection,
    })).resolves.toEqual({ authorized: false })
    ...
    active = {
      ...
      instanceId: nextRoute.routeInstanceId,
