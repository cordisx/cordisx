...
describe('OwnerDocumentStore', () => {
  it('persists an outbox operation across Host restart and enforces atomic CAS', async () => {
    ...
    const accepted = await first.replace({
      scope,
    ...
    await expect(cold.load(scope, 'room-registry')).resolves.toEqual({
      status: 'loaded',
      snapshot: (accepted as { snapshot: unknown }).snapshot,
    })
    const [winner, loser] = await Promise.all([
      cold.replace({
        ...
        expectedRevision: 1,
        schemaVersion: 3,
        value: { ...payload, state: 'sending-unknown' },
      }),
      cold.replace({
        scope,
        documentId: 'room-registry',
        expectedRevision: 1,
        schemaVersion: 3,
        value: { ...payload, state: 'committed' },
      }),
    ])
    expect([winner.status, loser.status].sort()).toEqual(['accepted', 'conflict'])
    expect([winner, loser].find(result => result.status === 'conflict')).toEqual({
      status: 'conflict',
      actualRevision: 2,
    })
  })
  ...
  it('linearizes the same expected revision across independent Node processes', async () => {
    const home = await root()
    await new OwnerDocumentStore(home).replace({
      scope,
      documentId: 'room-registry',
      expectedRevision: 0,
      schemaVersion: 3,
    ...
    const run = (state: string) =>
      new Promise<Record<string, unknown>>((resolve, reject) => {
        const child = spawn(process.execPath, ['--import', 'tsx', fixture, home, gate, state], {
          cwd: process.cwd(),
        ...
        child.once('close', code =>
          code === 0 ? resolve(JSON.parse(stdout.trim()) as Record<string, unknown>) : reject(new Error(stderr)))
      })
    const first = run('sending-unknown')
    const second = run('committed')
    await writeFile(gate, 'go')
    const results = await Promise.all([first, second])
    expect(results.map(item => item.status).sort()).toEqual(['accepted', 'conflict'])
    await expect(new OwnerDocumentStore(home).load(scope, 'room-registry')).resolves.toMatchObject({
      status: 'loaded',
      snapshot: { revision: 2 },
    })
  })
  ...
  it('isolates profile, source, and plugin id while ignoring runtime/module generations by construction', async () => {
    ...
    await expect(store.load({ ...scope, profileId: 'other' }, 'rooms')).resolves.toEqual({
      status: 'missing',
      revision: 0,
    })
    await expect(
      store.load({ ...scope, identity: { ...scope.identity, source: 'https://plugins.example/replacement' } }, 'rooms'),
    ).resolves.toEqual({ status: 'missing', revision: 0 })
    await expect(store.load({ ...scope, identity: { ...scope.identity, pluginId: 'other' } }, 'rooms')).resolves
      .toEqual({ status: 'missing', revision: 0 })
    await expect(store.load(scope, 'rooms')).resolves.toMatchObject({
      status: 'loaded',
      snapshot: { value: { owner: 'chatroom' } },
    })
  })
  ...
  it('fails closed on corruption without overwriting recoverable bytes', async () => {
    ...
    await expect(store.load(scope, 'rooms')).resolves.toEqual({
      status: 'unavailable',
      code: 'corrupt-store',
      diagnostic: 'owner document store is corrupt',
      recoverable: true,
    })
    await expect(
      store.replace({ scope, documentId: 'rooms', expectedRevision: 1, schemaVersion: 2, value: { migrated: true } }),
    )
  ...
  it('preserves unsupported Host envelopes and leaves consumer migration entirely CAS-owned', async () => {
    ...
    expect(legacy).toMatchObject({
      status: 'loaded',
      snapshot: { revision: 1, schemaVersion: 1, value: { legacy: true } },
    })
    ...
    await expect(store.load(scope, 'rooms')).resolves.toEqual({
      ...
      diagnostic: 'store schema is unsupported',
      recoverable: true,
    })
    await expect(
      store.replace({ scope, documentId: 'rooms', expectedRevision: 2, schemaVersion: 3, value: { no: 'overwrite' } }),
    )
