...
import {
  channelTaskScopeFingerprint,
  LauncherChannelTaskGateway,
  StaticChannelWorkspaceResolver,
} from '../packages/cli/src/launcher/channel-task-gateway.js'
  ...
  return {
    ...
    async request<Result>(method: string): Promise<Result> {
      ...
      if (method === 'thread/start') {
        return {
          thread: { id: 'session-1', modelProvider: 'alpha', cwd: '/workspace', createdAt: 1, updatedAt: 1 },
        } as Result
      }
...
describe('launcher Channel task gateway', () => {
  it('consumes one Host-private grant and preserves a created session when its initial turn fails', async () => {
    ...
    const input = {
      contract: 'cordisx.channel-user-input/v1' as const,
      ...
      source: {
        ...
        event: {
          ...
          threadId: 'one',
          semantics: 'conversation' as const,
          eventId: 'event-1',
        },
      },
    ...
    const result = await gateway.execute({
      ...
      routeId: 'route-1',
      input,
      serviceGeneration: 'channel-1',
      configurationRevision: 1,
    })
    expect(result).toMatchObject({
      session: { providerId: 'alpha', remoteSessionId: 'session-1' },
      dispatch: { status: 'created-initial-turn-failed', lifecycle: { afterSequence: 0 } },
    })
    expect(
