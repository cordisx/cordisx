...
const identitySetup: AgentSetup = {
  ...
  definitions: [{
    ...
    schemaVersion: 1,
    identity: { agentId: 'chatroom.lead', revision: 'durable-identity-1' },
    name: 'Lead',
    inherit: {
      promptSections: 'none',
      rules: 'none',
...
describe('Playground durable Agent Session store', () => {
  it('recovers from the same explicit Playground home and rejects a replaced launcher generation', async () => {
    ...
    const request = (token: string, runtimeGeneration: string, requestId: string, input: Record<string, unknown>) =>
      JSON.stringify({
        version: 1,
        token,
        runtimeGeneration,
    ...
    try {
      ...
      expect(
        await first.handleAgentSessionStoreRequest(request(firstToken, firstComposition.generation, 'create-1', {
          operation: 'create',
          session: persisted,
        })),
      ).toMatchObject({ requestId: 'create-1', value: { status: 'accepted', nextSeq: 1 } })
      expect(
        await first.handleAgentSessionStoreRequest(request(firstToken, firstComposition.generation, 'setup-1', {
          operation: 'update-setup',
      ...
      expect(
        await first.handleAgentSessionStoreRequest(request(firstToken, firstComposition.generation, 'setup-stale', {
          operation: 'update-setup',
          sessionId: persisted.id,
          sessionGeneration: 2,
          setup: identitySetup,
      ...
      expect(
        await second.handleAgentSessionStoreRequest(
          request(secondToken, secondComposition.generation, 'load-2', { operation: 'load' }),
        ),
      )
        .toMatchObject({
          requestId: 'load-2',
      ...
      expect(
        await second.handleAgentSessionStoreRequest(
          request(firstToken, firstComposition.generation, 'stale-1', { operation: 'load' }),
        ),
      )
      ...
      expect(
        await second.handleAgentSessionStoreRequest(
          request(resetToken, resetComposition.generation, 'load-reset', { operation: 'load' }),
        ),
      )
  ...
  it('recovers exact SessionEvent replay, then appends fenced live facts after a Host restart', async () => {
    ...
    try {
      ...
      expect(pages[0]).toMatchObject({
        phase: 'replay',
        replayThrough,
        seqs: expect.arrayContaining([0, replayThrough]),
      })
      ...
      expect(resumed).toMatchObject({
        sessionId: 'cx-session.room-7-run-3',
        sessionGeneration: 1,
        disposition: 'resumed',
      })
      ...
      expect(secondSnapshot).toMatchObject({
        status: 'available',
        snapshot: { sessionId: 'cx-session.room-7-run-3', sessionGeneration: 1 },
      })
      const live = pages.filter(page => page.phase === 'live')
      expect(live.length).toBeGreaterThan(0)
      expect(live.every(page => page.replayThrough === replayThrough && page.seqs.every(seq => seq > replayThrough)))
        .toBe(true)
  ...
  it('loads a v1 SessionEvent-only ledger and upgrades identity through the same Session record on resume', async () => {
    ...
    try {
      ...
      await store.create({
        id: 'cx-session.legacy-without-identity',
        generation: 1,
      ...
      const runtime = new CordisXAgentSessionRuntime({
        driver: new DeterministicAgentSessionTransport(legacy),
        authorize: async () => true,
        persistence: store,
      ...
      const recovered = new CordisXAgentSessionRuntime({
        driver: new DeterministicAgentSessionTransport(upgraded),
        authorize: async () => true,
        initialSessions: upgraded,
      })
      expect(recovered.definitionPresentation(identitySetup.definition)).toMatchObject({
        name: 'Lead',
        introduction: 'Lead the Room.',
      })
  ...
  it('atomically fences divergent cursors, Session generations, and non-JSON structured-clone values', async () => {
    ...
    try {
      const store = new PlaygroundAgentSessionStore(home)
      await store.create({
        id: 'cx-session.fences',
        generation: 3,
        header: { id: 'cx-session.fences', formatVersion: 1, createdAt: 10, isSeeded: false },
        events: [],
      })
      await store.append({
        sessionId: 'cx-session.fences',
        sessionGeneration: 3,
        expectedSeq: 0,
        events: [turnStart('cx-session.fences', 0, 1)],
      })
      await expect(
        store.append({
          sessionId: 'cx-session.fences',
      ...
      await expect(
        store.append({
          ...
          sessionGeneration: 4,
          expectedSeq: 1,
          events: [turnStart('cx-session.fences', 1, 2)],
        }),
      ...
      const concurrent = await Promise.allSettled([
        ...
        store.append({
          ...
          sessionGeneration: 3,
          expectedSeq: 1,
          events: [turnStart('cx-session.fences', 1, 9)],
        }),
      ])
      ...
      await expect(
        store.append({ sessionId: 'cx-session.fences', sessionGeneration: 3, expectedSeq: 2, events: [invalid] }),
      )
