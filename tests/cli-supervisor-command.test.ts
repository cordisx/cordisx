...
describe('supervisor management commands', () => {
  it('emits stable status JSON and cleans a stale state record instead of trusting its PID', async () => {
    ...
    expect(JSON.parse(output.pop()!)).toMatchObject({ status: 'stopped', pid: null })
  })
