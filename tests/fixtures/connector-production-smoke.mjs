...
const parsed = parseArgs({ options: {
  port: { type: 'string' }, report: { type: 'string' }, screenshot: { type: 'string' }, 'connector-harness-policy': { type: 'string' }, 'connector-harness-scenario': { type: 'string' },
} })
const port = Number(parsed.values.port)
if (!Number.isInteger(port) || parsed.values.report === undefined) throw new Error('connector production smoke requires --port and --report')
...
socket.on('message', data => {
  ...
  if (message.method === 'Runtime.exceptionThrown') runtimeExceptions.push(message.params?.exceptionDetails?.text ?? 'renderer exception')
...
for (let attempt = 0; attempt < 300; attempt += 1) {
  const evaluated = await send('Runtime.evaluate', { expression: "document.documentElement.dataset.connectorHarnessReport ?? null", returnByValue: true })
  const text = evaluated.result?.value
  if (typeof text === 'string') { assertion = JSON.parse(text); break }
...
const expected = assertion.status === 'passed'
  ...
  && (policy === 'allow'
    ? scenario === 'flow'
      ? assertion.listenerBeforeWatermark === true && assertion.replayToLive === true && assertion.replacementTerminal === true
...
const report = { appUrl: target.url, assertions: assertion, runtimeExceptions, ...(screenshot === undefined ? {} : { screenshot }) }
