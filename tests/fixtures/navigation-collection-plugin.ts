...
const feedback = {
  success: message('action.success', 'Action completed'),
  failure: message('action.failure', 'Action failed'),
}
...
let snapshot: CordisXNavigationCollectionSnapshotV3 = {
  ...
  items: [
    {
      id: 'latest',
      label: message('room.latest', 'Latest room'),
      ...
      actions: [
        {
          kind: 'command',
          id: 'pin',
          label: message('action.pin', 'Pin'),
          icon: 'host:pin',
          placement: 'direct',
          tone: 'neutral',
          pressed: false,
          disabled: { value: false },
          command: { id: 'pin-room', arguments: { roomId: 'latest' } },
          feedback,
        },
        {
          kind: 'copy-route-link',
          id: 'copy-link',
          label: message('action.copy-link', 'Copy link'),
          icon: 'host:link',
          placement: 'overflow',
          tone: 'neutral',
          pressed: false,
        ...
        },
        {
          kind: 'copy-text',
          id: 'copy-id',
          label: message('action.copy-id', 'Copy ID'),
          icon: 'host:copy',
          placement: 'overflow',
          tone: 'neutral',
          pressed: false,
        ...
        },
        {
          kind: 'command',
          id: 'delete',
          label: message('action.delete', 'Delete'),
          icon: 'host:delete',
          placement: 'overflow',
          tone: 'danger',
          pressed: false,
          disabled: { value: false },
          command: { id: 'delete-room', arguments: { roomId: 'latest' } },
          confirmation: {
            title: message('delete.title', 'Delete room?'),
            description: message('delete.description', 'This cannot be undone.'),
            confirmLabel: message('delete.confirm', 'Delete'),
          },
          feedback,
        },
      ],
    ...
    {
      id: 'older',
      label: message('room.older', 'Older room'),
      leadingVisual: imageVisual(),
...
export function apply(ctx: Context): void {
  ...
  ctx.commands.register({ id: 'pin-room', title: message('action.pin', 'Pin') }, () => {
    commands.push('pin')
  })
  ctx.commands.register({ id: 'delete-room', title: message('action.delete', 'Delete') }, () => {
    commands.push('delete')
  })
  ctx.i18n.define({
    namespace: 'navigation-collection',
    locale: 'en',
    default: true,
    messages: {
      'navigation.new': 'New room',
      'navigation.rooms': 'Rooms',
      'room.latest': 'Latest room',
      'room.older': 'Older room',
      'page.new.title': 'Start a room',
      'page.new.description': 'Composes the first message for a new room.',
      'page.room.title': 'Room conversation',
      'page.room.description': 'Shows one selected room conversation.',
      'route.new.title': 'New room',
      'route.new.description': 'Open a new room.',
      'route.room.title': 'Open room',
      'route.room.description': 'Open an existing room.',
    },
  })
  ctx.i18n.define({
    namespace: 'navigation-collection',
    ...
    messages: {
      ...
      'room.latest': '最新房间',
      'room.older': '较早房间',
      'page.new.title': '开始新房间',
      'page.new.description': '为新房间编写第一条消息。',
      'page.room.title': '房间会话',
      'page.room.description': '显示当前选中的房间会话。',
      'route.new.title': '新建房间',
      'route.new.description': '打开一个新房间。',
      'route.room.title': '打开房间',
      'route.room.description': '打开已有房间。',
    },
  })
  ctx.pages.register({
