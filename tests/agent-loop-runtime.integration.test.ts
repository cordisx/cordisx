  ...
  it('drives text create/send and proactive assistant, approval, and lifecycle events through ctx.agentLoop', async () => {
    ...
    const config = {
      ...
      providers: [{
        ...
        codexHome: '/tmp/cordisx-agent-loop-gateway',
        enabled: true,
        timeoutMs: 1_000,
      }],
    ...
    Object.defineProperty(dom.window, '__cordisxPermissionPolicyRequestV1', {
      ...
      value: (payload: string) => {
        ...
        queueMicrotask(() =>
          (dom.window as unknown as { __cordisxPermissionPolicyReceiveV1?: (response: string) => void })
            .__cordisxPermissionPolicyReceiveV1?.(
              JSON.stringify({ requestId: request.requestId, ok: true, value: request.records }),
            )
        )
      },
    })
      ...
      value: (payload: string) => {
        ...
        if (request.operation === 'status') {
          ...
          value = [{
            ...
            displayName: 'Gateway A',
            generation: 'gateway-generation-1',
            state: 'ready',
          }]
          ...
          value = {
            ...
            value: {
              ...
              models: [{
                ...
                schemaVersion: 1,
                ref: { providerId: 'gateway-a', modelId: 'model-1' },
                hostId: 'cli-proxy-api:gateway-a',
                label: 'Model 1',
                isDefault: true,
              }],
            },
          ...
          value = {
            ...
            value: {
              ...
              schemaVersion: 1,
              ref: { providerId: 'gateway-a', remoteSessionId: 'session-1' },
              hostId: 'cli-proxy-api:gateway-a',
              model: { providerId: 'gateway-a', modelId: 'model-1' },
              cwd: root,
              state: 'active',
            },
          }
          ...
          value = {
            ok: true,
            value: {
              contract: 'cordisx.platform-turn-start/v1',
              schemaVersion: 1,
          ...
          const events = turnStarted
            ? [
              ...
              },
              {
                ...
                turnId: 'turn-1',
                type: 'turn.completed',
                output: [{ type: 'text', text: 'Assistant reply' }],
              },
        ...
        queueMicrotask(() =>
          (dom.window as unknown as { __cordisxProviderReceiveV1?: (response: string) => void })
    ...
    const createdPromise = fixture.client.createOrBind({
      ...
      commandId: 'create-1',
      type: 'create-or-bind',
      definition: { agentId: 'internal-assistant', revision: 'r1' },
      definitions: [{
    ...
    const created = await Promise.race([
      ...
      new Promise<never>((_resolve, reject) =>
        setTimeout(() => reject(new Error(`AgentLoop create timeout: ${JSON.stringify(requests)}`)), 2_000)
      ),
    ])
    ...
    const subscription = await Promise.race([
      ...
      new Promise<never>((_resolve, reject) =>
        setTimeout(() => reject(new Error(`AgentLoop subscribe timeout: ${JSON.stringify(requests)}`)), 2_000)
      ),
    ])
    ...
    const nextPage = async () =>
      await Promise.race([
        iterator.next(),
        new Promise<never>((_resolve, reject) =>
          setTimeout(() => reject(new Error(`AgentLoop event timeout: ${JSON.stringify(requests)}`)), 2_000)
        ),
      ])
    await nextPage()
    const sent = await Promise.race([
      fixture.client.send({
        ...
        content: [{ kind: 'text', text: 'Hello AgentLoop' }],
      }),
      new Promise<never>((_resolve, reject) =>
        setTimeout(() => reject(new Error(`AgentLoop send timeout: ${JSON.stringify(requests)}`)), 2_000)
      ),
    ])
    ...
    expect(requests.find(request => request.operation === 'turns.submit')?.input).toMatchObject({
      message: 'Hello AgentLoop',
    })
    ...
    expect((dom.window as unknown as { __cordisxAgentLoopRuntimeFixture?: unknown }).__cordisxAgentLoopRuntimeFixture)
      .toBeUndefined()
    dom.window.close()
  })
})
