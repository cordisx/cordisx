...
const REQUEST = 'https://raw.githubusercontent.com/cordisx/cordisx-protocol/main/schemas/host-dom-bridge-request.v1.schema.json'
const RESULT = 'https://raw.githubusercontent.com/cordisx/cordisx-protocol/main/schemas/host-dom-bridge-result.v1.schema.json'
const CATALOG = 'https://raw.githubusercontent.com/cordisx/cordisx-protocol/main/schemas/host-dom-root-catalog.v1.schema.json'
...
function acceptedLease(capability: 'ui.host-dom.read' | 'ui.host-dom.modify', id: string): HostDomPermissionAccessDecision {
  return {
    authorized: true,
    state: 'allowed',
    ...
    lease: {
      ...
      key: {
        ...
        capability,
        scope: { rootIds: ['root.one', 'root.two'], operations: capability === 'ui.host-dom.read' ? ['inspect-structure', 'read-text', 'read-attributes', 'read-state'] : ['set-text', 'insert-owned-structured-child'] },
...
function roots(document: Document): readonly HostDomRootDefinition[] {
  return ['root.one', 'root.two'].map(rootId => ({
    ...
    modifyOperations: ['set-text', 'set-attribute', 'insert-owned-structured-child', 'remove-owned-child', 'focus'] as const,
...
function binding() {
  ...
  return {
    leases,
    listeners,
    setState(next: typeof state) { state = next; for (const listener of listeners) listener() },
    revoke(id: string) { leases.delete(id); for (const listener of listeners) listener() },
    value: {
...
describe('Host DOM authority', () => {
  it('reports unavailable and never authorizes in the current shared-renderer boundary', async () => {
    ...
    const result = await client.request({
      ...base('acquire-1'), type: 'acquire', capability: 'ui.host-dom.read', rootId: 'root.one', operations: ['read-text'],
    })
    expectFormal(validators.result, result)
    expect(result).toMatchObject({ status: 'unavailable', code: 'unsupported' })
    expect(authorize).not.toHaveBeenCalled()
    authority.dispose(); dom.window.close()
  })

  it('performs bounded reads and reversible writes with opaque owner/root/generation fenced handles', async () => {
    const validators = await formalHostDomValidators()
    const dom = new JSDOM('<body><main data-root="root.one"><span id="label">Before</span><span data-cordisx-private="true">Secret</span><span>Visible</span></main><aside data-root="root.two"><span>Other</span></aside></body>', { pretendToBeVisual: true })
    const owner = binding()
    const authority = new HostDomAuthority({ hostGeneration: 'host-1', isolatedPluginBoundary: true, roots: roots(dom.window.document) })
    const client = authority.bind(owner.value)
    const readHandle = await client.request({
      ...base('read-handle'), type: 'acquire', capability: 'ui.host-dom.read', rootId: 'root.one', operations: ['inspect-structure', 'read-text'],
    })
    ...
    const structure = await client.request({
      ...base('inspect'), type: 'read', handle: readHandle.handle, operation: 'inspect-structure',
    })
    ...
    const text = await client.request({ ...base('read-text'), type: 'read', handle: readHandle.handle, operation: 'read-text' })
    expectFormal(validators.result, text)
    expect(text).toMatchObject({
      status: 'accepted', projection: { kind: 'text', text: 'BeforeVisible', redacted: true, truncated: false },
    })
    expect(JSON.stringify(text)).not.toContain('Secret')

    const modifyHandle = await client.request({
      ...base('modify-handle'), type: 'acquire', capability: 'ui.host-dom.modify', rootId: 'root.one', operations: ['set-text', 'insert-owned-structured-child'],
    })
    ...
    const changed = await client.request({
      ...base('set-text'), type: 'modify', handle: modifyHandle.handle, node: label, operation: 'set-text', text: 'After',
    })
    ...
    const otherHandle = await client.request({
      ...base('other-handle'), type: 'acquire', capability: 'ui.host-dom.modify', rootId: 'root.two', operations: ['set-text'],
    })
    if (otherHandle.status !== 'accepted' || otherHandle.type !== 'acquire') throw new Error('other handle expected')
    await expect(client.request({
      ...base('cross-root'), type: 'modify', handle: otherHandle.handle, node: label, operation: 'set-text', text: 'Crossed',
    })).resolves.toMatchObject({ status: 'denied', code: 'scope-denied' })

    await expect(client.request({
      ...base('raw-selector'), type: 'modify', handle: modifyHandle.handle, node: label, operation: 'set-text', text: 'Bad', selector: '#label',
    ...
    client.dispose(); authority.dispose(); dom.window.close()
  })
  ...
  it('redacts executable, hidden, and private subtrees from text and structure projections', async () => {
    const dom = new JSDOM(`<body><main data-root="root.one"><span>Visible</span><style>.secret{content:'css-secret'}</style><script>script-secret</script><span hidden>hidden-secret</span><span aria-hidden="true">aria-secret</span><span style="display:none">display-secret</span></main><aside data-root="root.two"></aside></body>`, { pretendToBeVisual: true })
    const owner = binding()
    const authority = new HostDomAuthority({ hostGeneration: 'host-1', isolatedPluginBoundary: true, roots: roots(dom.window.document) })
    const client = authority.bind(owner.value)
    const acquired = await client.request({
      ...base('redacted-acquire'), type: 'acquire', capability: 'ui.host-dom.read', rootId: 'root.one', operations: ['inspect-structure', 'read-text', 'read-attributes', 'read-state'],
    })
    if (acquired.status !== 'accepted' || acquired.type !== 'acquire') throw new Error('read handle expected')
    const text = await client.request({ ...base('redacted-text'), type: 'read', handle: acquired.handle, operation: 'read-text' })
    ...
    const structure = await client.request({ ...base('redacted-structure'), type: 'read', handle: acquired.handle, operation: 'inspect-structure' })
    expect(structure).toMatchObject({ status: 'accepted', projection: { kind: 'structure', redacted: true } })
    if (structure.status !== 'accepted' || structure.type !== 'read' || structure.projection.kind !== 'structure') throw new Error('structure expected')
    ...
    await expect(client.request({
      ...base('ancestor-hidden-text'), type: 'read', handle: acquired.handle, node: visible, operation: 'read-text',
    })).resolves.toMatchObject({ status: 'accepted', projection: { kind: 'text', text: '', redacted: true } })
    await expect(client.request({
      ...base('ancestor-hidden-attributes'), type: 'read', handle: acquired.handle, node: visible, operation: 'read-attributes', attributes: ['title'],
    })).resolves.toMatchObject({ status: 'accepted', projection: { kind: 'attributes', attributes: [], redacted: true } })
    await expect(client.request({
      ...base('ancestor-hidden-state'), type: 'read', handle: acquired.handle, node: visible, operation: 'read-state',
    })).resolves.toMatchObject({ status: 'accepted', projection: { kind: 'state', visible: false, enabled: false, redacted: true } })
    ...
    const scriptAuthority = new HostDomAuthority({
      hostGeneration: 'host-1', isolatedPluginBoundary: true,
    ...
    const scriptHandle = await scriptClient.request({
      ...base('script-acquire'), type: 'acquire', capability: 'ui.host-dom.read', rootId: 'root.one', operations: ['read-text'],
    })
    ...
    await expect(scriptClient.request({
      ...base('script-read'), type: 'read', handle: scriptHandle.handle, operation: 'read-text',
    })).resolves.toMatchObject({ status: 'denied', code: 'scope-denied' })
    scriptClient.dispose(); scriptAuthority.dispose(); client.dispose(); authority.dispose(); dom.window.close()
  })
  ...
  it('serializes overlapping modify roots so lease rollback cannot restore an intermediate value', async () => {
    ...
    const read = await client.request({
      ...base('overlap-read'), type: 'acquire', capability: 'ui.host-dom.read', rootId: 'root.one', operations: ['inspect-structure'],
    })
    if (read.status !== 'accepted' || read.type !== 'acquire') throw new Error('read handle expected')
    const structure = await client.request({ ...base('overlap-inspect'), type: 'read', handle: read.handle, operation: 'inspect-structure' })
    const rootNode = nodeFrom(structure, 0)
    const first = await client.request({
      ...base('modify-first'), type: 'acquire', capability: 'ui.host-dom.modify', rootId: 'root.one', operations: ['set-text'],
    })
    if (first.status !== 'accepted' || first.type !== 'acquire') throw new Error('modify handle expected')
    await expect(client.request({
      ...base('modify-second'), type: 'acquire', capability: 'ui.host-dom.modify', rootId: 'root.one', operations: ['set-text'],
    })).resolves.toMatchObject({ status: 'denied', code: 'operation-denied' })
    const root = dom.window.document.querySelector('[data-root="root.one"]')!
    await client.request({ ...base('modify-value'), type: 'modify', handle: first.handle, node: rootNode, operation: 'set-text', text: 'After' })
    expect(root.textContent).toBe('After')
    await vi.advanceTimersByTimeAsync(60_000)
    expect(root.textContent).toBe('Before')
    await expect(client.request({
      ...base('modify-after-expiry'), type: 'acquire', capability: 'ui.host-dom.modify', rootId: 'root.one', operations: ['set-text'],
  ...
  it('freezes untrusted requests across authorization and rejects mismatched or stale leases before minting', async () => {
    ...
    const authorization = new Promise<HostDomPermissionAccessDecision>(resolve => { resolveAuthorization = resolve })
    ...
    const untrusted = {
      ...base('toctou'), type: 'acquire' as const, capability: 'ui.host-dom.read' as const,
      rootId: 'root.one', operations: ['read-text'] as ['read-text'],
    }
    ...
    await expect(pending).resolves.toMatchObject({
      status: 'accepted', capability: 'ui.host-dom.read', rootId: 'root.one', operations: ['read-text'],
    })
    ...
    await expect(mismatched.request({
      ...base('mismatch'), type: 'acquire', capability: 'ui.host-dom.read', rootId: 'root.one', operations: ['read-text'],
    ...
    await expect(mismatched.request({
      ...base('stale-root'), type: 'acquire', capability: 'ui.host-dom.read', rootId: 'root.one', operations: ['read-text'],
    })).resolves.toMatchObject({ status: 'unavailable', code: 'not-mounted' })
    client.dispose(); mismatched.dispose(); authority.dispose(); dom.window.close()
  })
  ...
  it('reserves the handle cap across concurrent authorization and validates only a cloned getter snapshot', async () => {
    ...
    const authority = new HostDomAuthority({ hostGeneration: 'host-1', isolatedPluginBoundary: true, roots: roots(dom.window.document) })
    const client = authority.bind({
      ownerKey: 'https://plugins.example/demo\u0000demo', profileId: 'work',
      identity: { source: 'https://plugins.example/demo', pluginId: 'demo' },
      runtimeGeneration: 'runtime-1', moduleGeneration: 'module-1', state: () => 'active',
      authorize: async () => await new Promise(resolve => authorizations.push(resolve)),
      leaseActive: id => activeLeases.has(id), subscribeInvalidation: () => () => undefined,
    })
    const pending = Array.from({ length: 33 }, (_, index) => client.request({
      ...base(`cap-${index}`), type: 'acquire', capability: 'ui.host-dom.read', rootId: 'root.one', operations: ['read-text'],
    }))
    ...
    client.dispose(); authority.dispose()

    const getterOwner = binding()
    const getterAuthority = new HostDomAuthority({ hostGeneration: 'host-1', isolatedPluginBoundary: true, roots: roots(dom.window.document) })
    const getterClient = getterAuthority.bind(getterOwner.value)
    let getterReads = 0
    const getterRequest = {
      ...base('getter-snapshot'), type: 'acquire', capability: 'ui.host-dom.read', rootId: 'root.one',
      get operations() { getterReads += 1; return getterReads === 1 ? ['read-text'] : ['set-text'] },
    } as unknown as HostDomBridgeRequest
    await expect(getterClient.request(getterRequest)).resolves.toMatchObject({
      status: 'accepted', capability: 'ui.host-dom.read', operations: ['read-text'],
    })
    expect(getterReads).toBe(1)
    getterClient.dispose(); getterAuthority.dispose(); dom.window.close()
  })

  it('creates only Host-owned structured children and cleans them on disable/uninstall invalidation', async () => {
    const dom = new JSDOM('<body><main data-root="root.one"><div id="seat"></div></main><aside data-root="root.two"></aside></body>')
    const owner = binding()
    const authority = new HostDomAuthority({ hostGeneration: 'host-1', isolatedPluginBoundary: true, roots: roots(dom.window.document) })
    const client = authority.bind(owner.value)
    const read = await client.request({
      ...base('read'), type: 'acquire', capability: 'ui.host-dom.read', rootId: 'root.one', operations: ['inspect-structure'],
    })
    if (read.status !== 'accepted' || read.type !== 'acquire') throw new Error('read handle expected')
    const structure = await client.request({ ...base('inspect'), type: 'read', handle: read.handle, operation: 'inspect-structure' })
    const seat = nodeFrom(structure, 1)
    const modify = await client.request({
      ...base('modify'), type: 'acquire', capability: 'ui.host-dom.modify', rootId: 'root.one', operations: ['insert-owned-structured-child'],
    })
    if (modify.status !== 'accepted' || modify.type !== 'acquire') throw new Error('modify handle expected')
    const inserted = await client.request({
      ...base('insert'), type: 'modify', handle: modify.handle, node: seat, operation: 'insert-owned-structured-child',
      child: { id: 'owned.action', kind: 'action', label: { key: 'run', fallback: 'Run' }, command: { id: 'demo.run', arguments: { value: 1 } } },
    })
    ...
    await expect(client.request({ ...base('stale'), type: 'release', handle: modify.handle })).resolves.toMatchObject({ status: 'unavailable', code: 'stale-handle' })
    client.dispose(); authority.dispose(); dom.window.close()
  })
  ...
  it('actively rolls back owned children and command callbacks on Host generation replacement', async () => {
    ...
    const authority = new HostDomAuthority({
      ...
      subscribeHostGenerationInvalidation: listener => { generationListeners.add(listener); return () => generationListeners.delete(listener) },
      isolatedPluginBoundary: true,
      roots: roots(dom.window.document),
    })
    const client = authority.bind(owner.value)
    const read = await client.request({ ...base('generation-read'), type: 'acquire', capability: 'ui.host-dom.read', rootId: 'root.one', operations: ['inspect-structure'] })
    if (read.status !== 'accepted' || read.type !== 'acquire') throw new Error('read handle expected')
    const structure = await client.request({ ...base('generation-inspect'), type: 'read', handle: read.handle, operation: 'inspect-structure' })
    const seat = nodeFrom(structure, 1)
    const modify = await client.request({ ...base('generation-modify'), type: 'acquire', capability: 'ui.host-dom.modify', rootId: 'root.one', operations: ['insert-owned-structured-child'] })
    if (modify.status !== 'accepted' || modify.type !== 'acquire') throw new Error('modify handle expected')
    await client.request({
      ...base('generation-insert'), type: 'modify', handle: modify.handle, node: seat, operation: 'insert-owned-structured-child',
      child: { id: 'owned.generation-action', kind: 'action', label: { key: 'run', fallback: 'Run' }, command: { id: 'demo.run' } },
    })
    const action = dom.window.document.querySelector<HTMLButtonElement>('[data-cordisx-host-dom-child="owned.generation-action"]')!
    action.click()
