...
describe('AgentLoop v3 compatibility facade', () => {
  it('routes v3 create/send/approval through v4 authority and preserves v3 decision vocabulary', async () => {
    ...
    const v4 = new CordisXAgentLoopBrokerV4(new PlaygroundMockAgentLoopV4Transport(host), host, 'profile', 'generation')
      .bind({
        ownerKey: 'owner',
        active: () => true,
    ...
    expect(await client.decideApproval(command)).toMatchObject({
      status: 'accepted',
      schemaVersion: 3,
      decision: 'deny',
    })
