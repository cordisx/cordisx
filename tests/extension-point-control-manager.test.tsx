...
function snapshot(
  ...
  policyOutcome: 'native' | 'selected',
  currentOutcome: 'native' | 'none' | 'selected' = policyOutcome,
): ManagerSnapshot {
  const decision = currentOutcome === 'selected'
    ...
    : {
      ...
      authority: 'user' as const,
      hostGeneration: 'host',
      reason: currentOutcome === 'native' ? 'user.native' : 'policy.no-selection',
    }
  ...
  return {
    ...
    extensionPointControls: {
      revision: policyRevision + 10,
      policyRevision,
      hostGeneration: 'host',
      diagnostics: [],
      points: [{
        id: identity.pointId,
        state: pointPending ? 'pending' : 'active',
        reason: pointPending ? 'point.not-mounted' : 'point.mounted',
        selected: currentOutcome === 'selected' ? [selectedClaim] : [],
        eligibleCandidates: [],
        deniedCandidates: [],
        ...
        candidates: [{
          ...selectedClaim,
          contributionId: 'overlay',
          exclusiveGroup: 'renderer',
          priority: 0,
          authorization: 'allowed',
          policy: 'allow',
          state: pointPending ? 'pending' : currentOutcome === 'selected' ? 'selected' : 'eligible',
          reason: pointPending
            ? 'point.not-active'
...
describe('React Manager controlled group select', () => {
  it('refreshes selected policy choice across policy CAS and remount, then dispatches native fallback', async () => {
    ...
    const router: ManagerRouter = {
      route: { kind: 'extension-point', pointId: identity.pointId },
      navigate() {},
      back() {},
    }
    ...
    try {
      await act(async () =>
        root.render(<ExtensionPointDetailPage model={model} snapshot={snapshot(0, 'native')} router={router} />)
      )
      expect(input()?.value).toBe('原生渲染')
      await act(async () =>
        root.render(
          <ExtensionPointDetailPage model={model} snapshot={snapshot(1, 'selected', 'none')} router={router} />,
        )
      )
      ...
      const native = [...dom.window.document.querySelectorAll<HTMLElement>('.t-select-option')].find(option =>
        option.textContent?.includes('原生渲染')
      )
