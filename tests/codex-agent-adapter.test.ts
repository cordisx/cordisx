...
interface Fixture {
  readonly nativeAdditionalContext: Readonly<
    Record<string, { readonly value: string; readonly kind: 'application' | 'untrusted' }>
  >
...
const message: CordisXUserMessage = Object.freeze({
  id: 'message-1',
  role: 'user',
  content: Object.freeze([{ type: 'text', text: 'CordisX context' }]),
...
async function fixture(): Promise<Fixture> {
  return JSON.parse(
    await readFile(new URL('./fixtures/codex-app-server-v0.145.0.json', import.meta.url), 'utf8'),
  ) as Fixture
}
...
describe('private Codex Agent adapter', () => {
  it('preserves native context, creates a collision-free key, and cannot elevate plugin trust', async () => {
    ...
    const second = projectCodexAdditionalContext(
      { ...colliding, [`${cordisxKey}.1`]: { value: 'native collision', kind: 'application' } },
      message,
      'generation-1',
    )
  ...
  it('projects only inside the adapter and preserves native entries on forward', async () => {
    ...
    expect(outcome).toMatchObject({ terminal: 'forwarded' })
  ...
  it('normalizes thread/turn/item/message/chunk lifecycle with stable identities', async () => {
    ...
    expect(events.filter(event => event.type === 'content.chunk').map(event => (event.data as { index: number }).index))
      .toEqual([0, 1])
  ...
  it('marks missing parents and duplicates as inferred diagnostics instead of observed facts', () => {
    ...
    expect(events.filter(event => event.type === 'diagnostic')).toEqual(expect.arrayContaining([
      expect.objectContaining({
        provenance: 'inferred',
        data: expect.objectContaining({ code: 'missing-thread-start' }),
      }),
      expect.objectContaining({
        provenance: 'inferred',
        data: expect.objectContaining({ code: 'duplicate-turn-start' }),
      }),
    ]))
    expect(events.find(event => event.type === 'session.lifecycle')).toMatchObject({
      provenance: 'inferred',
      data: { phase: 'resumed' },
    })
  })
