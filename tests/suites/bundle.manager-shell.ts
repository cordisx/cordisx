...
export async function verifyManagerShell(context: Awaited<ReturnType<typeof verifyRoutes>>) {
  ...
  managerModal?.querySelector<HTMLButtonElement>('[data-tab="notifications"]')?.click()
  await settle()
  expect(managerModal?.querySelector('[data-notification-rules-page="true"]')?.textContent).toContain('没有屏蔽规则')
  expect(dom.window.document.querySelector('dialog.cxn-rules')).toBeNull()
  managerModal?.querySelector<HTMLButtonElement>('.cxr-header [aria-label="返回"]')?.click()
  await settle()
  expect(managerModal?.querySelector('[data-unified-plugins-page="true"]')).not.toBeNull()
