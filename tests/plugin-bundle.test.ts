...
async function bundleFixture(input: {
  ...
  const members = [
    ...(input.includeStorage
      ? [{ id: 'storage', version: '1.0.0', path: './plugins/storage', required: true, enabledByDefault: true }]
      : []),
    {
      id: 'notes',
      version: input.memberVersion ?? '1.0.0',
      path: './plugins/notes',
      required: true,
  ...
  await Promise.all([
    ...
    writeFile(
      path.join(bundle, 'cordisx-bundle.json'),
      `${
        JSON.stringify(
          {
...
async function harness() {
  ...
  const coordinator = new PluginBundleCoordinator({
    homeDir,
    profileId: 'work',
    runtimeGeneration: 'runtime-1',
    pluginLifecycle,
...
async function inspectAndInstall(coordinator: PluginBundleCoordinator, directory: string) {
  ...
  const bundlePermissions = planned.plan!.permissionRequests.map(permission => ({
...
describe('Host plugin bundle coordinator', () => {
  ...
  it('installs exact members dependency-first and binds required permissions to explicit bundle choices', async () => {
    ...
    expect(inspected.plan?.permissionRequests).toMatchObject([{
      pluginId: 'notes',
    ...
    const blocked = await request(coordinator, {
      ...
      impactToken: inspected.impactToken!,
      bundlePermissions: [],
      pluginOverrides: [],
    })
    ...
    const installed = await request(coordinator, {
      ...
      candidateId: inspected.candidateId!,
      impactToken: inspected.impactToken!,
      bundlePermissions: inspected.plan!.permissionRequests.map(item => ({
  ...
  it('tracks cross-bundle claims and retains a shared plugin until the final claim is removed', async () => {
    ...
    const directFirst = await pluginLifecycle.stagePackageSource({
      kind: 'local-directory',
      location: pathToFileURL(path.join(first, 'plugins', 'notes')).href,
    })
    const directSecond = await pluginLifecycle.stagePackageSource({
      kind: 'local-directory',
      location: pathToFileURL(path.join(second, 'plugins', 'notes')).href,
    })
    ...
    expect(directSecond.digest, 'identical packages in different directories must have the same immutable digest').toBe(
      directFirst.digest,
    )
    await inspectAndInstall(coordinator, first)
    const shared = await inspectAndInstall(coordinator, second)
    expect(shared.planned.plan?.memberActions).toMatchObject([{
      pluginId: 'notes',
      action: 'share',
      reason: 'other-bundle-claim',
    }])
    ...
    const removedBundle = await request(coordinator, {
      kind: 'uninstall',
      bundleId: 'bundle-one',
    ...
    const final = await request(coordinator, {
      kind: 'uninstall',
      bundleId: 'bundle-two',
      impactToken: finalPreview.impactToken!,
    })
  ...
  it('merges shared bundle policy deny over allow and lets an explicit plugin override win globally', async () => {
    ...
    const preview = await request(coordinator, {
      ...
      bundleId: 'bundle-one',
      bundlePermissions: [{ permissionId: permission.permissionId, policy: 'deny' }],
      pluginOverrides: [],
    ...
    expect((await coordinator.snapshot()).bundles.every(bundle => bundle.permissions[0]?.effectivePolicy === 'deny'))
      .toBe(true)
    ...
    const overridePreview = await request(coordinator, {
      ...
      bundleId: 'bundle-one',
      bundlePermissions: [{ permissionId: permission.permissionId, policy: 'deny' }],
      pluginOverrides: [{ pluginId: 'notes', permissionId: permission.permissionId, policy: 'allow' }],
      clearPluginOverrides: [],
      impactToken: '',
    })
    await request(coordinator, {
      ...
      pluginOverrides: [{ pluginId: 'notes', permissionId: permission.permissionId, policy: 'allow' }],
      clearPluginOverrides: [],
      impactToken: overridePreview.impactToken!,
    })
    expect(
      (await coordinator.snapshot()).bundles.every(bundle =>
        bundle.permissions[0]?.effectiveSource === 'plugin-override'
        && bundle.permissions[0]?.effectivePolicy === 'allow'
      ),
    ).toBe(true)
    ...
    const clearPreview = await request(coordinator, {
      ...
      bundlePermissions: [{ permissionId: permission.permissionId, policy: 'deny' }],
      pluginOverrides: [],
      clearPluginOverrides: [{ pluginId: 'notes', permissionId: permission.permissionId }],
      impactToken: '',
    })
    await request(coordinator, {
      kind: 'set-permissions',
      ...
      pluginOverrides: [],
      clearPluginOverrides: [{ pluginId: 'notes', permissionId: permission.permissionId }],
      impactToken: clearPreview.impactToken!,
    })
    expect(
      (await coordinator.snapshot()).bundles.every(bundle =>
        bundle.permissions[0]?.pluginOverride === undefined && bundle.permissions[0]?.effectivePolicy === 'deny'
      ),
    ).toBe(true)
    ...
    const acceptWidening = await request(coordinator, {
      kind: 'set-permissions',
      bundleId: 'bundle-two',
      bundlePermissions: [{ permissionId: permission.permissionId, policy: 'allow' }],
      pluginOverrides: [],
      clearPluginOverrides: [],
      impactToken: '',
    })
    await request(coordinator, {
      ...
      pluginOverrides: [],
      clearPluginOverrides: [],
      impactToken: acceptWidening.impactToken!,
    })
    expect((await coordinator.snapshot()).bundles.find(bundle => bundle.id === 'bundle-two')!.permissions[0])
      .toMatchObject({ effectivePolicy: 'allow', effectiveSource: 'bundle' })
  })
  ...
  it('separates active enable intent from durable ownership and blocks direct removal of managed members', async () => {
    ...
    expect((await coordinator.snapshot()).bundles.find(bundle => bundle.id === 'bundle-two')).toMatchObject({
      enabled: false,
      status: 'disabled',
    })
    ...
    expect((await coordinator.snapshot()).bundles.find(bundle => bundle.id === 'bundle-one')).toMatchObject({
      enabled: false,
      status: 'disabled',
    })
    ...
    expect((await coordinator.snapshot()).bundles.flatMap(bundle => bundle.records.map(record => record.kind)))
      .toContain('disable')
  })
  ...
  it('updates an exclusively owned member and removes members dropped by the new bundle version', async () => {
    ...
    expect((await coordinator.snapshot()).bundles[0]).toMatchObject({
      version: '2.0.0',
      members: [{ pluginId: 'notes', requestedVersion: '2.0.0' }],
    })
  })
})
