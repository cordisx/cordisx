...
describe('Playground task delegation composer layout', () => {
  it('uses one compact tokenized four-control toolbar without changing form semantics', async () => {
    ...
    expect(component).toContain('<button className="pg-event-composer-submit" type="submit"')

    expect(toolbarStyles).toContain('--pg-event-control-height: 40px')
    expect(toolbarStyles).toContain('.pg-event-composer[data-composer-event-type="task-delegation"] { grid-template-columns: minmax(170px,.82fr) minmax(180px,1fr) minmax(240px,2.6fr) 78px; }')
  ...
  it('wraps delegation controls into explicit rows at narrow widths without an overflow column', async () => {
    ...
    expect(styles).toContain('.pg-event-composer[data-composer-event-type="task-delegation"] { grid-template-columns: minmax(132px,.82fr) minmax(180px,1.18fr) 40px; padding-inline: 12px; }')
    expect(styles).toContain('.pg-event-composer[data-composer-event-type="task-delegation"] .pg-event-composer-delegation > textarea { grid-column: 1 / -1; grid-row: 2; }')
    expect(styles).toContain('@container pg-task-debugger (max-width: 520px)')
    expect(styles).toContain('.pg-event-composer[data-composer-event-type="task-delegation"] { grid-template-columns: minmax(0,1fr) 40px; }')
    expect(styles).toContain('.pg-event-composer[data-composer-event-type="task-delegation"] .pg-event-composer-delegation-target { grid-column: 1 / -1; grid-row: 2; }')
    expect(styles).toContain('.pg-event-composer[data-composer-event-type="task-delegation"] .pg-event-composer-delegation > textarea { grid-column: 1 / -1; grid-row: 3; }')
  })
})
