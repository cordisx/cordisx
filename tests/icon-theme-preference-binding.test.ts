...
describe('browser icon-theme preference bridge', () => {
  it('keeps a pending document booting until a bounded same-epoch redrive acknowledges the required winner', async () => {
    ...
    const bridge = new BrowserIconThemePreferenceBridge('token', 'codex', 'default', 'host-12', {
      revision: 1,
      ...candidate,
    })
    ...
    globalThis.__cordisxIconThemePreferenceReceiveV1?.(JSON.stringify({
      ...
      ok: true,
      ...readyLease(),
      documentEpoch: first.documentEpoch,
    ...
    globalThis.__cordisxIconThemePreferenceReceiveV1?.(JSON.stringify({
      ...
      ok: true,
      ...readyLease(),
      documentEpoch: second.documentEpoch,
  ...
  it('bounds permanent pending retries, permits a later explicit ready round, and cancels backoff on dispose', async () => {
    ...
    globalThis.__cordisxIconThemePreferenceReceiveV1?.(
      JSON.stringify({ kind: 'sync', value: { revision: 2, ...candidate } }),
    )
    globalThis.__cordisxIconThemePreferenceReceiveV1?.(JSON.stringify({
      kind: 'document-ready',
      ...
      ok: true,
      ...readyLease(),
      documentEpoch: fourth.documentEpoch,
      synchronization: 'complete',
      requiredRevision: 2,
      currentRevision: 2,
    }))
    ...
    globalThis.__cordisxIconThemePreferenceReceiveV1?.(JSON.stringify({
      ...
      ok: true,
      ...readyLease(),
      documentEpoch: fifth.documentEpoch,
      synchronization: 'pending',
      requiredRevision: 3,
      currentRevision: 2,
    }))
  ...
  it('fails a stale ready lease closed while keeping the exact request available for its replacement lease', async () => {
    ...
    globalThis.__cordisxIconThemePreferenceReceiveV1?.(
      JSON.stringify({ kind: 'sync', value: { revision: 3, ...candidate } }),
    )
    ...
    expect(() =>
      globalThis.__cordisxIconThemePreferenceReceiveV1?.(JSON.stringify({
        ...
        requestId: request.requestId,
        ok: true,
        readyLeaseToken: 'not-a-valid-token',
        readyLeaseRevision: invalidReplacement.readyLeaseRevision,
        documentEpoch: request.documentEpoch,
        synchronization: 'complete',
        requiredRevision: 3,
        currentRevision: 3,
  ...
  it('retains an accepted pending request and requires a strictly newer opaque replacement lease', async () => {
    ...
    expect(() =>
      globalThis.__cordisxIconThemePreferenceReceiveV1?.(JSON.stringify({
        ...
        requestId: request.requestId,
        ok: true,
        readyLeaseToken: 'ready_divergent_00000001',
        readyLeaseRevision: pendingLease.readyLeaseRevision,
        documentEpoch: request.documentEpoch,
        synchronization: 'pending',
        requiredRevision: 2,
        currentRevision: 1,
  ...
  it('adopts a conflict winner and fences stale sync and post-dispose messages', async () => {
    ...
    globalThis.__cordisxIconThemePreferenceReceiveV1?.(
      JSON.stringify({ kind: 'sync', value: { revision: 1, ...candidate } }),
    )
    expect(bridge.current()).toEqual(builtin)
    globalThis.__cordisxIconThemePreferenceReceiveV1?.(
      JSON.stringify({ kind: 'sync', value: { revision: 3, ...candidate } }),
    ...
    bridge.dispose()
    globalThis.__cordisxIconThemePreferenceReceiveV1?.(
      JSON.stringify({ kind: 'sync', value: { revision: 4, ...builtin } }),
    )
    expect(bridge.current()).toEqual({ revision: 3, ...candidate })
  })
