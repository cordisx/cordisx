...
describe('Agent Session permission-v4 Host authority', () => {
  ...
  it('prompts for an exact Host-reserved SessionId and persists an allow decision', async () => {
    ...
    const input = {
      identity,
      capability: 'agents.create' as const,
      sessionId: 'host-reserved-session',
      scopeSource: { kind: 'host-create' as const, reservedSessionId: 'host-reserved-session' },
      connection,
    }
  ...
  it('uses the Host development authority to persist an exact lease without a dialog', async () => {
    ...
    const decision = await broker.authorizeDevelopmentAgentRuntime(authority, {
      ...
      scopeSource: { kind: 'host-exact', exactSessionId: 'room-a-run-a' },
      connection,
    })
  ...
  it('admits explicit local-development Agent runtime authorization for every supported manifest predecessor through v8, and rejects unknown versions', async () => {
    ...
    for (const candidate of manifests) {
      ...
      const result = await broker.authorizeDevelopmentAgentRuntime(authority, {
        ...
        scopeSource: { kind: 'host-create', reservedSessionId: `cx-session.manifest-v${candidate.schemaVersion}` },
        connection,
      })
    ...
    await expect(broker.authorizeDevelopmentAgentRuntime(authority, {
      ...
      scopeSource: { kind: 'host-create', reservedSessionId: 'cx-session.unknown' },
      connection,
  ...
  it('admits one unbound Session capability only through its Host-issued exact scope', async () => {
    ...
    const broker = new PermissionBroker(new MemoryPermissionPolicyStore(), {
      request: async input => {
        requested.push(String(input.requested.agentSessionId))
        return 'allow-once'
      },
    })
    broker.register(identity, {
    ...
    broker.replaceAgentRuntimeConnection(connection)
    expect(
      (await broker.authorizeAgentRuntime({
        identity,
        capability: 'sessions.subscribe',
        sessionId: 'room-a-run-a',
        scopeSource: { kind: 'host-exact', exactSessionId: 'room-a-run-a' },
        connection,
      })).authorized,
    ...
    expect(
      await broker.authorizeAgentRuntime({
        ...
        capability: 'sessions.subscribe',
        sessionId: 'room-a-run-a',
        scopeSource: { kind: 'host-exact', exactSessionId: 'different-session' },
        connection,
      }),
  ...
  it('allows only seeded exact create and active same-plugin route leases', async () => {
    ...
    const created = await broker.authorizeAgentRuntime({
      ...
      capability: 'agents.create',
      sessionId: 'room-a-run-a',
      scopeSource: { kind: 'host-create', reservedSessionId: 'room-a-run-a' },
      connection,
    })
  ...
  it('fences a route lease exactly once on route replacement and rejects its stale handle', async () => {
    ...
    broker.replaceAgentRuntimeRouteScope({
      ...route,
      routeInstanceId: 'main:route-2',
      params: { sessionId: 'room-a-run-b' },
    })
  ...
  it('adds a Host-authorized scenario route without replacing the visible Room lease', async () => {
    ...
    const scenarioRoute = {
      ...route,
      routeInstanceId: 'playground-scenario:run-one',
      params: { sessionId: 'room-a-reviewer' },
    }
    ...
    const reviewer = await broker.authorizeAgentRuntime({
      identity,
      capability: 'sessions.subscribe',
      sessionId: 'room-a-reviewer',
      scopeSource: {
        kind: 'host-route',
        routeInstanceId: scenarioRoute.routeInstanceId,
        routeId: scenarioRoute.routeId,
        path: scenarioRoute.path,
        params: scenarioRoute.params,
      },
      connection,
    })
  ...
  it('mounts a captured Shell scenario route without consulting a transient foreground route', async () => {
    ...
    const scenarioRoute = {
      ...route,
      routeInstanceId: 'playground-scenario:captured-run',
      params: { sessionId: 'room-a-reviewer' },
    }
  ...
  it('closes active and queued prompts when the plugin generation is replaced', async () => {
    ...
    const broker = new PermissionBroker(
      ...
      new BrowserPermissionPrompt(dom.window.document),
    )
    ...
    const authorize = (sessionId: string) =>
      broker.authorizeAgentRuntime({
        identity,
        capability: 'sessions.get' as const,
        sessionId,
        scopeSource: { kind: 'host-exact' as const, exactSessionId: sessionId },
        connection,
      })
