...
interface RuntimeSnapshot {
  ...
  navigation: {
    routes: readonly { qualifiedId: string; productMetadata: { title?: string; description?: string } }[]
    pages: readonly {
      qualifiedId: string
      metadata: { chrome?: string }
      productMetadata: { title?: string; description?: string }
    }[]
...
describe('settings navigation demo bundle', () => {
  it('projects a first-level Host-owned navigation entry, mounts its controlled body, and cleans it up', async () => {
    ...
    const dom = new JSDOM(
      '<html lang="en" dir="ltr" class="electron-dark"><body><div class="sidebar-header"><button id="workspace-switcher" aria-haspopup="menu">Codex</button></div></body></html>',
      {
    ...
    const item = (): HTMLButtonElement | null =>
      dom.window.document.querySelector('[data-settings-navigation-item="settings-tab-demo:navigation"]')
    ...
    const page = dom.window.document.querySelector<HTMLElement>(
      '[data-cordisx-manager-page="settings-tab-demo:navigation"]',
    )!
    ...
    expect(runtime.snapshot().navigation.outlets.find(outlet => outlet.id === 'manager.content')).toMatchObject({
      mounted: true,
      activeRoute: 'settings-tab-demo:navigation',
    })
    expect(runtime.snapshot().extensionPoints.accessDiagnostics.slice(-3).map(entry => entry.request.operation))
      .toEqual([
    ...
    await waitFor(() =>
      item() === null && dom.window.document.querySelector('[data-settings-navigation-demo-content]') === null
    )
    ...
    expect(runtime.snapshot().navigation.routes[0]?.productMetadata.description).toBe(
      '打开“演示插件设置”并编辑示例值。',
    )
    ...
    dom.window.document.querySelector<HTMLButtonElement>('.cxr-header button[aria-label="关闭 CordisX 管理器"]')!
      .click()
    await waitFor(() => dom.window.document.querySelector('[data-settings-navigation-demo-content]') === null)
    dom.window.eval(bundle)
    ...
    expect(
      replacement.snapshot().extensionPoints.accessDiagnostics.every(entry =>
        entry.request.generation !== priorGeneration
      ),
    ).toBe(true)
