...
const connection = {
  ...
  adapterKind: 'feishu' as const, enabled: true,
...
describe('official Feishu direct-message adapter', () => {
  it('normalizes allowed inbound text, drains it, and sends an outbound reply without retaining raw payloads', async () => {
    ...
    let listener: ((message: {
      messageId: string; chatId: string; chatType: 'p2p'; senderId: string; content: string; createTime: number; mentionedBot: boolean;
    ...
    const definition = createFeishuAdapterDefinition({
      ...
      createChannel: () => ({
        ...
        on: (_name: 'message', handler: typeof listener) => { listener = handler; return () => { listener = undefined } },
    ...
    const host: ChannelAdapterHost = {
      generation: 2, ref: connection.ref,
      receive: async value => { received.push(value); return { recordId: 'inbound-1', duplicate: false, status: 'queued' } },
      drainInbound: async () => { inboundDrains += 1; return 1 },
      drainOutbound: async () => { outboundDrains += 1; return 0 },
    }
    ...
    const delivery: ChannelOutboundDelivery = {
      deliveryId: 'delivery-1', target: { ...connection.ref, conversationId: 'chat-1', kind: 'direct', threadId: 'chat-1', semantics: 'conversation' },
      kind: 'reply', text: 'pong', createdAt: '2026-08-25T00:00:00.000Z',
    }
    await expect(active.send(delivery)).resolves.toEqual({ externalMessageId: 'outbound-1', recallHandle: 'outbound-1' })
