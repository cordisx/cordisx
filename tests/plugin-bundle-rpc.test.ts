...
} from '../packages/cli/src/launcher/plugin-lifecycle-rpc.js'
...
describe('plugin bundle lifecycle private binding', () => {
    ...
    expect(() =>
      parsePluginLifecycleBindingRequest({
        ...
        privateRequest: {
          ...
          profileId: 'work',
          runtimeGeneration: 'runtime-a',
          request: { ...request, operation: { kind: 'execute-bundle-code' } },
        },
      }, handler)
    ).toThrow('unsupported')
  })
  ...
  it('accepts only same-profile same-generation bundle snapshot reads', () => {
    ...
    ).toEqual({ kind: 'bundle-snapshot-v1', requestId: 'snapshot-1' })
    expect(() =>
      parsePluginLifecycleBindingRequest({
        token: 'secret',
        privateRequest: {
  ...
  it('requires explicit override-clear intent on permission operations', () => {
    const request = {
      $schema: CORDISX_PLUGIN_BUNDLE_LIFECYCLE_OPERATION_SCHEMA_V1,
      schemaVersion: 1,
      requestId: 'bundle-permissions-1',
      profileId: 'work',
      expectedRevision: 3,
      expectedPluginRevision: 7,
      runtimeGeneration: 'runtime-a',
    ...
    ).toThrow('clearPluginOverrides is required')
  })
})
