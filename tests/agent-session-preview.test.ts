...
describe('Agent/Session development composition', () => {
  it(
    'embeds a shared React/UI local artifact and projects its Session task without sourcemap imports or a permission dialog',
    async () => {
...
`,
      ...
      expect(ordinaryLocalDevelopmentBuild.moduleFactorySource).toContain(
      ...
      try {
        ...
        ;(dom.window as unknown as { __cordisxOwnerDocumentRequestV1?: (payload: string) => void })
          .__cordisxOwnerDocumentRequestV1 = payload => {
            void session.handleOwnerDocumentRequest(payload).then(value => {
              ;(dom.window as unknown as { __cordisxOwnerDocumentReceiveV1?: (payload: string) => void })
                .__cordisxOwnerDocumentReceiveV1?.(JSON.stringify(value))
            })
          }
        ...
        expect(
          (dom.window as unknown as { __playgroundLocalSessionGetApplied?: boolean })
            .__playgroundLocalSessionGetApplied,
        ).toBe(true)
          ...
          __cordisxRuntime: {
            playgroundAgentSessions?(): {
              readonly tasks: readonly {
                ...
                readonly events: readonly { readonly sessionEvent?: { readonly type: string } }[]
              }[]
            } | undefined
