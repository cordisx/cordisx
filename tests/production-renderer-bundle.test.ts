...
describe('production renderer bundle', () => {
  it('keeps the no-plugin CDP payload map-free and below the audited startup budget', async () => {
    ...
    const bundle = await buildRendererBundle({ ...config, plugins: [] })

    expect(bundle).not.toMatch(/sourceMappingURL=data:/u)
    expect(Buffer.byteLength(bundle)).toBeLessThanOrEqual(NO_PLUGIN_PRODUCTION_BUNDLE_BUDGET_BYTES)
  }, 90_000)
})
