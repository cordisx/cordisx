...
describe('CLIProxy provider plugin renderer', () => {
  ...
  it('uses the existing main outlet and keeps provider identity in models and colliding session rows', async () => {
    ...
    for (
      let attempt = 0;
      attempt < 200 && configPanel?.querySelectorAll('[data-service-config]').length !== 2;
      attempt += 1
    ) {
      await new Promise(resolve => setTimeout(resolve, 10))
    }
    expect(configPanel?.querySelectorAll('[data-service-config]')).toHaveLength(2)
    expect(configPanel?.querySelector('[data-service-config="providers-runtime"]')?.getAttribute('data-config-applies'))
      .toBe('service-restart')
    expect(configPanel?.querySelector('[data-service-config="providers-startup"]')?.getAttribute('data-config-applies'))
      .toBe('app-restart')
    ...
    const serviceForm = configPanel!.querySelector<HTMLFormElement>(
      'form[data-service-config-form="providers-runtime"]',
    )!
    const serviceInput = serviceForm.querySelector<HTMLElement & { value: string; onChange?: (value: string) => void }>(
      't-textarea',
    )!
    ...
    expect(serviceForm.querySelector<HTMLElement & { disabled: boolean }>('t-button[type="submit"]')?.disabled).toBe(
      false,
    )
    serviceForm.dispatchEvent(new dom.window.SubmitEvent('submit', { bubbles: true, cancelable: true }))
    for (
      let attempt = 0;
      attempt < 100 && !serviceConfigRequests.some(request => request.operation === 'mutate');
      attempt += 1
    ) {
      await new Promise(resolve => setTimeout(resolve, 10))
    }
    ...
    const providerInput = providerField!.querySelector<HTMLElement & { onChange?: (value: readonly string[]) => void }>(
      't-tag-input',
    )!
    ...
    const validInput = validPanel.querySelector<HTMLElement & { onChange?: (value: readonly string[]) => void }>(
      '[data-config-path="providerIds"] t-tag-input',
    )!
    ...
    expect((dom.window as unknown as { __cordisxServiceConfigReceiveV1?: unknown }).__cordisxServiceConfigReceiveV1)
      .toBeUndefined()
  }, 20_000)
})
