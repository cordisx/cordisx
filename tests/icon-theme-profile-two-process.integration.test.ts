...
describe('two-process icon-theme profile persistence', () => {
  ...
  it(
    'restores the same approved artifact across a fresh Host process and rejects changed or absent artifacts',
    async () => {
      ...
      expect(preference.providerGeneration).toBe(
        (processA.selected as { providerGeneration: string }).providerGeneration,
      )
    },
    120_000,
  )
