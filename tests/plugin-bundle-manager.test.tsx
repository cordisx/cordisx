...
function bundles(): CordisXPluginBundleManagerSnapshotV1 {
  return {
    ...
    bundles: [{
      ...
      authors: ['CordisX'],
      sourceLabel: 'team-workflow.bundle',
      canonicalSource: 'https://plugins.example/team-workflow',
      ...
      members: [{
        ...
        bundleIds: ['team-workflow', 'other-workflow'],
        directClaim: false,
        runtimeDependentIds: ['consumer'],
      }],
      permissions: [{
        permissionId: `permission:${'c'.repeat(64)}`,
        pluginId: 'notes',
        ...
        affectedBundleIds: ['team-workflow', 'other-workflow'],
      }],
      claims: [
...
function snapshot(): ManagerSnapshot {
  return {
    ...
    localization: { locale: 'zh-CN', direction: 'ltr', version: 0 },
    localeCatalogs: [],
    ...
    permissions: [],
    pluginLifecycle: { profileId: 'work', revision: 7, runtimeGeneration: 'runtime-a', operationsAvailable: true },
...
describe('React Manager plugin bundle pages', () => {
  it('keeps header metadata above the exact five tabs and renders only README inside the README panel', async () => {
    ...
    try {
      ...
      expect([...detail.querySelectorAll<HTMLButtonElement>('.cxr-tabs button')].map(item => item.textContent)).toEqual(
        ['README', '成员', '权限', '关联', '记录'],
      )
      ...
      expect(dom.window.document.querySelector('.cxr-bundle-permission-editor')?.textContent).toContain(
        'shared-bundle-merge',
      )
