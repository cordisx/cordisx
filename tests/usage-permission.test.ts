...
it('an exact usage operation lease cannot revive after policy replacement or regrant', async () => {
  const host = broker()
  const dispose = host.register(identity, manifest, { pluginId: identity.id, moduleGeneration: 'work-generation' })
  expect(await host.authorizeUsage(identity)).toBe(true)
  const old = host.usageOperationFence(identity, 'work-generation')
  expect(old()).toBe(true)
  host.setUsagePolicy(identity, false)
  expect(old()).toBe(false)
  host.setUsagePolicy(identity, true)
  expect(await host.authorizeUsage(identity)).toBe(true)
  expect(old()).toBe(false)
  expect(host.usageOperationFence(identity, 'work-generation')()).toBe(true)
  dispose()
  host.dispose()
})
