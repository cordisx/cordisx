...
describe('extension point runtime contract', () => {
  it('keeps candidate source policy private until the shared generation flip', () => {
    const activation = (
      ...
      plugins: [{
        id: 'demo',
        version: '1.0.0',
        digest: `sha256:${sourceDigest.repeat(64)}`,
        moduleGeneration,
        enabled: true,
        dependencies: [],
      }],
    ...
    const broker = new ExtensionPointPolicyBroker(
      descriptors,
      new MemoryExtensionPointPolicyStore(),
      'runtime-1',
      visibility,
    )
    broker.register({ source: 'https://plugins.example/old', id: 'demo' }, {
      pluginId: 'demo',
  ...
  it('declares the complete v8 catalog with static maturity and adapter support', () => {
    ...
    expect(registry.descriptors()).toHaveLength(36)
    expect(registry.descriptors().filter(item => item.kind === 'surface')).toHaveLength(31)
    ...
    expect(registry.descriptor('session.content')).toMatchObject({
      ...
      title: {
        ...
        key: 'outlet.session.content.title',
        fallback: 'Session content page',
      },
      icon: 'host:history',
    })
    expect(registry.descriptor('session.header.actions')).toMatchObject({
      kind: 'surface',
      payloadFamily: 'contextual-action',
      maturity: 'stable',
    ...
    expect(registry.descriptor('composer.submit.effects')).toMatchObject({
      kind: 'surface',
    ...
    const removeDuplicate = registry.registerCatalog({
      ...
      points: [{
        id: 'app',
        kind: 'surface',
        title: { key: 'duplicate.title', fallback: 'Duplicate' },
        description: { key: 'duplicate.description', fallback: 'Duplicate point' },
        icon: 'host:info',
      }],
    })
    ...
    const removeInvalid = registry.registerCatalog({
      ...
      points: [{
        id: 'adapter.panel',
        kind: 'outlet',
        title: { key: 'adapter.title' },
        description: { key: 'adapter.description', fallback: 'Adapter panel' },
        icon: 'plugin:panel',
      }],
    })
  ...
  it('normalizes a v1 catalog into stable supported descriptors without weakening v5 validation', () => {
    const legacyCatalogs = [
      ...CORDISX_EXTENSION_POINT_LOCALE_CATALOGS,
      {
        ...
        locale: 'en',
        default: true,
        messages: { 'legacy.title': 'Legacy', 'legacy.description': 'Legacy v1 surface' },
      },
      {
        namespace: 'host',
        locale: 'zh-CN',
        messages: { 'legacy.title': '旧版点位', 'legacy.description': '旧版 v1 界面点位' },
      },
    ]
    ...
    const remove = registry.registerCatalog({
      ...
      points: [{
        id: 'legacy.surface',
        kind: 'surface',
        title: { key: 'legacy.title', fallback: 'Legacy' },
        description: { key: 'legacy.description', fallback: 'Legacy v1 surface' },
        icon: 'host:info',
      }],
    })
    expect(registry.descriptor('legacy.surface')).toMatchObject({
      payloadFamily: 'action',
      maturity: 'stable',
      adapterSupport: 'supported',
    })
    expect(registry.diagnostics()).toEqual([])
    ...
    expect(registry.diagnostics()).toEqual([
      expect.objectContaining({
        ...
        pointId: 'stable.unverified',
        message: expect.stringContaining('must be supported'),
      }),
    ])
    removeInvalidV5()
    registry.dispose()
    ...
    const registry = new ExtensionPointDescriptorRegistry([
      {
        ...
        locale: 'en',
        default: true,
        messages: { 'missing.title': 'Title', 'missing.description': 'Description' },
      },
    ])
    const remove = registry.registerCatalog({
      $schema: CORDISX_HOST_EXTENSION_POINT_CATALOG_SCHEMA_V1,
  ...
  it('registers distinct Manager content-tab and first-level navigation points in the v5 catalog', () => {
    ...
    expect(registry.descriptor('manager.settings.navigation-items')?.description.fallback).toContain(
      'settings extension seam',
    )
    expect(registry.descriptor('manager.content')?.description.fallback).toContain(
      'standard Host-owned Manager page header',
    )
  ...
  it('projects current context separately from maturity and adapter support', () => {
    ...
    const snapshot = buildExtensionPointRuntimeSnapshot({
      ...
      surfaceCurrentContext: [{
        ...
        detail: { key: 'session.not-mounted', fallback: 'No native session header is mounted.' },
      }, {
        surface: 'workspace.toolbar.items',
        state: 'inactive',
        code: 'anchor.unresolved',
        detail: { key: 'workspace.unresolved', fallback: 'Workspace anchor is ambiguous.' },
      }, {
        ...
        anchors: [
          { id: 'submit', placements: ['before'], state: 'active' },
          {
            ...
            detail: { key: 'leading.not-mounted', fallback: 'Leading is not mounted.' },
          },
          {
            id: 'model',
            ...
            state: 'not-mounted',
            code: 'anchor.not-mounted',
            detail: { key: 'model.not-mounted', fallback: 'Model is not mounted.' },
          },
        ],
      }],
    })
    expect(snapshot.points.find(item => item.id === 'session.header.actions')).toMatchObject({
      ...
      availabilityDetail: 'No native session header is mounted.',
    })
    expect(snapshot.points.find(item => item.id === 'composer.toolbar.items')?.anchors).toEqual([
      ...
      expect.objectContaining({
        ...
        availabilityCode: 'anchor.not-mounted',
      }),
      expect.objectContaining({
        id: 'model',
        adapterSupport: 'unverified',
  ...
  it('keys inherit/allow/deny by canonical source, plugin id, and point id', () => {
    ...
    expect(broker.decision('legacy', 'sidebar.navigation.items', 'outlet')).toMatchObject({
      authorized: false,
      effectivePolicy: 'deny',
    })
  ...
  it('emits exact host-generated surface and outlet access origins', () => {
    ...
    expect(broker.authorizeSurfaceCommand('demo', 'sidebar.navigation.items', 'demo:navigation', 'demo:open'))
      .toMatchObject({ authorized: false })
    expect(broker.authorizeSurfaceRoute('demo', 'session.header.actions', 'demo:trace', 'demo:trace.route'))
    ...
    expect(broker.accessDiagnostics().map(item => item.request)).toEqual([
      ...
      expect.objectContaining({
        operation: 'surface.route.navigate',
        contributionId: 'demo:trace',
        routeId: 'demo:trace.route',
      }),
    ...
    expect(broker.authorizeSurfaceRoute('demo', 'sidebar.workspace.menu', 'demo:workspace', 'demo:workspace.route'))
      .toMatchObject({
        authorized: false,
  ...
  it('fails closed on duplicate persisted policy tuples until the user replaces them', () => {
    ...
    const broker = new ExtensionPointPolicyBroker(
      descriptors,
      new MemoryExtensionPointPolicyStore([record('allow'), record('deny')]),
    )
