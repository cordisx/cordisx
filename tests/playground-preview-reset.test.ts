...
describe('Playground preview reset readback', () => {
  it('adopts a generation-zero external-home startup without clearing retained data or opening recovery', () => {
    const dom = new JSDOM('<nav class="pg-navigation-seat"><div data-navigation-group><button data-sidebar-item /></div></nav><section data-playground-recent-tasks><div data-recent-task-row /></section>', { url: 'http://127.0.0.1/' })
    const { sessionStorage } = dom.window
    const bootstrap = { version: 1 as const, instanceId: 'new-server', generation: 0 }
    sessionStorage.setItem(PLAYGROUND_SIMULATOR_TASK_SNAPSHOT_KEY, JSON.stringify({ version: 2, tasks: [{ taskRef: 'retained-task' }] }))
    sessionStorage.setItem(PLAYGROUND_PREVIEW_RESET_MARKER_KEY, JSON.stringify({
      version: 1, nonce: 'old-server:1', phase: 'awaiting-readback', startedAt: '2026-09-05T00:00:00.000Z',
    }))
    sessionStorage.setItem(PLAYGROUND_PREVIEW_RESET_RESULT_KEY, JSON.stringify({
      version: 1, status: 'failed', roomRows: 3, recentTaskRows: 6, simulatorRecords: 0,
      sources: { liveRuntime: 0, runtimeMemory: 6, taskSnapshotRegistry: 0, hostSessionRegistry: 0, legacyAliasRegistry: 0, finalSelector: 6 },
      instanceId: 'old-server', serverGeneration: 1, appliedGeneration: 1, completedAt: '2026-09-05T00:00:00.000Z',
    }))
  ...
  it('does not downgrade a nonzero explicit reset into bootstrap reconciliation', () => {
    ...
    const { sessionStorage } = dom.window
    sessionStorage.setItem(PLAYGROUND_PREVIEW_RESET_MARKER_KEY, JSON.stringify({
      version: 1, nonce: 'current-server:1', phase: 'awaiting-readback', startedAt: '2026-09-05T00:00:00.000Z',
    }))
    sessionStorage.setItem(PLAYGROUND_PREVIEW_RESET_RESULT_KEY, JSON.stringify({
      version: 1, status: 'failed', roomRows: 1, recentTaskRows: 1, simulatorRecords: 0,
      sources: { liveRuntime: 0, runtimeMemory: 1, taskSnapshotRegistry: 0, hostSessionRegistry: 0, legacyAliasRegistry: 0, finalSelector: 1 },
      instanceId: 'current-server', serverGeneration: 1, appliedGeneration: 1, completedAt: '2026-09-05T00:00:00.000Z',
    }))

    expect(reconcilePlaygroundPreviewBootstrapEpoch(sessionStorage, { version: 1, instanceId: 'current-server', generation: 1 })).toBe('unchanged')
    ...
    expect(playgroundPreviewResetDisposition({
      ...resetReadback(0), roomRows: 1, recentTaskRows: 1,
      sources: { liveRuntime: 0, runtimeMemory: 1, taskSnapshotRegistry: 0, hostSessionRegistry: 0, legacyAliasRegistry: 0, finalSelector: 1 },
