...
describe('UI Playground', () => {
  ...
  it('keeps review tasks in one Recent tasks section and excludes Playground fixtures', async () => {
    ...
    expect(styles).toContain('.pg-recent-task-list > [data-recent-task-row] .cxsi-icon { display: grid; width: 18px !important; height: 18px !important;')
    expect(styles).toContain('.cxsi-icon > .cxa-avatar { width: 18px; height: 18px; border: 0; border-radius: 50%; }')
    ...
    expect(app).toContain('iconElement={<HostAgentAvatar')
    expect(app).toContain('avatar: task.effective.avatar')
    expect(app).not.toContain('icon="host:history"')
    expect(app).toContain('createPortal(props.iconElement, iconSeat)')
