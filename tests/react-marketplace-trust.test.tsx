...
describe('React Marketplace trust badges', () => {
  it('renders all four independent states with stacked Host icons, tooltips, and accessible names', async () => {
    ...
    try {
      ...
      expect(
        [...dom.window.document.querySelectorAll<HTMLElement>('[data-trust-dimension]')].map(item =>
          item.dataset.trustDimension
        ),
      ).toEqual(['certified'])
      ...
      expect(badges.map(item => item.querySelector('svg')?.dataset.hostIconKey)).toEqual([
        'trust.official',
        'trust.certified',
      ])
    } finally {
