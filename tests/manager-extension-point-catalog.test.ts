...
describe('Manager extension point catalog', () => {
  it('renders the complete localized catalog without normal/type tags or orphan status rows', () => {
    ...
    try {
      ...
      pending.click()
      expect(
        dom.window.document.querySelector('[data-extension-point-detail-tab="diagnostics"]')?.getAttribute(
          'aria-selected',
        ),
      ).toBe('true')
      ...
      expect(
        [...dom.window.document.querySelectorAll<HTMLElement>('[data-extension-point-id]')].filter(item =>
          !item.closest<HTMLElement>('[data-collection-item]')?.hidden
        ),
      ).toHaveLength(1)
      ...
      const reprojectedSearch = dom.window.document.querySelector<HTMLInputElement>(
        '[data-collection-search="extension-points"]',
      )!
      reprojectedSearch.value = 'Composer toolbar'
      reprojectedSearch.dispatchEvent(new dom.window.Event('input', { bubbles: true }))
      expect(
        dom.window.document.querySelector('[data-extension-point-id="composer.toolbar.items"] .cxc-title')?.textContent,
      ).toBe('Composer toolbar')
    } finally {
      dispose()
      dom.window.close()
    }
  ...
  it('renders localized contribution names in a compact searchable sublist without placeholder cards', () => {
    ...
    try {
      dom.window.document.querySelector<HTMLButtonElement>('[data-tab="extension-points"]')!.click()
      dom.window.document.querySelector<HTMLButtonElement>('[data-extension-point-id="composer.toolbar.items"]')!
        .click()
      ...
      expect(contribution.querySelector('.cxm-resource-description')?.textContent).toBe(
        '在提交前刷新当前数据。 · 已渲染',
      )
      ...
      const replacement = dom.window.document.querySelector<HTMLInputElement>(
        '[data-list-search^="extension-point-usage-"] .cxm-search',
      )!
      ...
      expect(dom.window.document.querySelector('[role="tabpanel"][aria-label="使用情况"]')?.textContent).toContain(
        '没有匹配的插件或贡献',
      )
    } finally {
      dispose()
