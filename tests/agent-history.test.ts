...
function line(
  ...
  ordinal: number,
  timestamp = `2026-08-24T00:00:${String(ordinal).padStart(2, '0')}.000Z`,
) {
...
function fixtureLines(sessionId = SESSION_A): string[] {
  return [
    ...
    line('response_item', {
      ...
      content: [{ type: 'input_text', text: 'hello sk-secret-token /Users/example/private.txt' }],
    }, 2),
    line('response_item', {
      type: 'function_call',
      id: 'item-tool-1',
      call_id: 'call-1',
      name: 'shell',
      arguments: '{"password":"never-render"}',
    }, 3),
    line('event_msg', {
      type: 'item_completed',
      item: { type: 'FunctionCall', id: 'item-tool-1', call_id: 'call-1', status: 'completed' },
    }, 4),
    line('response_item', {
    ...
    line(
      'compacted',
      { window_id: 'window-1', replacement_history: [{ role: 'developer', content: 'never import' }] },
      6,
    ),
...
function rolloutPath(codexHome: string, sessionId: string, archived = false): string {
  const directory = archived
    ...
    : path.join(codexHome, 'sessions', '2026', '08', '24')
...
describe('Codex Agent history Host', () => {
  it('projects only provable session/turn/message/tool/time/compaction facts and redacts content', async () => {
    ...
    expect(referenced).toMatchObject({
      ok: true,
      value: {
        source: { kind: 'historical', adapterId: 'codex-history' },
        coverage: { state: 'partial', compacted: true, corruptLines: 1 },
      },
    })
    ...
    expect(referenced.value.events.map(event => event.type)).not.toEqual(
      expect.arrayContaining(['message.delivery', 'input.contribution']),
    )
  ...
  it('pages more than 500 events, tails complete appended lines, and keeps stable ids across restart', async () => {
    ...
    await appendFile(
      ...
      `${
        line(
          'response_item',
          { type: 'function_call', id: 'tail-tool', call_id: 'tail-call' },
          302,
    ...
    expect(
      afterRestart.value.events.find(event => event.itemId === stableEvent?.itemId && event.type === stableEvent?.type)
        ?.eventId,
    ).toBe(stableEvent?.eventId)
    firstHost.dispose()
    restarted.dispose()
  })

  it(
    'buffers a partial tail, reports corrupt and oversized lines, and refuses stale generation/profile cursors',
    async () => {
      const { codexHome, cacheDir } = await historyRoot()
      ...
      await appendFile(
        file,
        `${line('response_item', { type: 'message', role: 'user', id: 'partial' }, 8).slice(40)}\n{bad}\n${
      ...
      await expect(
        service.tail({ sessionId: SESSION_A, tailCursor: tail.value.tailCursor!, limit: 500 }, {
          ...caller,
  ...
  it('isolates sessions, resolves archived files, and ignores symlinked rollout paths', async () => {
    ...
    await expect(service.query({ sessionId: SESSION_B }, caller)).resolves.toMatchObject({
      ok: true,
      value: { coverage: { state: 'unavailable' }, events: [] },
    })
    service.dispose()
  })
  ...
  it('persists a metadata key with owner-only permissions and exposes no path-shaped RPC input', async () => {
    ...
    ).toThrow('invalid Agent history request fields')
...
class FakeHistoryAdapter implements CordisXAgentHistoryAdapter {
  readonly query = vi.fn(async (
    ...
    value: {
      ...
      snapshotId: 'snapshot.history.0001',
      limit: input.limit ?? 500,
      requestedPayloadPolicy: input.payloadPolicy ?? 'referenced',
      effectivePayloadPolicy: input.payloadPolicy ?? 'referenced',
      source: {
        kind: 'historical',
        adapterId: 'fixture',
        adapterVersion: '1',
        hostId: 'fixture',
        profileId: 'profile.opaque.0001',
      },
      coverage: {
        state: 'complete',
        compacted: false,
      ...
      tailCursor: 'tail.history.0001',
      events: [],
    },
  }))
  readonly tail = vi.fn(async (
    input: CordisXAgentHistoryTailQuery,
  ): Promise<CordisXPlatformResult<CordisXAgentHistoryPage>> => await this.query(input))
  status(): CordisXAgentHistoryStatus {
    return {
      hostId: 'fixture',
      hostName: 'Fixture history',
      mode: 'available',
      adapterId: 'fixture',
      adapterVersion: '1',
      profileId: 'profile.opaque.0001',
...
describe('Agent history renderer service', () => {
  it('enforces ask/allow/deny, session scope, and fiber disposal without exposing adapter authority', async () => {
    ...
    await expect(ctx.agentHistory.query({ sessionId: SESSION_A })).resolves.toMatchObject({
      ok: false,
      error: { code: 'permission-denied' },
    })
    broker.setPolicy(identity, 'agent.history.read', 'allow')
    await expect(ctx.agentHistory.query({ sessionId: SESSION_B })).resolves.toMatchObject({
      ok: false,
      error: { code: 'permission-scope-denied' },
    })
