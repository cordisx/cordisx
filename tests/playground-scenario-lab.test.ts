...
describe('Playground Scenario Lab Phase 1', () => {
  ...
  it('keeps the entry developer-only and outside product navigation and Recent tasks', async () => {
    ...
    expect(component).toContain('Run to execute it automatically or Next to execute one step.')
    expect(component).toContain('Pause is available only while running.')
    expect(component).toContain('Reset removes every disposable scenario task.')
    ...
    expect(component).toContain('<AgentConversationRenderer')
    expect(component).toContain('Developer only · not saved to Rooms or Recent tasks')
