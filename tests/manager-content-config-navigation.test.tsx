...
describe('Manager navigation v4 Host config body', () => {
  it('replaces the plugin page body and closes the exact binding on declaration disposal', async () => {
    const dom = new JSDOM('<!doctype html><html><body><main id="manager"></main></body></html>', {
      url: 'https://host.test/',
    })
    ...
    configuration.register({
      identity: { id: 'chatroom', source: 'file:///chatroom.ts' },
      moduleGeneration: 'chatroom-g1',
    ...
    outlets.declare({
      ...
      getSnapshot: () => ({
        available: true,
        container: dom.window.document.getElementById('manager')!,
        placement: 'absolute',
      }),
      subscribe: () => () => {},
      show: () => {},
    ...
    expect(() =>
      navigation.managerContent.register(context, {
        $schema:
    ...
    ).toThrow('unsupported schema tuple')
    ...
    const unregister = navigation.managerContent.register(context, {
      $schema: CORDISX_MANAGER_CONTENT_NAVIGATION_SCHEMA_V4,
      schemaVersion: 4,
      id: 'settings',
      route: { id: 'settings' },
      header: { title: { kind: 'route' } },
    ...
    expect(navigation.managerContentPresentation('chatroom', { id: 'settings' })?.config?.source.binding.namespace)
      .toBe('chatroom')
