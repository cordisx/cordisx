...
describe('local development checkpoint runner', () => {
  ...
  it('creates private evidence roots and finds exact state artifacts', async () => {
    ...
    try {
      ...
      expect(await helper.findNamed(root, 'direct-device-bound.v1.json')).toEqual([
        path.join(nested, 'direct-device-bound.v1.json'),
      ])
      expect(await helper.findNamed(root, 'codex-app-profile')).toEqual([
        path.join(root, 'projects', 'demo', 'cache', 'codex-app-profile'),
      ])
    } finally {
  ...
  it('compares complete renderer generations and snapshots pre-existing runtime paths by content', async () => {
    const generation = {
      ...
      lifecycleRevision: 2,
      activationLastGoodRevision: 2,
      configRevision: 0,
      configLastGoodRevision: 0,
    }
    expect(helper.rendererGenerationProjection(generation)).toEqual({
      digest: 'sha256:1',
      moduleGeneration: 'module-1',
      activationDigest: 'sha256:1',
      activationModuleGeneration: 'module-1',
      runtimeGeneration: 'runtime-1',
      lifecycleRevision: 2,
      activationLastGoodRevision: 2,
      configRevision: 0,
    ...
    try {
      ...
      expect(before.records).toContainEqual(
        expect.objectContaining({ path: path.join('state', 'existing.json'), kind: 'file' }),
      )
