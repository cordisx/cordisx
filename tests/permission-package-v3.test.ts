...
describe('permission manifest-v4 package boundary', () => {
  ...
  const runtimeValidators = {
    [CORDISX_PLUGIN_MANIFEST_SCHEMA_V4]: (value: unknown) =>
      normalizePluginManifestV4(value, 'permission-package', catalog),
  }
  ...
  )('stages and reads back the %s production permission smoke fixture', async (_label, source, version) => {
