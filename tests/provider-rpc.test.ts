...
describe('provider CDP RPC boundary', () => {
  it('requires the launcher token and accepts only normalized public Platform operations', () => {
    ...
    expect(parseProviderBindingRequest({
      requestId: 'request-3',
      token: 'fixed-token',
      operation: 'agent-loop.create',
      input: {
        model: { providerId: 'alpha', modelId: 'same-model' },
        cwd: '/workspace',
        developerInstructions: 'Agent definition',
        effort: 'high',
      },
    ...
    expect(parseProviderBindingRequest({
      requestId: 'request-4',
      token: 'fixed-token',
      operation: 'agent-loop.lifecycle.read',
    ...
    expect(parseProviderBindingRequest({
      requestId: 'request-v4-approval',
      token: 'fixed-token',
      operation: 'agent-loop.v4.approval.decide',
      input: {
        scope,
        command,
    ...
    expect(parseProviderBindingRequest({
      requestId: 'request-v4-intro',
      token: 'fixed-token',
      operation: 'agent-loop.v4.introduction.request',
      input: {
        scope,
        command: { type: 'request-member-self-introduction', commandId: 'operation-2' },
    ...
    expect(parseProviderBindingRequest({
      requestId: 'request-v4-cancel',
      token: 'fixed-token',
      operation: 'agent-loop.v4.introduction.cancel',
      input: {
        scope,
        command: { type: 'cancel-member-self-introduction', commandId: 'operation-3' },
        operationId: 'operation-3',
        task: 'task-1',
        binding,
        definition,
        requestOperationId: 'operation-2',
        participantId: 'agent-1',
        memberId: 'member-1',
        runId: 'run-1',
      },
    ...
    expect(parseProviderBindingRequest({
      requestId: 'request-v4-lifecycle',
      token: 'fixed-token',
      operation: 'agent-loop.v4.lifecycle.read',
    ...
    expect(parseProviderBindingRequest({
      requestId: 'request-v4-session-resolve',
      token: 'fixed-token',
      operation: 'agent-loop.v4.session.resolve',
    ...
    expect(() =>
      parseProviderBindingRequest({
        ...
        token: 'fixed-token',
        operation: 'tasks.read',
        input: { session: { remoteSessionId: 'same-id' } },
      }, 'fixed-token')
    ).toThrow('session.providerId')
    expect(() =>
      parseProviderBindingRequest({
        requestId: 'request-5',
        token: 'fixed-token',
        ...
        input: {
          model: { providerId: 'alpha', modelId: 'same-model' },
          cwd: '/workspace',
          imagePath: '/private/raw.png',
        },
      }, 'fixed-token')
    ).toThrow('unknown field imagePath')
    expect(() =>
      parseProviderBindingRequest({
        requestId: 'request-v4-bad-decision',
        token: 'fixed-token',
        operation: 'agent-loop.v4.approval.decide',
        input: {
          scope,
    ...
    expect(() =>
      parseProviderBindingRequest({
        requestId: 'request-v4-session-resolve-private',
        token: 'fixed-token',
        operation: 'agent-loop.v4.session.resolve',
        input: { scope, task: 'task-1', binding, definition, remoteSessionId: 'forged-session' },
      }, 'fixed-token')
    ).toThrow('unknown field remoteSessionId')
  })
})
