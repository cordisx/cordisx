...
describe('CordisX brand assets', () => {
  it('publishes transparent light and dark SVGs with the same spherical geometry', async () => {
    ...
    expect(createHash('sha256').update(light).digest('hex')).toBe(
      'bd5afe024112063e487b2ee3c40d92c3f1236ab7dc957623e444fb0635836865',
    )
    expect(createHash('sha256').update(dark).digest('hex')).toBe(
      'cbf804af4f89ef57d5e74e097c2eff7cef4c746ec5858ba295f570e14226b544',
    )
  ...
  it('uses the same theme-aware assets in both repository READMEs', async () => {
    for (const path of ['README.md', 'README.zh-CN.md']) {
      ...
      const dom = new JSDOM(readme)
      try {
        for (const theme of ['dark', 'light']) {
          const source = dom.window.document.querySelector(`source[media="(prefers-color-scheme: ${theme})"]`)
          expect(source?.getAttribute('srcset')).toBe(`./packages/cli/assets/brand/cordisx-mark-${theme}.svg`)
        }
        const image = dom.window.document.querySelector('img[alt="CordisX three-ring spherical mark"]')
        expect(image?.getAttribute('src')).toBe('./packages/cli/assets/brand/cordisx-mark-light.svg')
