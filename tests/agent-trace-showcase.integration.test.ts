...
async function fixture(sessionId: string, options: { headerAvailable?: boolean } = {}): Promise<{
  dom: JSDOM
  ...
  const config: CordisXConfig = {
    ...
    plugins: [{
      ...
      enabled: true,
      config: { mode: 'fixture', sessionId, permissionPolicy: 'allow' },
    }],
  }
...
describe('Agent Trace Showcase renderer integration', () => {
  it('mounts a session-scoped fixture page without owning native shell DOM', async () => {
    ...
    entryButton.click()
    await settle(4)
    expect(runtime.snapshot().extensionPoints.accessDiagnostics.slice(-3).map(item => ({
      operation: item.request.operation,
      generation: item.request.generation,
      authorized: item.authorized,
    }))).toEqual([
      { operation: 'surface.command.invoke', generation: expect.stringMatching(/^generation-/), authorized: true },
      { operation: 'outlet.route.navigate', generation: expect.stringMatching(/^generation-/), authorized: true },
      { operation: 'outlet.page.mount', generation: expect.stringMatching(/^generation-/), authorized: true },
    ])
    ...
    await settle(2)
    expect(page.querySelector('.cxt-integrity')?.textContent).toContain('loaded 19/27')
