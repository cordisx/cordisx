...
import {
  AgentEventLedgerError,
  CordisXAgentEventLedger,
  CordisXAgentEventService,
} from '../packages/cli/src/renderer/agent-events.js'
import {
...
function external(
  sessionId: string,
  seq: number,
  eventId = `cxevt:${encodeURIComponent(sessionId)}:${seq}`,
): CordisXAgentEvent<'session.lifecycle'> {
  return {
...
describe('Agent event ledger', () => {
  it('assigns stable per-session ids and publishes one committed range for a batch', () => {
    ...
    const events = ledger.commitBatch([
      ...
      },
      {
        ...
        provenance: 'observed',
        source,
        data: { channel: 'assistant', index: 0, delta: 'hello' },
      },
    ])
  ...
  it('keeps pagination snapshot-bounded and enforces the 500-event boundary', () => {
    ...
    ledger.commitBatch([
      session(),
      ...Array.from({ length: 4 }, (_, index): CordisXAgentEventDraft<'diagnostic'> => ({
        sessionId: 'session-1',
        type: 'diagnostic',
        provenance: 'inferred',
        source,
  ...
  it('rejects gaps, out-of-order input, duplicate ids, and chunk-index gaps', () => {
    ...
    expect(() => order.append(external('session-order', 0))).toThrowError(
      expect.objectContaining({ code: 'out-of-order' }),
    )

    const duplicate = new CordisXAgentEventLedger()
    duplicate.append(external('session-a', 0))
    expect(() => duplicate.append(external('session-b', 0, 'cxevt:session-a:0'))).toThrowError(
    ...
    const delivery = (stage: string, owner = pluginSource): CordisXAgentEventDraft<'message.delivery'> => ({
      ...
      data: {
        ...
        ...(stage === 'requested'
          ...
          : {}),
        ...(stage === 'cancelled'
          ? { cancelReason: 'requested' as const, diagnostic: { code: 'interrupted', message: 'cancelled' } }
          : {}),
      },
    })
    for (const stage of ['requested', 'permission', 'queued', 'claimed']) cancelAfterClaim.commit(delivery(stage))
    expect(() => cancelAfterClaim.commit(delivery('cancelled'))).toThrowError(
      expect.objectContaining({ code: 'invalid' }),
    )

    const ownerChange = new CordisXAgentEventLedger()
    ownerChange.commit(delivery('requested'))
    expect(() => ownerChange.commit(delivery('failed', { ...pluginSource, id: 'other' }))).toThrowError(
      expect.objectContaining({ code: 'invalid' }),
    )

    const terminal = new CordisXAgentEventLedger()
    for (const stage of ['requested', 'permission', 'queued', 'claimed', 'projected', 'forwarded']) {
      terminal.commit(delivery(stage))
    }
  ...
  it('brokers read/query and subscription by plugin identity and session scope', async () => {
    ...
    await expect(ctx.agentEvents.query({ sessionId: 'session-1' })).resolves.toMatchObject({
      ok: true,
      value: { events: [{ seq: 0 }] },
    })
    await expect(ctx.agentEvents.query({ sessionId: 'session-2' })).resolves.toMatchObject({
      ok: false,
    ...
    ledger.commit({
      ...
      provenance: 'inferred',
      source,
      data: { code: 'scoped', message: 'scoped' },
    })
