...
describe('renderer bundle', () => {
  it('boots the structured demo, routes all outlets, reprojects locale, and disposes one generation', async () => {
    ...
    expect(snapshot.extensionPoints.points).toHaveLength(38)
    expect(snapshot.extensionPoints.points.filter(item => item.kind === 'surface')).toHaveLength(31)
    ...
    expect(dom.window.document.querySelectorAll('[data-brand-rendering="direct-host"]')).toHaveLength(2)
