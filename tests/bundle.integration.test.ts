...
describe('renderer bundle', () => {
  it('boots the structured demo, routes all outlets, reprojects locale, and disposes one generation', async () => {
    ...
    expect(primaryNavigation.at(-1)?.querySelector('[data-cordisx-brand-mark]')).not.toBeNull()
    ...
    expect(pluginDetailTabs.map(tab => tab.tabIndex)).toEqual([0, 0, 0, 0, 0])
    ...
    expect(settingsTabs.map(tab => tab.tabIndex)).toEqual([0, 0, 0])
