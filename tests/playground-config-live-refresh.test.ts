...
describe('Playground effective configuration refresh', () => {
  ...
  it(
    'invalidates a same-server composition cache after commit and hydrates a new renderer from the committed revision',
    async () => {
      ...
      const persisted = JSON.parse(
        await readFile(path.join(value.homeDir, 'config', 'playground.config.json'), 'utf8'),
  ...
  it('publishes only advancing successful commits and fences stale generations and disposal', async () => {
    ...
    const session = await createPlaygroundSession(value.configPath, {
      homeDir: value.homeDir,
      onEffectiveConfigCommitted: commit => {
        commits.push(commit)
      },
    })
    ...
    await session.handleConfigRequest(binding(reset.source, reset.token, reset.generation, {
      operation: 'stage',
      requestId: 'stage-after-reset',
      expectedRevision: 0,
      config: { shortcutPolicy: 'mod-enter' },
    }))
    await session.handleConfigRequest(binding(reset.source, reset.token, reset.generation, {
      operation: 'commit',
      requestId: 'commit-after-reset',
      candidateRevision: 1,
    }))
