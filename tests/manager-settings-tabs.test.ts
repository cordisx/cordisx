...
function modelSnapshot(
  ...
  tabs: readonly ManagerSettingsTabSnapshot[] = [],
): ManagerSnapshot {
  return {
    version: '0.1.0',
    plugins: [{
      ...
      inject: [],
      config: {},
      configuration: { applies: 'plugin-restart', fields: [], revision: 0, writable: true },
      status: 'active',
    }],
    registrations: [],
    commands: [],
    navigation: {
      routes: [],
      pages: [],
      outlets: [{ id: 'manager.settings.content', available: false, mounted: false, presentation: 'inactive' }],
    },
...
function item(overrides: Partial<ManagerSettingsNavigationItemSnapshot> = {}): ManagerSettingsNavigationItemSnapshot {
  return {
    ...
    disabled: false,
    title: 'Demo settings',
    description: 'Host-rendered Manager destination.',
    pageTitle: 'Demo settings',
    pageDescription: 'Host header and controlled plugin body.',
    icon: 'host:settings',
    route: { id: 'navigation' },
    ...overrides,
  }
...
function managerDom(): JSDOM {
  const dom = new JSDOM(
    '<!doctype html><html><head></head><body><div class="sidebar-header"><button id="workspace-switcher" aria-haspopup="menu">Codex</button></div></body></html>',
...
describe('Manager settings navigation projection', () => {
  it('keeps A descriptor compatibility but exposes no global Settings page, row, or mount', () => {
    ...
    const legacy: ManagerSettingsTabSnapshot = {
      id: 'demo:legacy',
      owner: 'demo',
      title: 'Legacy',
      icon: 'host:settings',
    ...
    const manager: ManagerModel = {
      snapshot: () => modelSnapshot([], [legacy]),
      setPluginBlocked: async () => {},
      setPermissionPolicy: async () => {},
      subscribe: () => () => {},
      mountSettingsTab: async () => {
        mounts += 1
    ...
    try {
      ...
      expect(dom.window.document.querySelector('.cxm-heading .cxm-heading-icon')?.getAttribute('data-host-icon-key'))
        .toBe('plugins')
      expect(CORDISX_BUILTIN_MANAGER_SETTINGS_TABS).toEqual([])
      expect(
        dom.window.document.querySelector('[data-tab="settings"],[data-settings-tab],[data-settings-navigation-item]'),
      ).toBeNull()
  ...
  it('renders B through a Host row and controlled body, then aborts on Manager close', async () => {
    ...
    const manager: ManagerModel = {
      ...
      mountManagerContent: async (id, _reference, container): Promise<ManagedManagerPageMount> => {
        body = container.ownerDocument.createElement('section')
        body.dataset.demoBody = id
    ...
    try {
      dom.window.document.querySelector<HTMLButtonElement>('[data-cordisx-manager-trigger]')!.click()
      expect(dom.window.document.querySelector('[data-tab="plugins"] svg')?.getAttribute('data-host-icon-state')).toBe(
        'active',
      )
      expect(dom.window.document.querySelector('[data-tab="plugins"] svg')?.getAttribute('data-host-icon-variant'))
        .toBe('filled')
      expect(dom.window.document.querySelector('[data-tab="routes"] svg')?.getAttribute('data-host-icon-state')).toBe(
        'default',
      )
      expect(dom.window.document.querySelector('[data-tab="routes"] svg')?.getAttribute('data-host-icon-variant')).toBe(
        'regular',
      )
      const entry = dom.window.document.querySelector<HTMLButtonElement>(
        '[data-settings-navigation-item="demo:navigation"]',
      )!
      expect(entry.querySelector('[data-host-icon="host:settings"]')).not.toBeNull()
      expect(entry.querySelector('style,section')).toBeNull()
      ...
      entry.click()
      await settle()
      const activeEntry = dom.window.document.querySelector<HTMLButtonElement>(
        '[data-settings-navigation-item="demo:navigation"]',
      )!
      expect(dom.window.document.querySelector('[data-demo-body="demo:navigation"]')).not.toBeNull()
      expect(activeEntry.getAttribute('aria-current')).toBe('page')
      ...
      expect(dom.window.document.querySelector('.cxm-heading .cxm-heading-icon[data-host-icon="host:settings"]')).not
        .toBeNull()
  ...
  it('renders disabled B data without activation and falls back to Plugins after active removal', async () => {
    ...
    let state = modelSnapshot([
      item(),
      item({ id: 'demo:disabled', order: 170, disabled: true, disabledReason: 'Unavailable' }),
    ])
    ...
    const manager: ManagerModel = {
      ...
      mountManagerContent: async (id, _reference, container) => {
        const node = container.ownerDocument.createElement('div')
        node.dataset.activeBody = id
    ...
    try {
      dom.window.document.querySelector<HTMLButtonElement>('[data-cordisx-manager-trigger]')!.click()
      const disabled = dom.window.document.querySelector<HTMLButtonElement>(
        '[data-settings-navigation-item="demo:disabled"]',
      )!
      expect(disabled.disabled).toBe(true)
      expect(disabled.title).toBe('Unavailable')
      disabled.click()
      await settle()
      expect(dom.window.document.querySelector('[data-active-body]')).toBeNull()
      dom.window.document.querySelector<HTMLButtonElement>('[data-settings-navigation-item="demo:navigation"]')!.click()
      await settle()
      state = modelSnapshot([item({ id: 'demo:disabled', disabled: true })])
  ...
  it('renders manager-content navigation as Host icon tabs with complete ARIA and keyboard focus recovery', async () => {
    ...
    const manager: ManagerModel = {
      ...
      managerContentPresentation: (_id, reference) => ({
        title: 'Demo account',
        description: 'Host-owned detail navigation.',
        icon: 'host:layers',
      ...
      mountManagerContent: async (_id, reference, container) => {
        ...
        return {
          ...
          routeId: `demo:${reference.id}`,
          pageId: `demo:${reference.id}`,
          signal: controller.signal,
          abort: () => controller.abort(),
          dispose: async () => body.remove(),
        }
    ...
    try {
      ...
      const tabs = () => [
        ...dom.window.document.querySelectorAll<HTMLButtonElement>(
          '[data-manager-content-tabs] [data-manager-content-tab]',
        ),
      ]
      ...
      Object.defineProperty(tablist, 'scrollWidth', {
        configurable: true,
        get: () =>
          Math.max(...tabs().map(tab => tab.getBoundingClientRect().right - tablist.getBoundingClientRect().left)),
      })
