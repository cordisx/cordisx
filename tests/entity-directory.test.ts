...
describe('Host profile entity directory', () => {
  it('materializes exact package bytes only when absent and recovers CAS state across restart', async () => {
    const home = await mkdtemp(path.join(os.tmpdir(), 'cordisx-entities-'))
    roots.add(home)
    ...
    const binding = {
      profileId: 'profile-a',
      installationId: entityInstallationId('profile-a', 'chatroom'),
      pluginId: 'chatroom',
      pluginGeneration: 1,
    }
    ...
    expect(await authority.materialize(binding, '1.0.0', `sha256:${'1'.repeat(64)}`, [template]))
      .toMatchObject([{
        status: 'materialized',
        code: 'created',
        entity: { identity: { agentId: 'lead', revision: declaration.digest }, origin: 'materialized-template' },
      }])
    ...
    const updated = await authority.save(binding, {
      mutationId: 'edit-lead',
      expectedRevision: current.digest,
      entity: entity('Lead locally edited'),
      promptFiles: [{ path: './prompts/role.md', text: 'Coordinate local work.\n' }],
    })
    expect(updated).toMatchObject({
      status: 'applied',
      disposition: 'updated',
      entity: { origin: 'local', definition: { name: 'Lead locally edited' } },
    })
    expect(
      await authority.save(binding, {
        mutationId: 'edit-lead',
        expectedRevision: current.digest,
        entity: entity('Lead locally edited'),
        promptFiles: [{ path: './prompts/role.md', text: 'Coordinate local work.\n' }],
    ...
    expect(await readFile(path.join(home, 'profiles/profile-a/entities/lead/entity.json'), 'utf8')).toContain(
      'Lead locally edited',
    )
    ...
    expect(await restarted.snapshot({ ...binding, pluginGeneration: 2 })).toMatchObject({
      binding: { pluginGeneration: 2 },
      entities: [{
        owner: { profileId: 'profile-a', pluginId: 'chatroom' },
        definition: { name: 'Lead locally edited' },
        origin: 'local',
      }],
    })
    await writeFile(
      path.join(home, 'profiles/profile-a/entities/lead/entity.json'),
      `${JSON.stringify(entity('Lead externally edited'), null, 2)}\n`,
    )
    const watched = await restarted.snapshot({ ...binding, pluginGeneration: 2 })
    expect(watched).toMatchObject({
      registryRevision: 2,
      entities: [{ definition: { name: 'Lead externally edited' } }],
    })
    expect(await restarted.changes({ ...binding, pluginGeneration: 2 }, 1, 1))
      .toMatchObject({
        revision: 2,
        changes: [{ sequence: 2, kind: 'entity-updated', entity: { definition: { name: 'Lead externally edited' } } }],
      })
  })

  it('fails closed for undeclared writes and symlink-backed entity trees', async () => {
    const home = await mkdtemp(path.join(os.tmpdir(), 'cordisx-entities-fence-'))
    roots.add(home)
    const authority = new EntityDirectoryAuthority(home, 'profile-a')
    const binding = {
      profileId: 'profile-a',
    ...
    expect(
      await authority.save(binding, {
        mutationId: 'no-declaration',
        expectedRevision: null,
        entity: entity('Lead'),
        promptFiles: [{ path: './prompts/role.md', text: 'Role' }],
      }),
    )
    ...
    await expect(
      import('node:fs/promises').then(async fs =>
        await fs.symlink(path.join(home, 'outside.json'), path.join(home, 'profiles/profile-a/entities/lead'))
      ),
    ...
    const declaration = {
      ...
      digest: entityTreeDigest(source, prompts),
    }
    authority.register(binding, [declaration])
    expect(
      await authority.materialize(binding, '1.0.0', `sha256:${'3'.repeat(64)}`, [{
        declaration,
        entityText: source,
        promptFiles: prompts,
      }]),
  ...
  it('claims but never overwrites an existing declared local entity', async () => {
    const home = await mkdtemp(path.join(os.tmpdir(), 'cordisx-entities-existing-'))
    roots.add(home)
    ...
    const declaration = {
      agentId: 'lead',
      entityPath: './entities/lead/entity.json' as const,
      digest: entityTreeDigest(templateText, prompts),
    }
    const binding = {
      profileId: 'profile-a',
      installationId: entityInstallationId('profile-a', 'chatroom'),
    ...
    authority.register(binding, [declaration])
    expect(
      await authority.materialize(binding, '1.0.0', `sha256:${'5'.repeat(64)}`, [{
        declaration,
        entityText: templateText,
        promptFiles: prompts,
      }]),
    )
      .toEqual([expect.objectContaining({ status: 'preserved', code: 'entity-present' })])
    expect(await readFile(path.join(home, 'profiles/profile-a/entities/lead/entity.json'), 'utf8')).toBe(localText)
    expect(await authority.snapshot(binding)).toMatchObject({
      entities: [{ definition: { name: 'User local Lead' }, origin: 'local' }],
    })
  })
})
