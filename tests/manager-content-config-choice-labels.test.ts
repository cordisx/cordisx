...
import { CORDISX_MANAGER_CONTENT_NAVIGATION_SCHEMA_V5 } from '../packages/cli/src/contracts.js'
...
describe('Manager content localized config choices', () => {
  ...
  it('negotiates descriptor v3/source v2 and resolves zh, en, and fallback labels without changing values', async () => {
    ...
    locale.value = 'fr'
    expect(handle.snapshotForHost().fields.find(field => field.path[0] === 'shortcutPolicy')?.choices?.[0]?.label).toBe(
      'Enter sends',
    )
    authority.dispose()
  })
  ...
  it('keeps exact choice values through save, reload, CAS, and subscription page v2', async () => {
    ...
    expect(await source.snapshot()).toMatchObject({
      status: 'available',
      body: { configuration: { version: 3, revision: 1, value: { shortcutPolicy: 'mod-enter' } } },
    })
    ...
    const conflict = await source.execute(command(source, 'stale-choice', 0, {
      operation: 'draft.save',
      mutationId: 'stale-choice-mutation',
  ...
  it.each([
    ...
    ['duplicate choice', {
      ...
      fields: [{
        ...
        choices: [presentation.fields[0].choices[0], presentation.fields[0].choices[0]],
      }],
    }],
    ['missing enum choice', {
      ...presentation,
      fields: [{ ...presentation.fields[0], choices: [presentation.fields[0].choices[0]] }],
    }],
    ['additional enum choice', {
      ...presentation,
      fields: [{
        ...presentation.fields[0],
        choices: [...presentation.fields[0].choices, { value: 'future', label: { key: 'future', fallback: 'Future' } }],
      }],
    }],
    ['malformed localized label', {
      ...presentation,
      fields: [{
        ...presentation.fields[0],
        choices: [{ value: 'enter', label: { key: 'bad key', fallback: 'Enter' } }, presentation.fields[0].choices[1]],
      }],
    }],
    ['unknown presenter option', {
      ...presentation,
      fields: [{ ...presentation.fields[0], presenter: { ...presentation.fields[0].presenter, future: true } }],
    }],
    ['non-scalar enum target', {
      ...presentation,
      fields: [{ path: ['title'], choices: presentation.fields[0].choices }],
    }],
