...
describe('Playground deterministic Agent/Session driver', () => {
  it('creates an exact caller Session, appends fixture assistant facts, and replays them through the same Session', async () => {
    ...
    expect(created.handle.agent.status).toEqual({ status: 'unavailable', code: 'whole-agent-idle-unobservable' })
    ...
    expect(snapshot).toMatchObject({ status: 'available', snapshot: { snapshotSeq: 11 } })
    ...
    expect(replay.page.events.map(event => event.type)).toEqual([
      'agent/inbox/spliced', 'user/message', 'turn/start', 'step/start', 'tool/call', 'tool/result', 'assistant/chunk',
    ...
    expect(live).toEqual(['agent/status:running', 'agent/inbox/inserted:', 'agent/status:idle'])
  ...
  it('uses the Host approval seam, supports pending-message discard and emits one cancelled terminal', async () => {
    ...
    expect(await created.handle.agent.discard('m-pending')).toMatchObject({ status: 'accepted', messageId: 'm-pending' })
    ...
    expect(read.page.events.at(-1)).toMatchObject({ type: 'turn/end', data: { reason: { kind: 'interrupted' } } })
