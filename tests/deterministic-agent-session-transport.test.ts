...
describe('Playground deterministic Agent/Session driver', () => {
  it('creates an exact caller Session, appends fixture assistant facts, and replays them through the same Session', async () => {
    ...
    expect(created).toMatchObject({
      status: 'accepted',
      sessionId: 'room-preview-session-1',
    ...
    const live: string[] = []
    const agentLive = await created.handle.agent.subscribe(event => {
      live.push(`${event.type}:${'status' in event.data ? event.data.status : ''}`)
    })
    expect(agentLive.status).toBe('subscribed')
    expect(created.handle.agent.status).toEqual({ status: 'available', value: 'idle' })
    expect(await created.handle.agent.followup(message('m-1', 'hello fixture [tool]'))).toMatchObject({
      status: 'accepted',
      messageId: 'm-1',
    })
    await settle()
    ...
    expect(replay.page.events.find(event => event.type === 'tool/result')).toMatchObject({
      data: { meta: { fixture: 'deterministic-agent-session', deterministic: true } },
    })
    expect(replay.page.events.find(event => event.type === 'assistant/message')).toMatchObject({
      data: { message: { source: { provider: 'deterministic-agent-session' } } },
    })
  ...
  it('uses the Host approval seam, supports pending-message discard and emits one cancelled terminal', async () => {
    ...
    expect(await created.handle.agent.followup(message('m-approval', '[approval]'))).toMatchObject({
      status: 'accepted',
    })
    await settle()
    expect(await created.handle.agent.followup(message('m-pending', '[pending]'))).toMatchObject({ status: 'accepted' })
    expect(await created.handle.agent.discard('m-pending')).toMatchObject({
      status: 'conflict',
    ...
    expect(read.page.events.find(event => event.type === 'approval/decided')).toMatchObject({
      data: { outcome: 'allowed-once' },
    })
    expect(read.page.events.find(event => event.type === 'agent/inbox/spliced' && event.data.outcome === 'canceled'))
      .toBeDefined()
    expect(read.page.events.find(event => event.type === 'turn/end' && event.data.reason.kind === 'interrupted'))
      .toMatchObject({
        type: 'turn/end',
