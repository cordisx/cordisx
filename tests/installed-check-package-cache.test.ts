...
it('repacks one verified external dependency closure without rerunning Git prepare hooks', async () => {
  ...
  try {
    ...
    await Promise.all(externalPackages.map(async packageName => {
      const packageRoot = path.join(runner, 'node_modules', ...packageName.split('/'))
    ...
    const original = JSON.parse(
      await readFile(path.join(runner, 'node_modules', '@cordisx', 'plugin-cli-proxy-api', 'package.json'), 'utf8'),
    )
