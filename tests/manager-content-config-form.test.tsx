...
describe('Host Manager config form mount', () => {
  it('materializes defaults once, subscribes to the Host source, and unsubscribes on route unmount', async () => {
    ...
    const binding = {
      ...
      declarationId: 'settings', namespace: 'chatroom',
    } as const
    const body: ManagerContentPluginConfigFormProjectionV1 = {
      ...
      configuration: {
        ...
        revision: 3, lastGoodRevision: 3, applies: 'live', writable: true, secrets: [],
      },
      draft: { baseRevision: 3, dirty: false, value: { shortcutPolicy: 'enter' }, validation: { state: 'unvalidated' } },
    }
    const closed: ManagerContentConfigSubscriptionClosedV1 = {
      $schema: 'https://raw.githubusercontent.com/cordisx/cordisx-protocol/main/schemas/manager-content-config-subscription-close.v1.schema.json',
      contract: 'cordisx.manager-content-config-subscription-close/v1', schemaVersion: 1,
      subscriptionId: 'cx-manager-config-sub:test', binding, status: 'closed', code: 'unsubscribed',
    }
    const unsubscribe = vi.fn(async () => closed)
    const execute = vi.fn(async () => ({
      $schema: 'https://raw.githubusercontent.com/cordisx/cordisx-protocol/main/schemas/manager-content-config-result.v1.schema.json' as const,
      contract: 'cordisx.manager-content-config-result/v1' as const, schemaVersion: 1 as const,
      commandId: 'defaults', binding, expectedRevision: 3, operation: 'defaults.materialize' as const,
      status: 'preserved' as const, code: 'values-present' as const, revision: 3,
    }))
    const source = {
      binding,
      ...
      subscribe: async () => ({
        ...
        subscription: {
          ...
          closed: new Promise<never>(() => {}), unsubscribe,
        },
      }),
    } as unknown as ManagerContentConfigSourceV1
    const handle = {
      owner: 'chatroom', declarationId: 'settings', moduleGeneration: 'chatroom-g1',
      body: {
        kind: 'plugin-config-form' as const, namespace: 'chatroom',
        defaultMaterialization: { mode: 'missing-only' as const, fields: [{ path: ['shortcutPolicy'] as const, value: 'enter' }] },
      },
      source,
      snapshotForHost: () => ({
        namespace: 'chatroom', schemaKind: 'schemastery' as const, applies: 'live' as const,
        writable: true, revision: 3, lastGoodRevision: 3, value: { shortcutPolicy: 'enter' },
        fields: [], secrets: [],
      }),
      close: () => {},
    ...
    expect(execute.mock.calls[0]?.[0]).toMatchObject({
      operation: 'defaults.materialize', expectedRevision: 3, binding,
  ...
  it('unsubscribes a source that resolves after the route was already unmounted', async () => {
    ...
    const body: ManagerContentPluginConfigFormProjectionV1 = {
      ...
      configuration: {
        version: 2, identity: binding.identity, scope: binding.scope, namespace: 'chatroom',
        schema: { kind: 'standard', renderable: false }, value: {},
    ...
    const closed: ManagerContentConfigSubscriptionClosedV1 = {
      $schema: 'https://raw.githubusercontent.com/cordisx/cordisx-protocol/main/schemas/manager-content-config-subscription-close.v1.schema.json',
      contract: 'cordisx.manager-content-config-subscription-close/v1', schemaVersion: 1,
      subscriptionId: 'cx-manager-config-sub:delayed', binding, status: 'closed', code: 'unsubscribed',
    }
    const unsubscribe = vi.fn(async () => closed)
    ...
    const source = {
      ...
      snapshot: async () => ({ status: 'available' as const, body }),
      execute: vi.fn(),
      subscribe: () => new Promise<Awaited<ReturnType<ManagerContentConfigSourceV1['subscribe']>>>(resolve => { resolveSubscribe = resolve }),
    } as unknown as ManagerContentConfigSourceV1
    const handle = {
      owner: 'chatroom', declarationId: 'settings', moduleGeneration: 'chatroom-g1',
      body: { kind: 'plugin-config-form' as const, namespace: 'chatroom' }, source,
      snapshotForHost: () => ({
        namespace: 'chatroom', schemaKind: 'schemastery' as const, applies: 'live' as const,
        writable: true, revision: 0, lastGoodRevision: 0, value: {}, fields: [], secrets: [],
      }),
      close: () => {},
    } satisfies ManagerContentConfigBindingHandle
    ...
    resolveSubscribe({
      ...
      subscription: {
        ...
        pages: { [Symbol.asyncIterator]: () => ({ next: () => new Promise<never>(() => {}) }) },
        closed: Promise.resolve(closed), unsubscribe,
      } as never,
    })
    await new Promise(resolve => setImmediate(resolve))
    expect(unsubscribe).toHaveBeenCalledOnce()
