import type { ManagerCollectionQueryV1, ManagerCollectionSnapshotV1 } from '@cordisx/protocol/manager-collection/v1'
...
describe('Host Manager collection', () => {
  it('uses the frozen Unicode 17 NFKC casefold and whitespace transform', () => {
    ...
    expect(() => normalizeManagerCollectionSearch('x'.repeat(257), { maximumInputCodePoints: 256 })).toThrow(
      /code-point bound/,
    )
  })
  ...
  it('owns page-scoped search, text input action forwarding, revision fencing, and cleanup', async () => {
    ...
    const snapshot = (query: ManagerCollectionQueryV1): ManagerCollectionSnapshotV1 => ({
      ...
      items: [{
        id: 'room-1',
        title: text('room.title', 'Straße planning'),
        ...
        actions: [{
          ...
          input: {
            argument: 'title',
            title: text('rename.title', 'Rename Room'),
            label: text('rename.label', 'Title'),
            submitLabel: text('rename.submit', 'Rename'),
            minLength: 1,
            maxLength: 80,
            trim: 'both',
          },
    ...
    const registry = new HostManagerCollectionPageRegistry({
      ...
      clearTextSite() {},
      navigate: async () => {},
      deepLink: () => 'https://example.test/manager/extensions/chatroom/rooms/room-1',
  ...
  it('focuses and dismisses the Host dialog, then removes the collection on unload', async () => {
    ...
    const mounted = mountManagerCollectionHost(container, {
      ...
      executeCommand: async () => {
        throw new Error('dialog was not confirmed')
      },
      writeClipboard: async () => {},
      hostCopy: key => key,
    })
        ...
        snapshot: query => ({
          ...
          items: [{
            ...
            actions: [{
              ...
              feedback: {
                success: text('delete.success', 'Deleted'),
                failure: text('delete.failure', 'Delete failed'),
              },
            }],
          }],
        }),
