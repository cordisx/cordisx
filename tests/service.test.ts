...
const raster = (data = PNG): RasterImageSnapshotV1 => ({
  ...
  mediaType: 'image/png', encoding: 'base64', data, width: 1, height: 1,
})
...
describe('CordisXSlotService', () => {
  ...
  it('atomically projects generic collection images and fences invalid, stale, or late updates', async () => {
    ...
    let state: CordisXNavigationCollectionSnapshotV3 = {
      revision: 1,
      items: [
        { id: 'older', label: { key: 'older', fallback: 'Older' }, route: { id: 'item', params: { id: 'older' } }, order: 20, leadingVisual: visual() },
        { id: 'latest', label: { key: 'latest', fallback: 'Latest' }, route: { id: 'item', params: { id: 'latest' } }, order: 10, leadingVisual: visual() },
      ],
    }
    ...
    const plugin = ctx.extend({ [CORDISX_PLUGIN_ID]: 'demo' }).plugin({
      ...
      apply(pluginCtx: Context) {
        pluginCtx.slots.registerCollection({
          name: 'sidebar.navigation.items', id: 'items', contract: 'cordisx.navigation-collection/v3',
    ...
    await plugin
    expect(service.snapshot().map(item => (item.item as { label: { fallback: string } }).label.fallback)).toEqual(['Latest', 'Older'])
    ...
    state = { revision: 2, items: [
      { id: 'new', label: { key: 'new', fallback: 'New' }, route: { id: 'item', params: { id: 'new' } }, order: 0, leadingVisual: visual() },
      { id: 'latest', label: { key: 'latest', fallback: 'Latest' }, route: { id: 'item', params: { id: 'latest' } }, order: 10, leadingVisual: visual() },
    ] }
    ...
    state = { revision: 3, items: [{
      id: 'unsafe', label: { key: 'unsafe', fallback: 'Unsafe' }, route: { id: 'item' }, order: 0,
    ...
    expect(service.snapshot().map(item => (item.item as { label: { fallback: string } }).label.fallback)).toEqual(['New', 'Latest'])
  ...
  it('isolates identical collection item ids by exact owner-qualified identity', async () => {
    ...
    const service = ctx.slots as CordisXSlotService
    service.setResolvers({ command: () => true, route: () => true })
    const mount = (owner: string) => ctx.extend({ [CORDISX_PLUGIN_ID]: owner }).plugin({
      inject: ['slots'],
      apply(pluginCtx: Context) {
        pluginCtx.slots.registerCollection({
          name: 'sidebar.navigation.items', id: 'items', contract: 'cordisx.navigation-collection/v3',
          ...
          snapshot: () => ({ revision: 1, items: [{ id: 'same', label: { key: 'same', fallback: 'Same' }, order: 0, route: { id: 'item' }, leadingVisual: visual() }] }),
          subscribe: () => () => undefined,
        })
      },
    })
