...
import { stagePluginPackageSourceV1, type HostResolvedRuntimeManifest } from '../packages/cli/src/launcher/packages/index.js'
...
describe('plugin package v5 entity templates', () => {
  it('stages exact template bytes in the immutable package and rejects undeclared schema compatibility', async () => {
    const root = await mkdtemp(path.join(os.tmpdir(), 'cordisx-entity-package-')); roots.add(root)
    const source = path.join(root, 'source'); const homeDir = path.join(root, 'home')
    ...
    const runtime = { $schema: CORDISX_PLUGIN_MANIFEST_SCHEMA_V1, schemaVersion: 1, id: 'entity-fixture', capabilities: [] } as const
    ...
    const declaration = { agentId: 'lead', entityPath: './entities/lead/entity.json' as const, digest: entityTreeDigest(entityText, [{ path: './prompts/role.md', text: promptText }]) }
    const manifest = (includeEntitySchema: boolean) => ({
      $schema: 'https://raw.githubusercontent.com/cordisx/cordisx-protocol/main/schemas/plugin-package.v5.schema.json', schemaVersion: 5,
      id: 'entity-fixture', version: '1.0.0', entry: './src/index.ts',
      distribution: { mode: 'explicit-local-v1', signature: 'unsupported' },
      compatibility: { runtimeAbi: 1, protocolSchemas: [CORDISX_PLUGIN_MANIFEST_SCHEMA_V1, ...(includeEntitySchema ? [entity.$schema] : [])] },
      dependencies: [], runtimeManifest: { path: './runtime.json', schema: CORDISX_PLUGIN_MANIFEST_SCHEMA_V1, digest: `sha256:${createHash('sha256').update(runtimeText).digest('hex')}` },
      entityTemplates: [declaration],
    })
    await writeFile(path.join(source, 'cordisx-package.json'), `${JSON.stringify(manifest(false), null, 2)}\n`)
    const runtimeValidators = { [CORDISX_PLUGIN_MANIFEST_SCHEMA_V1]: (value: unknown) => value as HostResolvedRuntimeManifest }
    await expect(stagePluginPackageSourceV1({ kind: 'local-directory', location: new URL(`file://${source}/`).href }, { homeDir, runtimeValidators }))
    ...
    const staged = await stagePluginPackageSourceV1({ kind: 'local-directory', location: new URL(`file://${source}/`).href }, { homeDir, runtimeValidators })
    expect(staged.entityTemplates).toEqual([{ declaration, entityText, promptFiles: [{ path: './prompts/role.md', text: promptText }] }])
