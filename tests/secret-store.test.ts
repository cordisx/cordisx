...
describe('launcher Host-private channel secret store', () => {
  it('writes, resolves, replaces and deletes with no secret/ref in renderer-safe results', async () => {
    ...
    await expect(resolveLauncherSecret(reference, { platform: 'darwin', keychainBackend: backend })).resolves.toBe('first-test-secret')

    await expect(store.capture({ captureId: capture.captureId, secret: 'rotated-test-secret' })).resolves.toMatchObject({ state: 'set' })
    await expect(resolveLauncherSecret(reference, { platform: 'darwin', keychainBackend: backend })).resolves.toBe('rotated-test-secret')
  ...
  it('is honestly unavailable on non-macOS and rejects unknown capture authority', async () => {
    ...
    const capture = store.beginCapture({ profileId: 'work', connectionId: 'feishu-prod' })
    await expect(store.capture({ captureId: capture.captureId, secret: 'test-secret' })).resolves.toMatchObject({ state: 'unavailable' })
  ...
  it('uses the exact opaque channel ref shape and refuses unsafe identity segments', () => {
    ...
    expect(() => channelKeychainReference({ profileId: '../unsafe', connectionId: 'lark' })).toThrow('invalid secret profile id')
    expect(() => channelKeychainReference({ profileId: 'default', connectionId: 'lark/unsafe' })).toThrow('invalid secret connection id')
  })
})
