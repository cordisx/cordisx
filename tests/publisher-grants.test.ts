...
function statement(overrides: Partial<Record<string, unknown>> = {}): PublisherGrantStatement {
  const unsigned = {
    ...
    payload: {
      grantId: 'grant-000000000001', pluginId: 'example.paid-plugin', offerId: 'pro-annual',
      devicePublicKeyHash: devicePublicKeyHash(devicePublicKey), nonce: '0123456789abcdefghijklmnopqrstuv',
      notBefore: '2026-08-26T00:00:00Z', expiresAt: '2026-09-25T00:00:00Z', refreshAfter: '2026-09-18T00:00:00Z',
      offlineGraceSeconds: 604800, versionRange: '>=1.0.0 <2.0.0', features: ['sync', 'export'],
    },
    ...overrides,
  }
  return { ...unsigned, signature: { algorithm: 'Ed25519', value: sign(null, canonicalPublisherGrantSigningInput(unsigned as Omit<PublisherGrantStatement, 'signature'>), publisher.privateKey).toString('base64url') } } as PublisherGrantStatement
}

const keys: PublisherKeyRegistry = { async resolve(input) { return input.id === issuer.id && input.keyId === issuer.keyId && input.environment === issuer.environment ? publisher.publicKey : undefined } }
const devices: DeviceKeyProvider = { async current() { return { keyId: 'host-device-key-1', publicKey: devicePublicKey, async sign(input) { return sign(null, input, device.privateKey) } } } }
...
function clock(initial?: string): TrustedTimeStore & { state: { lastTrustedAt?: string } } {
  ...
  return { state, async read() { return { ...state } }, async write(value) { state.lastTrustedAt = value.lastTrustedAt } }
}
...
describe('PublisherGrant launcher gate', () => {
  it('verifies a Host-registered issuer key and rejects altered claims', async () => {
    ...
    await expect(verifyPublisherGrantStatement({ ...valid, payload: { ...valid.payload as object, pluginId: 'other.plugin' } }, keys)).rejects.toThrow('signature')
  })
  ...
  it('rejects unknown fields, invalid timing, and self-transfers before key use', () => {
    ...
    expect(() => parsePublisherGrantStatement({ ...valid, payload: { ...valid.payload as object, refreshAfter: '2026-10-01T00:00:00Z' } })).toThrow('timing')
    expect(() => parsePublisherGrantStatement({ ...valid, kind: 'transfer', payload: { grantId: 'grant-000000000001', fromDevicePublicKeyHash: devicePublicKeyHash(devicePublicKey), toDevicePublicKeyHash: devicePublicKeyHash(devicePublicKey), nonce: '0123456789abcdefghijklmnopqrstuv', notBefore: '2026-08-26T00:00:00Z', expiresAt: '2026-08-27T00:00:00Z' } })).toThrow('transfer')
  })
  ...
  it('uses a non-decreasing trusted clock and only permits bounded offline grace', () => {
    ...
    expect(evaluatePublisherGrantTime(grant, { lastTrustedAt: '2026-09-20T00:00:00Z' }, new Date('2026-09-01T00:00:00Z'))).toMatchObject({ state: 'active', clockRollbackDetected: true })
    expect(evaluatePublisherGrantTime(grant, { lastTrustedAt: '2026-09-26T00:00:00Z' }, new Date('2026-09-26T00:00:00Z'))).toMatchObject({ state: 'grace' })
    expect(evaluatePublisherGrantTime(grant, { lastTrustedAt: '2026-10-03T00:00:01Z' }, new Date('2026-10-03T00:00:01Z'))).toMatchObject({ state: 'expired' })
  })
  ...
  it('uses proof of possession, persists only registry-attested time, and projects features after atomic activation', async () => {
    ...
    const gate = new PublisherGrantLifecycleGate(keys, devices, trusted, { async activate(input) { request = input; return { status: 'activated', trustedAt: '2026-08-27T00:00:00Z' } } })
    await expect(gate.activate(statement(), target, new Date('2026-08-27T00:00:00Z'))).resolves.toEqual({ state: 'activated', features: ['sync', 'export'] })
