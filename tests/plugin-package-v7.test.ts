...
async function fixture(packageVersion: 6 | 7): Promise<string> {
  const root = await mkdtemp(path.join(os.tmpdir(), 'cordisx-package-v7-'))
  roots.add(root)
...
describe('plugin-package/v7 Host boundary', () => {
    ...
    new JsonPackageManifestV2Resolver({
      runtimeValidators: {
        [CORDISX_PLUGIN_MANIFEST_SCHEMA_V7]: value =>
          normalizePluginManifestV7(value, 'canvas-v7', new CapabilityRiskCatalog()),
      },
    })
  ...
  it('rejects Host DOM capabilities from the isolated canvas manifest', () => {
    ...
    ).toThrow(/must not declare Host DOM/u)
  })
})
