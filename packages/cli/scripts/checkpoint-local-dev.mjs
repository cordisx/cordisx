...
import {
  ...
  parseCheckpointArgs,
  pathExists,
  rendererGenerationProjection,
  REPORT_SCHEMA,
  repositoryStatus,
  sha256,
  snapshotRuntimePaths,
...
await Promise.all([
  ensurePrivateDirectory(checkpointRoot),
...
const protectedRepoBefore = await Promise.all(
  protectedRepoPaths.map(async target => ({ target, exists: await pathExists(target) })),
)
...
const repoRuntimeTargets = [
  ...
  'artifacts',
  'cordisx.config.json',
  'config/ui-demos/apps',
  'config/ui-demos/cache',
  'config/ui-demos/state',
]
...
const report = {
  ...
  inputs: {
    repoRoot,
    repoHead: execFileSync('git', ['-C', repoRoot, 'rev-parse', 'HEAD'], { encoding: 'utf8' }).trim(),
    executable,
    cliMode: options['cli-bin'] === undefined ? 'workspace-build' : 'packed-binary',
    cli: options['cli-bin'] ?? cliEntry,
    checkpointRoot,
    sourceRoot,
    entry,
    cordisxHome,
  ...
  artifacts: {
    ...
    failedScreenshot: failedScreenshotPath,
...
class CdpClient {
  ...
  async value(expression, awaitPromise = false) {
    ...
    if (response.exceptionDetails !== undefined) {
      throw new Error(
        response.exceptionDetails.exception?.description ?? response.exceptionDetails.text ?? 'CDP evaluation failed',
      )
    }
...
async function waitForTarget(id) {
  ...
  while (Date.now() < deadline) {
    ...
    try {
      const target = (await targetList()).find(item =>
        item.type === 'page' && item.url === 'app://-/index.html' && (id === undefined || item.id === id)
      )
      if (target?.webSocketDebuggerUrl !== undefined) {
        const client = await new CdpClient(target.webSocketDebuggerUrl).open()
        const ready = await client.value(
...
const stateExpression = `(() => {
  ...
  return {
    ...
    publicPrivacy: { sourcePathAbsent: !raw.includes(${
  JSON.stringify(entry)
}), localDevelopmentAbsent: !raw.includes('localDevelopment'), developmentOwnProperty: plugin === undefined ? null : Object.hasOwn(plugin, 'development') },
    manager: { detailOpen: detail !== null, sourcePath: privatePath, state: privateState, pathVisible: privatePath === ${
  JSON.stringify(entry)
...
async function openManagerRuntime(client) {
  ...
  return await waitFor(
    client,
    stateExpression,
    value => value.manager.pathVisible && value.manager.state !== null,
    'Manager private local-dev projection',
  )
}
...
function assertStateSafety(state, label) {
  if (
    !state.publicPrivacy.sourcePathAbsent || !state.publicPrivacy.localDevelopmentAbsent
    || state.publicPrivacy.developmentOwnProperty !== false
  ) {
...
function processInventory(profilePath) {
  ...
  return {
    launcher: launcherRecord
      ?? (launcherPid === undefined ? null : { pid: launcherPid, ppid: null, pgid: null, command: null }),
    descendants,
    appProcesses,
    profileProcesses,
    processGroupIds: [
      ...new Set([launcherRecord?.pgid, ...appProcesses.map(item => item.pgid)].filter(Number.isInteger)),
...
try {
  ...
  const initial = await waitFor(
    main.client,
    stateExpression,
    value => value.plugin?.name === 'Checkpoint DEV-1' && value.plugin.status === 'active',
    'DEV-1 generation',
  )
  ...
  const failed = await waitFor(
    main.client,
    stateExpression,
    value => value.manager.state === 'failed',
    'syntax failure',
  )
  assertStateSafety(failed, 'failed generation')
  const invariantsPassed =
    JSON.stringify(invariantProjection(failed)) === JSON.stringify(invariantProjection(initialManager))
  ...
  const dev2 = await waitFor(
    main.client,
    stateExpression,
    value => value.plugin?.name === 'Checkpoint DEV-2' && value.manager.state === 'ready',
    'DEV-2 recovery',
  )
  ...
  const version = await fetch(`http://127.0.0.1:${port}/json/version`, { signal: AbortSignal.timeout(1_000) }).then(
  ...
  const secondaryState = await waitFor(
    secondary.client,
    stateExpression,
    value =>
      value.plugin?.name === 'Checkpoint DEV-2' && value.plugin.status === 'active'
      && value.publicPrivacy.sourcePathAbsent,
    'second renderer generation',
  )
  ...
  if (JSON.stringify(primaryDev2Generation) !== JSON.stringify(secondaryDev2Generation)) {
    throw new Error(
      `second renderer generation differs from primary DEV-2: ${
        JSON.stringify({ primaryDev2Generation, secondaryDev2Generation })
      }`,
    )
  }
  ...
  while (Date.now() < secondaryGoneDeadline && (await targetList()).some(item => item.id === secondaryId)) {
    await new Promise(resolve => setTimeout(resolve, 100))
  }
  if ((await targetList()).some(item => item.id === secondaryId)) {
    throw new Error(`secondary renderer remained open: ${secondaryId}`)
  }

  await writeFile(entry, fixture('DEV-3'), { mode: 0o600 })
  const dev3 = await waitFor(
    main.client,
    stateExpression,
    value => value.plugin?.name === 'Checkpoint DEV-3' && value.manager.state === 'ready',
    'post-secondary DEV-3 generation',
  )
  assertStateSafety(dev3, 'post-secondary DEV-3')
  report.stages.multiRenderer = {
    targetId: secondaryId,
    joined: secondaryState,
    closed: true,
    exactDev2GenerationMatch: true,
    primaryDev2Generation,
    secondaryDev2Generation,
    postCloseGeneration: dev3,
    sha256: sha256({ secondaryState, dev3 }),
  }
  ...
  if (homeMode !== 0o700 || stateMode !== 0o700 || profileMode !== 0o700 || grantMode !== 0o600) {
    throw new Error(
      `unexpected runtime modes: home=${homeMode.toString(8)} state=${stateMode.toString(8)} profile=${
        profileMode.toString(8)
      } grant=${grantMode.toString(8)}`,
    )
  }
  ...
  const protectedRepoAfter = await Promise.all(
    protectedRepoPaths.map(async target => ({ target, exists: await pathExists(target) })),
  )
  if (
    repoStateAfter !== repoStateBefore || JSON.stringify(protectedRepoAfter) !== JSON.stringify(protectedRepoBefore)
  ) {
    throw new Error('real App checkpoint changed the Host owning repository')
  }
  report.filesystem = {
    home: { path: cordisxHome, mode: '0700' },
    state: { path: stateRoot, mode: '0700' },
    publisherGrant: { path: grants[0], mode: '0600' },
    profile: { path: profiles[0], mode: '0700' },
    cwdStateAbsent: true,
  ...
  secondary?.client.close()
  main?.client.close()
  if (secondaryId !== undefined && browser !== undefined) {
    await browser.send('Target.closeTarget', { targetId: secondaryId }).catch(() => undefined)
  }
  browser?.close()
  await stopLauncher()
  closeSync(logFd)
  ...
  const processGroupsAfter = beforeInventory === undefined
    ? []
    : afterTable.filter(item => beforeInventory.processGroupIds.includes(item.pgid))
  const profileProcessesAfter = profilePath === undefined
    ? []
    : afterTable.filter(item => item.command.includes(profilePath))
  const finalRepoStatus = repositoryStatus(repoRoot)
  const finalProtectedRepoPaths = await Promise.all(
    protectedRepoPaths.map(async target => ({ target, exists: await pathExists(target) })),
  )
  ...
  report.cleanup = {
    launcherExited: launcherExit !== undefined,
    launcherExit: launcherExit ?? null,
    portClosed,
    remainingAppProcesses: remaining,
  ...
  if (
    !portClosed || remaining.length > 0 || processGroupsAfter.length > 0 || profileProcessesAfter.length > 0
    || !finalCwdStateAbsent || !finalRepoUnchanged || !runtimeFilesUnchanged
  ) {
    report.result = 'failed'
    report.error ??= {
      message:
        'checkpoint cleanup left a process group, profile process, CDP port, cwd state, or owning-repository file change',
    }
  }
  ...
  console.log(
    JSON.stringify(
      { result: report.result, report: reportPath, screenshot: finalScreenshotPath, port, cleanup: report.cleanup },
      null,
      2,
    ),
  )
