...
import type { CordisXPersistedSession, CordisXSessionEventPersistence } from '../renderer/agent-session-runtime.js'
...
function sessionHeader(value: unknown, id: string): SessionHeader {
  ...
  if (
    item.id !== id || !Number.isSafeInteger(item.formatVersion) || (item.formatVersion as number) < 1
  ...
  if (item.parentSessionId !== undefined && !opaque(item.parentSessionId)) {
    throw new Error('Parent SessionId is invalid')
  }
  if (item.origin !== undefined && item.origin !== 'subagent') throw new Error('Session origin is invalid')
  if (
    item.delegationDepth !== undefined
    && (!Number.isSafeInteger(item.delegationDepth) || (item.delegationDepth as number) < 0)
  ) throw new Error('Session delegation depth is invalid')
  if (item.agentPreset !== undefined && typeof item.agentPreset !== 'string') {
    throw new Error('Session agent preset is invalid')
  }
...
function sessionEvent(value: unknown, sessionId: string, seq: number): SessionEvent {
  ...
  if (
    item.$schema !== EVENT_SCHEMA || item.contract !== 'cordisx.session-event/v1' || item.schemaVersion !== 1
  ...
  ) throw new Error(`Session event ${seq} envelope is invalid`)
  if (item.ignorable !== undefined && item.ignorable !== true) {
    throw new Error(`Session event ${seq} ignorable flag is invalid`)
  }
  if (!KNOWN_EVENT_TYPES.has(item.type as string) && item.ignorable !== true) {
    throw new Error(`Session event ${seq} has an unknown required type`)
  }
  if (
    item.sourceEventSeqs !== undefined && (!Array.isArray(item.sourceEventSeqs)
      || item.sourceEventSeqs.some(source => !Number.isSafeInteger(source) || (source as number) < 0))
  ) throw new Error(`Session event ${seq} source cursors are invalid`)
  if (
    !SURFACE_EVENT_TYPES.has(item.type as string)
    && (item.sourceEventSeqs !== undefined || item.surfaceOp !== undefined)
  ) {
  ...
  if (item.surfaceOp !== undefined) {
    ...
    if (item.surfaceOp !== 'append') {
      ...
      if (
        operation.op !== 'replace' || !Number.isSafeInteger(operation.start) || (operation.start as number) < 0
...
function persistedSession(value: unknown): CordisXPersistedSession {
  ...
  if (
    !opaque(item.id) || !Number.isSafeInteger(item.generation) || (item.generation as number) < 1
...
function emptyLedger(): StoredLedger {
  return {
    ...
    sessions: Object.create(null) as Record<string, StoredSession>,
  }
}
...
function parseLedger(value: unknown): StoredLedger {
  ...
  if (root.contract !== CONTRACT || root.storeVersion !== STORE_VERSION) {
    throw new Error('Playground Agent Session store version is unsupported')
  }
...
export class PlaygroundAgentSessionStore implements CordisXSessionEventPersistence {
  ...
  async load(): Promise<readonly CordisXPersistedSession[]> {
    return await this.serialized(async () =>
      Object.values((await this.read()).sessions).map(session => persistedSession(session))
    )
  }
  ...
  async append(
    ...
    if (
      !opaque(input.sessionId) || !Number.isSafeInteger(input.sessionGeneration) || input.sessionGeneration < 1
      || !Number.isSafeInteger(input.expectedSeq) || input.expectedSeq < 0 || input.events.length === 0
    ...
    await this.serialized(async () => {
      ...
      if (
        input.expectedSeq !== current.events.length || current.events.length + events.length > MAX_EVENTS_PER_SESSION
      ) throw new Error('cursor-conflict')
  ...
  async updateSetup(
    input: { readonly sessionId: string; readonly sessionGeneration: number; readonly setup: AgentSetup },
  ): Promise<void> {
  ...
  async handle(request: PlaygroundAgentSessionStoreRequest): Promise<PlaygroundAgentSessionStoreResult> {
    try {
      ...
      const code = error instanceof Error
          && ['session-conflict', 'generation-conflict', 'cursor-conflict', 'invalid-request', 'store-unavailable']
