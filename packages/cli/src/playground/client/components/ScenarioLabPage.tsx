...
class ScenarioWorkbenchErrorBoundary extends Component<SimulatorTaskScenarioWorkbenchProps & { readonly children: ReactNode }, ScenarioWorkbenchErrorBoundaryState> {
  ...
  render() {
    ...
    >
      ...
      }}>{en ? 'Retry debugger' : '重试调试台'}</button>
    </section>
  }
}
...
type ComposerEventType = 'agent-reply' | 'permission-request' | 'task-delegation' | 'member-self-introduction' | 'typed-failure'
...
function SimulatorTaskScenarioWorkbenchContent({ locale, task, controller }: SimulatorTaskScenarioWorkbenchProps) {
  ...
  const inputInvalid = eventType === 'agent-reply' ? message.trim() === ''
    : eventType === 'permission-request' ? permissionRationale.trim() === ''
      : eventType === 'task-delegation' ? delegationMemberId === '' || delegationTask.trim() === ''
      : eventType === 'typed-failure' ? failureMessage.trim() === ''
        : participantId.trim() === '' || memberId.trim() === '' || runId.trim() === ''
  const writesToRoom = eventType === 'agent-reply' || eventType === 'permission-request' || eventType === 'task-delegation'
  ...
  return <ConfigProvider globalConfig={{ attach: attachTDesignPopup }}><div
    ref={tDesignHostRef}
  ...
  >
    <ol ref={timelineRef} className="pg-event-timeline" aria-label={en ? `${task.debugTaskId} event timeline` : `${task.debugTaskId} 事件时间线`}>
            ...
            return <li key={entry.id} className="pg-event-timeline-item" data-trace-source={entry.source} data-trace-direction={entry.direction} data-trace-presentation={presentation} data-timeline-alignment={alignment}>
              ...
              {compactEvent
                ...
                : <article className={`pg-event-message${toolEvent ? ' pg-event-tool-message' : ''}`}>
                    ...
                    {pendingApproval ? <div className="pg-event-inline-actions" role="group" aria-label={en ? 'Permission decision' : '权限决定'}>
                      <button type="button" onClick={() => { void controller.resolvePendingApproval('approved') }}>{en ? 'Approve' : '允许'}</button>
                      <button type="button" onClick={() => { void controller.resolvePendingApproval('denied') }}>{en ? 'Deny' : '拒绝'}</button>
                      <button type="button" onClick={() => { void controller.resolvePendingApproval('cancelled') }}>{en ? 'Cancel' : '取消'}</button>
                    </div> : null}
                  </article>}
            </li>
          })}
    ...
    >
      <div className="pg-event-composer-type">
        ...
        <Select
          ...
          onChange={value => { if (isComposerEventType(value)) setEventType(value) }}
        />
      </div>
      <div className="pg-event-composer-input">
        {eventType === 'agent-reply' ? <textarea rows={1} value={message} onChange={event => setMessage(event.currentTarget.value)} placeholder={en ? 'Reply from the bound Agent to Chatroom' : '绑定 Agent 回复 Chatroom 的消息'} aria-label={en ? 'Agent reply' : 'Agent 回复内容'} /> : null}
        {eventType === 'permission-request' ? <textarea rows={1} value={permissionRationale} onChange={event => setPermissionRationale(event.currentTarget.value)} placeholder={en ? 'Why is the bound Agent requesting approval?' : '说明绑定 Agent 请求批准的原因'} aria-label={en ? 'Agent approval reason' : 'Agent 请求批准原因'} /> : null}
        {eventType === 'task-delegation' ? <div className="pg-event-composer-delegation">
          <Select
          ...
          />
          <textarea rows={1} value={delegationTask} onChange={event => setDelegationTask(event.currentTarget.value)} placeholder={en ? 'Work for the new entity session' : '输入要交给新实体完成的工作'} aria-label={en ? 'Delegated task' : '下发任务内容'} />
        </div> : null}
        {eventType === 'typed-failure' ? <textarea rows={1} value={failureMessage} onChange={event => setFailureMessage(event.currentTarget.value)} placeholder={en ? 'Failure input' : '模拟失败输入'} aria-label={en ? 'Failure input' : '模拟失败输入'} /> : null}
        {eventType === 'member-self-introduction' ? <div className="pg-event-composer-identifiers">
          <input value={participantId} onChange={event => setParticipantId(event.currentTarget.value)} placeholder="participantId" aria-label="participantId" />
          <input value={memberId} onChange={event => setMemberId(event.currentTarget.value)} placeholder="memberId" aria-label="memberId" />
          <input value={runId} onChange={event => setRunId(event.currentTarget.value)} placeholder="runId" aria-label="runId" />
        </div> : null}
        {isolatedAgentProbe ? <span className="pg-event-composer-scope">
          {en ? 'Runs only in the disposable local Agent generation; it is not projected into the Room.' : '仅在可丢弃的本地 Agent generation 中模拟，不会写入 Room。'}
        </span> : null}
      </div>
      <button className="pg-event-composer-submit" type="submit" disabled={snapshot.injector.phase === 'injecting' || inputInvalid || roomBridgeUnavailable} title={roomBridgeUnavailable ? snapshot.injector.roomBridge.message : undefined} aria-label={eventType === 'agent-reply' ? (en ? 'Emit Agent reply' : '发送 Agent 回复') : eventType === 'permission-request' ? (en ? 'Emit Agent approval request' : '发送 Agent 批准请求') : eventType === 'task-delegation' ? (en ? 'Delegate task' : '下发任务') : (en ? 'Trigger event' : '触发事件')}>
        <HostIcon token="turns-submit" />
        <span>{eventType === 'agent-reply' ? (en ? 'Reply' : '回复') : eventType === 'permission-request' ? (en ? 'Request approval' : '请求批准') : eventType === 'task-delegation' ? (en ? 'Delegate' : '下发') : (en ? 'Trigger' : '触发')}</span>
      </button>
    </form>

    {selectedEvent === undefined ? null : <aside className="pg-event-drawer" role="dialog" aria-modal="false" aria-labelledby="pg-event-drawer-title" data-event-details-drawer="true">
      <header>
        <div>
          <span>{directionLabels[selectedEvent.direction][locale]}</span>
          <h2 id="pg-event-drawer-title">{selectedEvent.type}</h2>
        </div>
        <IconButton ref={drawerCloseRef} icon="close" label={en ? 'Close event details' : '关闭事件详情'} onClick={closeDrawer} />
      </header>
      <div className="pg-event-drawer-body">
        <p>{selectedEvent.summary}</p>
        <dl>
          <div><dt>{en ? 'Source' : '来源'}</dt><dd>{selectedEvent.source}</dd></div>
...
export function SimulatorTaskScenarioWorkbench(props: SimulatorTaskScenarioWorkbenchProps) {
  return <ScenarioWorkbenchErrorBoundary key={props.task.taskRef} {...props}>
