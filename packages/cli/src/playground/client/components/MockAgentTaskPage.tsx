        ...
        <div>
          ...
          <p>
            {en
              ...
              : '当前可丢弃的 Playground 会话中已没有此任务；它可能已在刷新或重置后过期。'}
          </p>
        </div>
      </header>
      <section
        className="pg-simulator-task-not-found-card"
        aria-label={en ? 'Unavailable task details' : '不可用任务详情'}
      >
        <dl>
          <div>
            <dt>{en ? 'Requested task' : '请求的任务'}</dt>
            <dd>{taskId}</dd>
          </div>
          <div>
            <dt>{en ? 'State' : '状态'}</dt>
            <dd>{en ? 'Not found or expired' : '不存在或已过期'}</dd>
          </div>
        </dl>
      ...
      <header className="pg-simulator-task-header">
        ...
        <div className="pg-simulator-task-header-actions">
          {task.scenario === undefined ? null : (
            ...
            >
              {en ? 'Scenario' : '场景'} {task.scenario.code} · {en ? 'step' : '步骤'}{' '}
              {task.scenario.stepIndex}/{task.scenario.stepCount}
            </span>
          )}
          <span className="pg-simulator-task-status" data-status={task.status}>{task.status}</span>
          <span className="pg-simulator-task-mode">
            {task.origin === 'agent-session'
              ? 'Agent / Session'
              : task.origin === 'host-session'
              ? (en ? 'Host snapshot' : 'Host 快照')
              : (en ? 'Simulator' : '模拟器')}
          </span>
          <MoreMenu
            label={en ? 'Task actions' : 'Task 操作'}
            items={[{
