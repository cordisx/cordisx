...
const unavailable = (en: boolean): string =>
  en ? 'Connector unavailable in Playground' : 'Playground 中 Connector 不可用'

export function createPlaygroundConversationCommands(
...
export function createPlaygroundConversationFixture(
  mode: PlaygroundFixtureMode,
  locale: 'zh-CN' | 'en',
    ...
    entries: [
      {
        ...
        body: [
          en
            ? 'Review the Host-owned conversation shell at common window sizes.'
            : '请在常见窗口尺寸下评审 Host-owned 会话壳。',
        ],
      ...
      },
      {
        ...
        timestamp: '2026-08-29T01:00:05.000Z',
        deliveryState: 'sent',
        runState: 'running',
        ariaLive: 'polite',
        actions: [],
      },
      {
        ...
        itemId: 'entry-5',
        sequence: 5,
        label: en ? 'One optional task was stopped.' : '一个可选任务已停止。',
        state: 'warning',
        ariaLive: 'off',
      },
      {
        ...
        timestamp: '2026-08-29T01:00:08.000Z',
    ...
    headerActions: [
      {
        ...
        label: en ? 'Stop' : '停止',
        icon: 'host:close',
        command: { id: 'room.stop' },
        disabled: true,
        disabledReason: unavailableReason,
      },
      {
        id: 'room-menu',
        label: en ? 'Room menu' : '房间菜单',
        icon: 'host:more',
        command: { id: 'room.menu' },
        disabled: true,
        disabledReason: unavailableReason,
      },
    ],
