...
import { PreviewConnectionNotice, type PlaygroundPreviewConnectionState } from './components/PreviewConnectionNotice.js'
...
type PlaygroundTaskDetailsResolverWindow = Window & {
  [PLAYGROUND_TASK_DETAILS_SESSION_RESOLVER]?: (input: PlaygroundHostSessionTaskContext) => PlaygroundHostSessionTaskContext['detailsUrl']
}

function PlaygroundNavigationToggleIcon({ collapsed }: { readonly collapsed: boolean }) {
  return <svg className="pg-host-chrome-icon pg-host-navigation-toggle-icon" viewBox="0 0 16 16" fill="none" aria-hidden="true">
    ...
    <path d={collapsed ? 'm8.25 5.75 2.25 2.25-2.25 2.25' : 'm10.5 5.75-2.25 2.25 2.25 2.25'} stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
  </svg>
}

function initialNavigationCollapsed(): boolean {
  try { return sessionStorage.getItem(PLAYGROUND_NAVIGATION_COLLAPSED_KEY) === 'true' } catch { return false }
}

function previewResetReadback(): Pick<PlaygroundPreviewResetResult, 'roomRows' | 'recentTaskRows' | 'simulatorRecords' | 'sources' | 'instanceId' | 'serverGeneration' | 'appliedGeneration'> {
...
function previewResetMessage(result: Pick<PlaygroundPreviewResetResult, 'roomRows' | 'recentTaskRows' | 'simulatorRecords' | 'sources' | 'serverGeneration' | 'appliedGeneration'>, en: boolean): string {
...
function PreviewResetConfirmation({ locale, phase, error, onCancel, onConfirm }: {
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      const buttons = [cancel.current, confirm.current].filter((button): button is HTMLButtonElement => button !== null && !button.disabled)
  ...
  return <div className="pg-preview-reset-backdrop" onPointerDown={event => {
    if (event.target === event.currentTarget && !resetting) onCancel()
  }}>
    <section className="pg-preview-reset-confirm" role="alertdialog" aria-modal="true" aria-labelledby="pg-preview-reset-title" aria-describedby="pg-preview-reset-description">
      <h2 id="pg-preview-reset-title">{en ? 'Clear local preview data?' : '清空本地预览数据？'}</h2>
      <p id="pg-preview-reset-description">{en
        ...
        : '这会永久清除连接到当前预览实例的所有标签中的可丢弃 Chatroom fixture、最近 Simulator tasks、快照、ledgers 与调试会话。每个标签应用同一清理代际后会自动刷新。主题、语言和导航偏好会保留。'}</p>
      ...
      <footer>
        ...
        <button ref={confirm} type="button" className="pg-preview-reset-danger" disabled={resetting} onClick={onConfirm}>{resetting ? (en ? 'Clearing…' : '正在清空…') : (en ? 'Clear and refresh' : '清空并刷新')}</button>
      </footer>
    </section>
  </div>
}
...
export function App() {
  ...
  const [previewConnection, setPreviewConnection] = useState<'connected' | PlaygroundPreviewConnectionState>('connected')
  const initialResetMarker = useRef<PlaygroundPreviewResetMarker | undefined>(readPlaygroundPreviewResetMarker(sessionStorage))
  ...
  const scenarioControllers = useRef(new Map<string, PlaygroundScenarioLabController | {
    readonly bindingSignature: string
    readonly controller: PlaygroundScenarioLabController
  }>())
  const scenarioControllerFor = (entry: PlaygroundScenarioLabController | {
    readonly bindingSignature: string
    readonly controller: PlaygroundScenarioLabController
  ...
  try { simulatorSessionRecordCount = countPlaygroundSimulatorSessionRecords(sessionStorage) } catch { /* optional browser session storage */ }
  ...
  useEffect(() => {
    ...
    const refresh = () => {
      ...
      setRouteHistory(current => current.available === next.available
  ...
  const simulatorTask = recentTasks.find(task => task.taskRef === simulatorTaskId || task.debugTaskId === simulatorTaskId)
  ...
  if (simulatorTask !== undefined) {
    ...
    if (existing !== undefined && 'controller' in existing && existing.bindingSignature === bindingSignature) {
      ...
      scenarioControllers.current.set(simulatorTask.taskRef, { bindingSignature, controller: simulatorScenarioController })
    }
  }
  useEffect(() => () => {
    for (const entry of scenarioControllers.current.values()) scenarioControllerFor(entry).dispose()
  ...
  const toggleNavigation = () => {
    ...
    try { sessionStorage.setItem(PLAYGROUND_NAVIGATION_COLLAPSED_KEY, String(next)) } catch { /* optional developer-session preference */ }
  }
  const moveRouteHistory = (delta: -1 | 1) => {
    const go = window.__cordisxRuntime?.goPlaygroundRouteHistory
    if (go === undefined) return
    void go(delta).catch(() => setRouteHistory(window.__cordisxRuntime?.playgroundRouteHistory?.() ?? UNAVAILABLE_ROUTE_HISTORY))
  }
  ...
  const closeResetConfirmation = () => {
    ...
    setResetConfirmationOpen(false)
    queueMicrotask(() => shell.current?.querySelector<HTMLButtonElement>('.pg-sidebar-control')?.focus({ preventScroll: true }))
  }
  ...
  const reset = async () => {
    ...
    try {
      ...
      if (resetRequestAuthority.current !== authority) throw new Error('Preview reset request authority is no longer active')
  ...
  const menuItems: readonly HostMenuItem[] = [
    ...
    { kind: 'action' as const, id: 'scenario-lab', label: en ? 'Chatroom interaction scenarios' : 'Chatroom 交互场景', selected: simulatorTask !== undefined, disabled: recentTasks.length === 0, onSelect: openScenarioWorkbench },
    ...
    { kind: 'heading', id: 'runtime-reset-epoch', label: en
      ? `Preview reset epoch · server ${resetEpoch.server?.generation ?? 'starting'} · this tab ${resetEpoch.applied?.generation ?? 'not applied'}${resetEpoch.synchronized ? ' · synchronized' : ' · pending'}`
      : `预览清理代际 · server ${resetEpoch.server?.generation ?? '启动中'} · 本标签 ${resetEpoch.applied?.generation ?? '未应用'}${resetEpoch.synchronized ? ' · 已同步' : ' · 待同步'}` },
    { kind: 'heading', id: 'runtime-simulator-sources', label: en
    ...
    ...(resetResult === undefined ? [] : [{
      ...
      id: 'reset-result',
      label: `${resetResult.status === 'complete' ? (en ? 'Last clear complete' : '上次清理完成') : (en ? 'Last clear failed' : '上次清理失败')} · ${previewResetMessage(resetResult, en)}`,
    }]),
    { kind: 'action', id: 'reload', label: en ? 'Reload plugins' : '重载插件', onSelect: () => window.location.reload() },
    { kind: 'action', id: 'reset', label: en ? 'Clear preview data…' : '清空预览数据…', onSelect: openResetConfirmation },
  ]
  ...
  >
    <button
      ...
      aria-label={navigationCollapsed ? (en ? 'Expand Playground navigation' : '展开 Playground 导航') : (en ? 'Collapse Playground navigation' : '折叠 Playground 导航')}
      title={navigationCollapsed ? (en ? 'Expand navigation · developer simulation' : '展开导航 · 开发模拟') : (en ? 'Collapse navigation · developer simulation' : '折叠导航 · 开发模拟')}
      aria-expanded={!navigationCollapsed}
      onClick={toggleNavigation}
    ><PlaygroundNavigationToggleIcon collapsed={navigationCollapsed} /></button>
    <button
      type="button"
    ...
    ><HostIcon token="action.back" className="pg-host-chrome-icon" size={16} /></button>
    <button
      type="button"
      className="pg-host-chrome-button"
      ...
      onClick={() => moveRouteHistory(1)}
    ><HostIcon token="control.chevron-right" className="pg-host-chrome-icon" size={16} /></button>
  ...
  return (
    ...
    >
      {resetConfirmationOpen ? <PreviewResetConfirmation
      ...
      <div className="pg-main-column">
        {previewNoticeState === undefined
          ? simulatorTaskId !== undefined && simulatorTask === undefined
            ? <MockAgentTaskNotFoundPage taskId={simulatorTaskId} locale={environment.locale} onReturn={closeSimulatorTask} />
            : simulatorTask === undefined || simulatorScenarioController === undefined
              ? <HostSeats mode={fixtureMode} locale={environment.locale} />
              : <MockAgentTaskPage task={simulatorTask} locale={environment.locale} scenarioController={simulatorScenarioController} />
