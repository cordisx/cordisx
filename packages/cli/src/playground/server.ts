...
window.__cordisxPlaygroundAgentSessionRequestV1=(payload)=>fetch('/api/agent-sessions',{method:'POST',headers:{'content-type':'application/json'},body:payload}).then(r=>r.text()).then(value=>window.__cordisxPlaygroundAgentSessionReceiveV1?.(value)).catch(error=>window.__cordisxPlaygroundAgentSessionReceiveV1?.(JSON.stringify({requestId:JSON.parse(payload).requestId,ok:false,error:String(error)})));
window.__cordisxOwnerDocumentRequestV1=(payload)=>fetch('/api/documents',{method:'POST',headers:{'content-type':'application/json'},body:payload}).then(r=>r.text()).then(value=>window.__cordisxOwnerDocumentReceiveV1?.(value)).catch(error=>window.__cordisxOwnerDocumentReceiveV1?.(JSON.stringify({requestId:JSON.parse(payload).requestId,ok:false,error:String(error)})));
...
window.__cordisxProviderRequestV1=(payload)=>fetch('/api/provider',{method:'POST',headers:{'content-type':'application/json'},body:payload}).then(r=>r.text()).then(value=>window.__cordisxProviderReceiveV1?.(value)).catch(error=>window.__cordisxProviderReceiveV1?.(JSON.stringify({requestId:JSON.parse(payload).requestId,ok:false,error:String(error)})));
  ...
  const server: Server = createServer(async (request, response) => {
    try {
      ...
      if (request.method === 'POST' && url.pathname === '/api/agent-sessions') {
        json(response, 200, await session.handleAgentSessionStoreRequest(await body(request)))
        return
      }
      ...
      if (request.method === 'POST' && url.pathname === '/api/provider') {
        json(response, 200, await session.handleProviderRequest(await body(request)))
        return
      }
