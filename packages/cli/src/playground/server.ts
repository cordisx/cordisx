...
function fixtureInfo(source: Record<string, unknown>, sourcePath: string): FixtureInfo {
  const playground = source.playground
  const name = playground !== null && typeof playground === 'object' && typeof (playground as Record<string, unknown>).name === 'string'
    ? (playground as Record<string, unknown>).name as string
    : path.basename(sourcePath)
  return { name, source: path.basename(sourcePath) }
}

function page(fixture: FixtureInfo): string {
...
async function boot(reason){status.textContent='building '+reason+'…';const source=await fetch('/api/bundle?revision='+revision,{cache:'no-store'}).then(r=>r.text());(0,eval)(source);for(let i=0;i<100&&document.documentElement.dataset.cordisxReady!=='true';i++)await new Promise(r=>setTimeout(r,20));const runtime=window.__cordisxRuntime;const snapshot=runtime?.snapshot?.();renderSnapshot(snapshot);status.textContent=runtime?'active · generation '+revision:'boot failed'}
  ...
  const server: Server = createServer(async (request, response) => {
    try {
      ...
      if (request.method === 'GET' && url.pathname === '/') {
        response.writeHead(200, { 'content-type': 'text/html; charset=utf-8', 'cache-control': 'no-store' }); response.end(page(fixture)); return
      }
