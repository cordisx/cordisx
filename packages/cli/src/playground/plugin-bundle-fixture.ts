...
export function playgroundPluginBundleSnapshot(generation: string): CordisXPluginBundleManagerSnapshotV1 {
  ...
  const shared = (id: typeof bundleIds[number], name: string, policy: 'allow' | 'deny'): CordisXPluginBundleManagerItemV1 => ({
    id,
    name,
    description: id === 'workflow-essentials' ? 'A visual fixture for the Host-owned plugin bundle Manager.' : 'Shares one exact member with Workflow Essentials.',
    version: '1.0.0',
    ...
    members: [{
      pluginId: 'shared-notes', name: 'Shared Notes', requestedVersion: '1.0.0', installedVersion: '1.0.0', installedDigest: pluginDigest,
      required: true, enabledByDefault: true, enabled: true, state: 'shared', installedViaBundle: true,
      bundleIds, directClaim: false, runtimeDependentIds: ['workflow-runner'],
    }],
    permissions: [{
      permissionId, pluginId: 'shared-notes', capability: 'models.read', scopeLabel: '{}', required: true,
      bundlePolicy: policy, effectivePolicy: 'deny', effectiveSource: 'shared-bundle-merge', affectedBundleIds: bundleIds,
    }],
    claims: [
    ...
    records: [{ recordId: `${id}-install`, at: '2026-09-02T00:00:00.000Z', kind: 'install', outcome: 'applied', message: `Installed ${name}.`, pluginIds: ['shared-notes'] }],
  })
  return {
    $schema: CORDISX_PLUGIN_BUNDLE_MANAGER_SNAPSHOT_SCHEMA_V1,
    schemaVersion: 1,
    profileId: 'playground', revision: 2, pluginRevision: 4, runtimeGeneration: generation,
    operationsAvailable: false,
    bundles: [shared('workflow-essentials', 'Workflow Essentials', 'allow'), shared('team-governance', 'Team Governance', 'deny')],
  }
}
