...
function messageText(message: CordisXUserMessage): string {
  return message.content.map((block: CordisXAgentContentBlock) =>
    block.type === 'text'
      ? block.text
      : `[${block.mediaType} reference ${block.ref}]${block.summary === undefined ? '' : ` ${block.summary}`}`
  ).join('\n')
}
...
export class CodexCurrentConnectionAgentAdapter implements CordisXAgentAdapter {
  ...
  async deliver(
    input: CordisXAgentDeliveryInput,
    control: CordisXAgentDeliveryControl,
  ): Promise<CordisXAgentDeliveryOutcome> {
    ...
    if (!control.claim()) {
      return {
        terminal: 'failed',
        diagnostic: { code: 'interrupted', message: 'Agent delivery was cancelled before Codex claim' },
      }
    }
    control.projected()
    try {
      ...
      if (!result.accepted) {
        return {
          ...
          diagnostic: result.diagnostic
            ?? { code: 'adapter-failure', message: 'Codex current connection rejected Agent delivery' },
        }
      }
      return {
        terminal: 'forwarded',
      ...
      return {
        ...
        diagnostic: {
          ...
          message: 'Codex current-connection Agent delivery failed',
          retryable: true,
        },
      }
    }
...
function deltaChannel(
  method: string,
): 'assistant' | 'reasoning' | 'plan' | 'command' | 'file-change' | 'tool' | 'other' {
...
export class CodexAgentEventNormalizer {
  ...
  observe(notification: CodexAppServerNotification): void {
    ...
    if (notification.method === 'thread/started' && sessionId !== undefined) {
      if (this.sessions.has(sessionId)) {
        return this.diagnostic(sessionId, 'duplicate-thread-start', `Duplicate thread/started for ${sessionId}`)
      }
    ...
    if (!this.sessions.has(sessionId)) {
      ...
      const inferred = this.ledger.commit({
        sessionId,
        type: 'session.lifecycle',
        provenance: 'inferred',
        source: this.source,
        data: { phase: 'resumed', history: 'unknown' },
      })
      this.sessions.add(sessionId)
    ...
    if (notification.method === 'turn/started' && turnId !== undefined) {
      if (this.turns.has(turnId)) {
        return this.diagnostic(sessionId, 'duplicate-turn-start', `Duplicate turn/started for ${turnId}`)
      }
      const event = this.ledger.commit({
        sessionId,
        turnId,
        ...
        provenance: 'observed',
        source: this.source,
        ...causal(this.parents.get(`session:${sessionId}`)),
        data: { phase: 'started' },
      })
    ...
    if (notification.method === 'item/started' && turnId !== undefined && itemId !== undefined) {
      if (!this.turns.has(turnId)) {
        this.diagnostic(sessionId, 'missing-turn-start', `item/started arrived before turn/started for ${turnId}`)
      }
      if (this.items.has(itemId)) {
        return this.diagnostic(sessionId, 'duplicate-item-start', `Duplicate item/started for ${itemId}`)
      }
    ...
    if (notification.method === 'item/completed' && turnId !== undefined && itemId !== undefined) {
      ...
      if (existing === undefined) {
        return this.diagnostic(
          sessionId,
          'missing-item-start',
          `item/completed arrived before item/started for ${itemId}`,
        )
      }
      const event = this.ledger.commit({
        sessionId,
        ...
        data: { phase: 'completed', kind: existing.kind },
      })
      this.parents.set(`item:${itemId}`, event.eventId)
      return
    }
    if (notification.method === 'turn/completed' && turnId !== undefined) {
      if (!this.turns.has(turnId)) {
        return this.diagnostic(
          sessionId,
          'missing-turn-start',
          `turn/completed arrived before turn/started for ${turnId}`,
        )
      }
      ...
      const event = this.ledger.commit({
        ...
        provenance: 'observed',
        source: this.source,
        ...causal(this.parents.get(`turn:${turnId}`)),
    ...
    if ((notification.method === 'thread/compacted' || notification.method === 'thread/compaction/completed')) {
      const event = this.ledger.commit({
        ...
        source: this.source,
        ...causal(this.parents.get(`session:${sessionId}`)),
        data: { phase: 'compacted', history: 'unknown' },
      })
      this.parents.set(`session:${sessionId}`, event.eventId)
      return
    }
    if (notification.method.includes('/delta') && turnId !== undefined && itemId !== undefined) {
      if (!this.items.has(itemId)) {
        return this.diagnostic(
          sessionId,
          'missing-item-start',
          `${notification.method} arrived before item/started for ${itemId}`,
        )
      }
      const channel = deltaChannel(notification.method)
      const delta = string(params.delta)
      const ref = string(params.ref)
      if ((delta === undefined) === (ref === undefined)) {
        return this.diagnostic(sessionId, 'invalid-content-delta', `${notification.method} has no unique delta or ref`)
      }
      ...
      const event = this.ledger.commit({
        ...
        data: {
          channel,
          index,
          ...(delta === undefined ? { ref: ref! } : { delta }),
          ...(params.final === true ? { final: true } : {}),
        },
      })
  ...
  private diagnostic(sessionId: string, code: string, message: string): void {
    this.ledger.commit({
      ...
      source: this.source,
      ...causal(this.parents.get(`session:${sessionId}`)),
      data: { code, message, status: 'experimental' },
    })
  }
}
