...
const ENTITY_FILE_SCHEMA = 'https://raw.githubusercontent.com/cordisx/cordisx-protocol/main/schemas/entity-file.v1.schema.json' as const
const SNAPSHOT_SCHEMA = 'https://raw.githubusercontent.com/cordisx/cordisx-protocol/main/schemas/entity-registry-snapshot.v1.schema.json' as const
const TEMPLATE_RESULT_SCHEMA = 'https://raw.githubusercontent.com/cordisx/cordisx-protocol/main/schemas/entity-template-materialization-result.v1.schema.json' as const
...
export async function readEntityTemplatePayload(
  ...
  const load = async (relative: string): Promise<string> => {
    ...
    if (!resolved.startsWith(canonical) || !metadata.isFile() || metadata.isSymbolicLink()) throw new Error('entity template file escapes package root')
    return await readUtf8(resolved)
  }
  if (!TEMPLATE_PATH.test(declaration.entityPath)
    || !ENTITY_ID.test(declaration.agentId) || !/^sha256:[a-f0-9]{64}$/u.test(declaration.digest)) throw new Error('entity template declaration is invalid')
  ...
  const promptFiles = await Promise.all(referencedPromptPaths(entity).map(async promptPath => ({
    path: promptPath,
    text: await load(`${templateDirectory}/${promptPath.slice(2)}`),
  })))
  if (entityTreeDigest(entityText, promptFiles) !== declaration.digest) throw new Error('entity template digest mismatch')
...
function ownerKey(owner: EntityOwnerScope): string { return JSON.stringify([owner.profileId, owner.installationId, owner.pluginId]) }
function sameOwner(left: EntityOwnerScope, right: EntityOwnerScope): boolean { return ownerKey(left) === ownerKey(right) }
...
function u64(value: number): Buffer { const result = Buffer.allocUnsafe(8); result.writeBigUInt64BE(BigInt(value)); return result }
...
function referencedPromptPaths(entity: EntityFile): readonly EntityPromptFile['path'][] {
  const paths = (entity.promptSections ?? []).flatMap(section => section.source.kind === 'markdown' ? [section.source.path] : [])
...
function parseEntityFile(text: string): EntityFile {
  ...
  exactKeys(source, ['$schema', 'contract', 'schemaVersion', 'agentId', 'name', 'description', 'avatar', 'extends', 'inherit', 'promptSections', 'rules', 'skills', 'tools', 'mcpServers', 'runtimeDefaults'], 'entity')
  if (source.$schema !== ENTITY_FILE_SCHEMA || source.contract !== 'cordisx.entity-file/v1' || source.schemaVersion !== 1
    || typeof source.agentId !== 'string' || !ENTITY_ID.test(source.agentId)) throw new Error('entity contract or agentId is invalid')
  if (source.name !== undefined && (typeof source.name !== 'string' || source.name.length < 1 || source.name.length > 256)) throw new Error('entity name is invalid')
  if (source.description !== undefined && (typeof source.description !== 'string' || source.description.length > 2_000)) throw new Error('entity description is invalid')
  const inherit = record(source.inherit, 'entity.inherit')
  exactKeys(inherit, ['promptSections', 'rules', 'skills', 'tools', 'mcpServers', 'runtimeDefaults', 'avatar'], 'entity.inherit')
  for (const key of ['promptSections', 'rules', 'skills'] as const) {
    if (!['none', 'replace', 'append', 'prepend', 'merge'].includes(String(inherit[key]))) throw new Error(`entity.inherit.${key} is invalid`)
  }
  for (const key of ['tools', 'mcpServers', 'runtimeDefaults'] as const) {
    if (!['none', 'replace', 'append', 'prepend', 'merge'].includes(String(inherit[key]))) throw new Error(`entity.inherit.${key} is invalid`)
  }
  if (inherit.avatar !== undefined && inherit.avatar !== 'inherit' && inherit.avatar !== 'none') throw new Error('entity.inherit.avatar is invalid')
  if (source.extends !== undefined) {
    ...
    const keys = source.extends.map((item, index) => {
      const identity = record(item, `entity.extends[${index}]`); exactKeys(identity, ['agentId', 'revision'], `entity.extends[${index}]`)
      if (typeof identity.agentId !== 'string' || !ENTITY_ID.test(identity.agentId) || typeof identity.revision !== 'string' || identity.revision === '') throw new Error('entity parent identity is invalid')
  ...
  if (source.promptSections !== undefined) {
    if (!Array.isArray(source.promptSections) || source.promptSections.length > MAX_PROMPTS) throw new Error('entity.promptSections is invalid')
    const ids = source.promptSections.map((item, index) => {
      const section = record(item, `entity.promptSections[${index}]`); exactKeys(section, ['sectionId', 'kind', 'source'], `entity.promptSections[${index}]`)
      if (typeof section.sectionId !== 'string' || !ENTITY_ID.test(section.sectionId)) throw new Error('entity prompt section id is invalid')
      if (!['introduction', 'personality', 'role', 'operations', 'tools', 'knowledge', 'memory-policy', 'memory', 'other'].includes(String(section.kind))) throw new Error('entity prompt kind is invalid')
      ...
      if (promptSource.kind === 'inline') {
        ...
        if (typeof promptSource.text !== 'string' || promptSource.text.trim() === '') throw new Error('inline prompt is invalid')
      } else if (promptSource.kind === 'markdown') {
        exactKeys(promptSource, ['kind', 'path'], `entity.promptSections[${index}].source`)
        if (typeof promptSource.path !== 'string' || !MARKDOWN_PATH.test(promptSource.path) || promptSource.path.includes('..')) throw new Error('markdown prompt path is invalid')
...
function compile(entity: EntityFile, digest: EntityDigest, promptFiles: readonly EntityPromptFile[]): EntityDefinitionResolution {
...
export class EntityDirectoryAuthority {
  ...
  register(binding: EntityDirectoryBinding, declarations: readonly EntityTemplateDeclaration[]): void {
    ...
    for (const declaration of declarations) {
      if (!ENTITY_ID.test(declaration.agentId) || !TEMPLATE_PATH.test(declaration.entityPath)
        || declaration.entityPath !== `./entities/${declaration.agentId}/entity.json`
        || !/^sha256:[a-f0-9]{64}$/u.test(declaration.digest) || byId.has(declaration.agentId)) throw new Error('entity template declaration is invalid')
      byId.set(declaration.agentId, immutable(declaration))
    }
    this.#declarations.set(ownerKey(binding), byId)
  }

  async materialize(binding: EntityDirectoryBinding, packageVersion: string, packageDigest: EntityDigest, templates: readonly EntityTemplatePayload[]): Promise<readonly EntityTemplateMaterializationResult[]> {
    return await this.serial(async () => {
      this.assertBinding(binding)
      const owner = this.owner(binding)
      const output: EntityTemplateMaterializationResult[] = []
      for (const template of templates) {
        const envelope = { $schema: TEMPLATE_RESULT_SCHEMA, contract: 'cordisx.entity-template-materialization-result/v1' as const, schemaVersion: 1 as const, owner, packageVersion, packageDigest, agentId: template.declaration.agentId }
        ...
        if (expected === undefined || JSON.stringify(expected) !== JSON.stringify(template.declaration)) {
          output.push({ ...envelope, status: 'rejected', code: 'invalid-template' }); continue
        }
        ...
        if (existing) {
          ...
          if (!metadata.isDirectory() || metadata.isSymbolicLink()) {
            output.push({ ...envelope, status: 'rejected', code: 'symlink-escape' }); continue
          }
          ...
          if (existingOwner !== undefined && !sameOwner(existingOwner, owner)) {
            output.push({ ...envelope, status: 'rejected', code: 'ownership-conflict' }); continue
          }
          if (existingOwner === undefined) {
            if (Object.keys(owners).length >= MAX_ENTITIES) {
              output.push({ ...envelope, status: 'rejected', code: 'quota-authorization-required' }); continue
            }
        ...
        try { source = this.validateSource(template.entityText, template.promptFiles, template.declaration.agentId) }
        catch { output.push({ ...envelope, status: 'rejected', code: 'invalid-template' }); continue }
        if (source.digest !== template.declaration.digest) { output.push({ ...envelope, status: 'rejected', code: 'template-digest-mismatch' }); continue }
        const owners = await this.readOwners()
        if (owners[template.declaration.agentId] !== undefined && !sameOwner(owners[template.declaration.agentId]!, owner)) { output.push({ ...envelope, status: 'rejected', code: 'ownership-conflict' }); continue }
        if (Object.keys(owners).length >= MAX_ENTITIES) { output.push({ ...envelope, status: 'rejected', code: 'quota-authorization-required' }); continue }
        try {
          if (owners[template.declaration.agentId] === undefined) await this.writeOwners({ ...owners, [template.declaration.agentId]: owner })
  ...
  async snapshot(binding: EntityDirectoryBinding): Promise<EntityRegistrySnapshot> {
    return await this.serial(async () => {
      this.assertBinding(binding); await this.refresh()
      const owner = this.owner(binding)
      const entities = [...this.#states.values()].flatMap(state => state.record !== undefined && sameOwner(state.record.owner, owner) ? [state.record] : [])
        .sort((left, right) => left.identity.agentId.localeCompare(right.identity.agentId))
      return immutable({ $schema: SNAPSHOT_SCHEMA, contract: 'cordisx.entity-registry-snapshot/v1', schemaVersion: 1, binding, registryRevision: this.#revision, entities })
    })
  }
  ...
  async get(binding: EntityDirectoryBinding, identity: AgentDefinitionIdentity): Promise<EntityRecord | undefined> {
    return await this.serial(async () => {
      ...
      return record !== undefined && sameOwner(record.owner, this.owner(binding)) && record.identity.revision === identity.revision ? immutable(record) : undefined
    })
  }
  ...
  async save(binding: EntityDirectoryBinding, request: EntitySaveRequest): Promise<EntitySaveResult> {
    return await this.serial(async () => {
      ...
      const owner = this.owner(binding)
      let source: ReturnType<typeof this.validateSource>
      try { source = this.validateSource(`${JSON.stringify(request.entity, null, 2)}\n`, request.promptFiles, request.entity.agentId) }
      catch (error) {
        const message = error instanceof Error ? error.message : ''
        const code = message.includes('prompt path') ? 'invalid-prompt-path' : message.includes('missing prompt') ? 'missing-prompt-file' : message.includes('unexpected prompt') ? 'unexpected-prompt-file' : message.includes('duplicate prompt') ? 'duplicate-prompt-file' : 'invalid-entity'
        return { status: 'rejected', code }
      }
      if (this.#declarations.get(ownerKey(owner))?.has(request.entity.agentId) !== true) return { status: 'rejected', code: 'entity-not-declared' }
      ...
      if (prior !== undefined) return prior.fingerprint === fingerprint
        ? prior.result.status === 'applied' ? immutable({ ...prior.result, disposition: 'replayed' }) : immutable(prior.result)
      ...
      const result = await (async (): Promise<EntitySaveResult> => {
        if (request.expectedRevision === null && current !== undefined) return { status: 'conflict', code: 'entity-exists', currentRevision: current.digest }
        if (request.expectedRevision !== null && (current === undefined || !sameOwner(current.owner, owner) || current.identity.revision !== request.expectedRevision)) {
          return { status: 'conflict', code: 'revision-conflict', ...(current === undefined ? {} : { currentRevision: current.digest }) }
        }
        const owners = await this.readOwners()
        if (current === undefined && Object.keys(owners).length >= MAX_ENTITIES) return { status: 'rejected', code: 'quota-authorization-required' }
        if (owners[request.entity.agentId] !== undefined && !sameOwner(owners[request.entity.agentId]!, owner)) return { status: 'rejected', code: 'sharing-authorization-required' }
        try {
          ...
          try { await this.publishTree(request.entity.agentId, text, request.promptFiles, current !== undefined) }
          catch (error) {
        ...
        } catch { return { status: 'rejected', code: 'symlink-escape' } }
      })()
  ...
  async changes(binding: EntityDirectoryBinding, afterRevision: number, replayThrough: number): Promise<{ readonly revision: number; readonly changes: readonly EntityChange[] }> {
    return await this.serial(async () => {
      this.assertBinding(binding); await this.refresh()
      const owner = this.owner(binding)
      const upper = replayThrough > afterRevision ? Math.min(replayThrough, this.#revision) : this.#revision
      return immutable({ revision: this.#revision, changes: this.#changes.filter(change => {
        return change.sequence > afterRevision && change.sequence <= upper && sameOwner(changeOwner(change), owner)
      }) })
    })
  }

  private owner(binding: EntityDirectoryBinding): EntityOwnerScope { return { profileId: binding.profileId, installationId: binding.installationId, pluginId: binding.pluginId } }
  private assertBinding(binding: EntityDirectoryBinding): void {
    if (binding.profileId !== this.#profileId || !Number.isSafeInteger(binding.pluginGeneration) || binding.pluginGeneration < 1) throw new Error('entity registry binding is stale')
    opaqueId(binding.installationId, 'installationId'); localId(binding.pluginId, 'pluginId')
  }
  private entityDirectory(agentId: string): string { if (!ENTITY_ID.test(agentId)) throw new Error('agentId is invalid'); return path.join(this.#root, agentId) }
  private async pathExists(candidate: string): Promise<boolean> { try { await lstat(candidate); return true } catch (error) { if ((error as NodeJS.ErrnoException).code === 'ENOENT') return false; throw error } }
  private async serial<Value>(operation: () => Promise<Value>): Promise<Value> {
    const next = this.#queue.then(operation, operation); this.#queue = next.then(() => undefined, () => undefined); return await next
  }
  private validateSource(entityText: string, promptFiles: readonly EntityPromptFile[], expectedAgentId: string): { readonly resolution: EntityDefinitionResolution; readonly digest: EntityDigest } {
    const entity = parseEntityFile(entityText)
    ...
    for (const file of promptFiles) {
      ...
      if (typeof file.text !== 'string' || byteLength(file.text) > MAX_PROMPT_BYTES) throw new Error('prompt file is invalid')
  ...
  private entityRecord(owner: EntityOwnerScope, resolution: EntityDefinitionResolution, origin: EntityRecord['origin']): EntityRecord {
  ...
  private async ensureRoot(): Promise<void> {
    ...
    if (!homeMetadata.isDirectory() || homeMetadata.isSymbolicLink()) throw new Error('CordisX data root must be a real directory')
    for (const candidate of [path.join(this.#homeDir, 'profiles'), profileRoot, this.#root]) {
      ...
      if (!metadata.isDirectory() || metadata.isSymbolicLink()) throw new Error('entity directory ancestor must be a real directory')
  ...
  private async readOwners(): Promise<Record<string, EntityOwnerScope>> {
    ...
    try {
      ...
      if (!metadata.isFile() || metadata.isSymbolicLink()) throw new Error('entity ownership metadata must be a real file')
      ...
      if (source.contract !== 'cordisx.entity-directory-owners/v1' || Object.keys(owners).length > MAX_ENTITIES) throw new Error('entity ownership metadata is invalid')
      ...
      for (const [agentId, candidate] of Object.entries(owners)) {
        ...
        if (owner.profileId !== this.#profileId || typeof owner.installationId !== 'string' || typeof owner.pluginId !== 'string') throw new Error('entity ownership scope is invalid')
        validated[agentId] = {
  ...
  private async writeOwners(owners: Readonly<Record<string, EntityOwnerScope>>): Promise<void> {
    ...
    await syncedWrite(temporary, `${JSON.stringify({ contract: 'cordisx.entity-directory-owners/v1', owners }, null, 2)}\n`)
    await rename(temporary, this.#ownersFile)
  }
  private async readTree(agentId: string): Promise<{ readonly entityText: string; readonly prompts: readonly EntityPromptFile[] }> {
    await this.ensureRoot()
    ...
    if (!entityReal.startsWith(canonical) || !entityMetadata.isFile() || entityMetadata.isSymbolicLink()) throw new Error('symlink-escape')
    const entityText = await readUtf8(entityReal)
    const entity = parseEntityFile(entityText)
    const prompts = await Promise.all(referencedPromptPaths(entity).map(async promptPath => {
      const candidate = path.resolve(directory, promptPath.slice(2))
      const resolved = await realpath(candidate)
      const metadata = await lstat(resolved)
      if (!resolved.startsWith(canonical) || !metadata.isFile() || metadata.isSymbolicLink()) throw new Error('symlink-escape')
      return { path: promptPath, text: await readUtf8(resolved) }
    }))
    return { entityText, prompts }
  }
  private async publishTree(agentId: string, entityText: string, promptFiles: readonly EntityPromptFile[], replace: boolean): Promise<void> {
    await this.ensureRoot()
    ...
    try {
      ...
      try { await rename(candidate, target) } catch (error) { if (replace) await rename(backup, target).catch(() => undefined); throw error }
      if (replace) await rm(backup, { recursive: true, force: true })
    } finally { await rm(candidate, { recursive: true, force: true }); await rm(backup, { recursive: true, force: true }) }
  }
  private async refresh(): Promise<void> {
    ...
    for (const [agentId, owner] of Object.entries(owners)) {
      ...
      try {
        ...
        const message = error instanceof Error ? error.message : ''
        const invalid = message.includes('symlink') ? 'symlink-escape' : message.includes('missing prompt') || (error as NodeJS.ErrnoException).code === 'ENOENT' ? 'missing-prompt-file' : message.includes('entity') ? 'invalid-entity' : 'io-error'
        next.set(agentId, { invalid })
      }
    }
    const ids = new Set([...this.#states.keys(), ...next.keys()])
    for (const id of ids) {
      const before = this.#states.get(id); const after = next.get(id); const owner = owners[id] ?? before?.record?.owner
      if (owner === undefined) continue
      if (after?.record !== undefined && before?.record === undefined) this.#changes.push({ kind: 'entity-added', sequence: ++this.#revision, entity: after.record })
      else if (after?.record !== undefined && before?.record !== undefined && after.record.digest !== before.record.digest) this.#changes.push({ kind: 'entity-updated', sequence: ++this.#revision, entity: after.record })
      else if (after === undefined && before?.record !== undefined) this.#changes.push({ kind: 'entity-removed', sequence: ++this.#revision, identity: before.record.identity, owner })
      else if (after?.invalid !== undefined && after.invalid !== before?.invalid) this.#changes.push({ kind: 'entity-invalidated', sequence: ++this.#revision, agentId: id, owner, code: after.invalid })
    }
    this.#states.clear(); for (const [id, state] of next) this.#states.set(id, state)
  }
}
