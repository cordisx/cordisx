...
export const ENTITY_OPERATIONS = new Set(['entity-snapshot', 'entity-get', 'entity-save', 'entity-subscribe', 'entity-read', 'entity-unsubscribe'])
...
function object(value: unknown): Record<string, unknown> {
  if (value === null || typeof value !== 'object' || Array.isArray(value)) throw new Error('entity request must be an object')
...
export interface EntityBridgeHandler {
  issue(identity: { readonly source: string; readonly pluginId: string }, moduleGeneration: string): EntityBridgePrincipalBinding
...
export function createEntityBridgeHandler(input: {
  ...
  const resolve = (request: EntityRequest): { readonly principal: OwnerDocumentPrincipal; readonly binding: EntityDirectoryBinding } => {
    const principal = verifyOwnerDocumentPrincipalToken(input.secret, request.token)
    const binding = request.binding
    if (principal === undefined || binding === undefined || principal.profileId !== input.profileId || principal.generation !== input.generation
      || !input.principalAllowed(principal) || binding.profileId !== principal.profileId || binding.pluginId !== principal.identity.pluginId
      || binding.installationId !== entityInstallationId(principal.profileId, principal.identity.pluginId)
      || binding.pluginGeneration !== entityPluginGeneration(principal.moduleGeneration)) throw new Error('entity principal is stale')
  ...
  return {
    issue(identity, moduleGeneration) {
      ...
      return {
        ...identity, moduleGeneration, installationId: entityInstallationId(input.profileId, identity.pluginId),
    ...
    async handle(value) {
      ...
      if (request.version !== 1 || typeof request.requestId !== 'string' || typeof request.token !== 'string' || !ENTITY_OPERATIONS.has(request.operation)) throw new Error('entity request envelope is invalid')
      if (request.operation === 'entity-unsubscribe') {
        ...
        const principal = verifyOwnerDocumentPrincipalToken(input.secret, request.token)
        if (subscription === undefined || principal === undefined || !input.principalAllowed(principal)
          || subscription.principalKey !== JSON.stringify([principal.identity.source, principal.identity.pluginId, principal.moduleGeneration])) throw new Error('entity subscription is stale')
        subscriptions.delete(request.subscriptionId)
        return { status: 'closed', code: 'unsubscribed' }
      }
      if (request.operation === 'entity-read') {
        if (typeof request.subscriptionId !== 'string' || !Number.isSafeInteger(request.afterRevision) || !Number.isSafeInteger(request.replayThrough)) throw new Error('entity read cursor is invalid')
        const subscription = subscriptions.get(request.subscriptionId)
        const principal = verifyOwnerDocumentPrincipalToken(input.secret, request.token)
        if (subscription === undefined || principal === undefined || !input.principalAllowed(principal)
          || subscription.principalKey !== JSON.stringify([principal.identity.source, principal.identity.pluginId, principal.moduleGeneration])) throw new Error('entity subscription is stale')
      ...
      if (request.operation === 'entity-subscribe') {
        if (!Number.isSafeInteger(request.afterRevision) || request.afterRevision! < 0) throw new Error('entity subscription cursor is invalid')
        const snapshot = await input.authority.snapshot(binding)
        const subscriptionId = `cx-entity-subscription.${randomUUID()}`
        subscriptions.set(subscriptionId, {
          principalKey: JSON.stringify([principal.identity.source, principal.identity.pluginId, principal.moduleGeneration]), binding, replayThrough: snapshot.registryRevision,
        })
        return {
          subscriptionId, binding: structuredClone(binding), afterRevision: request.afterRevision!, replayThrough: snapshot.registryRevision,
        }
      }
...
export function entityBridgeUnavailable(binding: EntityRegistryBinding, subscriptionId: string): EntitySubscriptionClosed {
  return {
    $schema: 'https://raw.githubusercontent.com/cordisx/cordisx-protocol/main/schemas/entity-registry-subscription-close.v1.schema.json',
    contract: 'cordisx.entity-registry-subscription-close/v1', schemaVersion: 1,
    subscriptionId, binding, status: 'closed', code: 'connection-replaced',
  }
}
