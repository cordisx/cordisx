...
export async function buildRendererCompositionSource(
  ...
  const [version, readmes, pluginBundles] = await Promise.all([
    ...
    Promise.all(enabled.map(async plugin => {
      ...
      if (plugin.runtimeGraph !== undefined && !usesIsolatedWorker(plugin)) return {
        source: '',
  ...
  const runtimeGraphs = enabled.flatMap(plugin => plugin.runtimeGraph === undefined || usesIsolatedWorker(plugin)
    ? []
    : [plugin.runtimeGraph])
  const graphLoads = enabled.map(plugin => plugin.runtimeGraph === undefined || usesIsolatedWorker(plugin)
