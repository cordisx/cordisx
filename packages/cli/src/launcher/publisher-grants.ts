...
export class StaticPublisherKeyRegistry implements PublisherKeyRegistry {
  ...
  constructor(entries: readonly PublisherRegisteredKey[]) {
    for (const entry of entries) {
      ...
      this.keys.set(
        key,
        createPublicKey({ key: Buffer.from(entry.publicKeySpki, 'base64url'), format: 'der', type: 'spki' }),
      )
    }
  }
  async resolve(input: PublisherIssuer): Promise<KeyObject | undefined> {
    return this.keys.get(`${input.environment}:${input.id}:${input.keyId}`)
  }
}
...
export class MacOSMachineIdentityProvider implements DeviceKeyProvider {
  ...
  async current(): Promise<DeviceKeyIdentity | undefined> {
    ...
    try {
      ...
      try {
        await this.backend.upsert(MACHINE_IDENTITY_SERVICE, MACHINE_IDENTITY_ACCOUNT, serialized)
      } catch {
        return undefined
      }
    }
...
export async function createDeviceChallenge(provider: DeviceKeyProvider, now = new Date()): Promise<DeviceChallenge> {
  ...
  const unsigned = {
    ...
    devicePublicKeyHash: digest,
    nonce: randomBytes(32).toString('base64url'),
    issuedAt: now.toISOString(),
  }
  return Object.freeze({
    ...unsigned,
    proof: Buffer.from(await device.sign(Buffer.from(JSON.stringify(unsigned), 'utf8'))).toString('base64url'),
  })
}
...
interface DirectGrantState {
  readonly contract: typeof DIRECT_STATE_CONTRACT
  readonly revision: number
  readonly lastTrustedAt?: string
  readonly statements: Readonly<Record<string, string>>
  readonly grants: Readonly<Record<string, DirectGrantRecord>>
}
export type DirectPublisherGrantStatus =
  | 'authorized'
  | 'refresh-due'
  | 'grace'
  | 'expired'
...
function grantRecordKey(statement: PublisherGrantStatement, grantId: string): string {
  return `${statement.issuer.environment}:${statement.issuer.id}:${grantId}`
}
function directInitial(): DirectGrantState {
  return { contract: DIRECT_STATE_CONTRACT, revision: 0, statements: {}, grants: {} }
}
...
export class DirectPublisherGrantStore implements TrustedTimeStore {
  ...
  private constructor(readonly homeDir: string) {
    this.file = path.join(homeDir, 'state', 'publisher-grants', 'direct-device-bound.v1.json')
  }
  static async open(homeDir: string): Promise<DirectPublisherGrantStore> {
    ...
    try {
      ...
      await store.writeState(directInitial())
    }
    return store
  }
  async read(): Promise<TrustedTimeState> {
    const state = await this.readState()
    return state.lastTrustedAt === undefined ? {} : { lastTrustedAt: state.lastTrustedAt }
  }
  async write(value: TrustedTimeState): Promise<void> {
    await this.update(state => ({
      ...state,
      ...(value.lastTrustedAt === undefined ? {} : { lastTrustedAt: value.lastTrustedAt }),
    }))
  }
  async import(statement: PublisherGrantStatement): Promise<void> {
    ...
    await this.update(state => {
      ...
      if (known !== undefined && known !== fingerprint) {
        throw new Error('PublisherGrant statementId replay differs from the accepted statement')
      }
      const statements = { ...state.statements, [statement.statementId]: fingerprint }
      if (statement.kind === 'transfer') {
        throw new Error('transfer requires a publisher-signed grant for the new device')
      }
      if (statement.kind === 'revoke') {
        ...
        return {
          ...
          ...(lastTrustedAt === undefined ? {} : { lastTrustedAt }),
          statements,
          grants: current === undefined
            ? state.grants
            : { ...state.grants, [key]: { ...current, revokedAt: payload.effectiveAt } },
        }
      }
      ...
      return {
        ...
        ...(lastTrustedAt === undefined ? {} : { lastTrustedAt }),
        statements,
        grants: { ...state.grants, [key]: { statement, importedAt: new Date().toISOString() } },
      }
    })
  }
  async records(): Promise<readonly DirectGrantRecord[]> {
    return Object.values((await this.readState()).grants)
  }
  private async readState(): Promise<DirectGrantState> {
    ...
    if (
      value.contract !== DIRECT_STATE_CONTRACT || !Number.isInteger(value.revision) || value.statements === null
      || value.grants === null
    ) throw new Error('PublisherGrant state is invalid')
    return value
  }
...
export class DirectPublisherGrantAuthority {
  constructor(
    private readonly keys: PublisherKeyRegistry,
    private readonly device: DeviceKeyProvider,
    private readonly store: DirectPublisherGrantStore,
  ) {}
  ...
  async import(value: unknown): Promise<DirectPublisherGrantProjection> {
    ...
    if (statement.kind === 'grant' || statement.kind === 'renew') {
      ...
      if (
        current === undefined || devicePublicKeyHash(current.publicKey) !== grantPayload(statement).devicePublicKeyHash
      ) return { status: 'device-mismatch', features: [] }
    }
    await this.store.import(statement)
    if (statement.kind === 'revoke') {
      return { status: 'revoked', grantId: (statement.payload as { readonly grantId: string }).grantId, features: [] }
    }
    ...
    return evaluated.state === 'active'
      ? {
        ...
        refreshAfter: grant.refreshAfter,
      }
      ...
      : {
        status: evaluated.state === 'expired' ? 'expired' : 'not-yet-valid',
        grantId: grant.grantId,
        features: [],
        expiresAt: grant.expiresAt,
        refreshAfter: grant.refreshAfter,
  ...
  async status(target: PublisherGrantTarget, now = new Date()): Promise<DirectPublisherGrantProjection> {
    ...
    const candidates = records.filter(record => record.statement.kind === 'grant' || record.statement.kind === 'renew')
      ...
      .filter(({ grant }) =>
        grant.pluginId === target.pluginId && publisherGrantVersionMatches(grant.versionRange, target.version)
      )
    ...
    if (candidate.grant.devicePublicKeyHash !== digest) {
      return { status: 'device-mismatch', grantId: candidate.grant.grantId, features: [] }
    }
    const effectiveNow = evaluatePublisherGrantTime(candidate.grant, state, now)
    const features = target.requestedFeatures === undefined
      ? candidate.grant.features
      : target.requestedFeatures.filter(feature => candidate.grant.features.includes(feature))
    if (
      candidate.record.revokedAt !== undefined
      && Date.parse(candidate.record.revokedAt) <= Date.parse(effectiveNow.effectiveNow ?? now.toISOString())
    ) {
      return {
        status: 'revoked',
        grantId: candidate.grant.grantId,
        features: [],
        expiresAt: candidate.grant.expiresAt,
        refreshAfter: candidate.grant.refreshAfter,
      }
    }
    if (effectiveNow.state === 'active') {
      return {
        status: effectiveNow.refreshDue ? 'refresh-due' : 'authorized',
        grantId: candidate.grant.grantId,
        features,
        expiresAt: candidate.grant.expiresAt,
        refreshAfter: candidate.grant.refreshAfter,
    ...
    return {
      status: effectiveNow.state === 'expired' ? 'expired' : 'not-yet-valid',
      grantId: candidate.grant.grantId,
      features: [],
      expiresAt: candidate.grant.expiresAt,
      refreshAfter: candidate.grant.refreshAfter,
...
function time(value: unknown, label: string): string {
  ...
  if (!Number.isFinite(Date.parse(result))) throw new Error(`${label} must be an RFC 3339 timestamp`)
  return result
}
...
function authorization(value: unknown): PublisherGrantAuthorization {
  ...
  exact(input, [
    ...
    'versionRange',
    'features',
  ], 'PublisherGrant authorization')
  const features = input.features
  if (
    !Array.isArray(features) || features.some(item => typeof item !== 'string')
    || new Set(features).size !== features.length
  ) throw new Error('PublisherGrant features are invalid')
  if (!Number.isInteger(input.offlineGraceSeconds)) throw new Error('offlineGraceSeconds is invalid')
  const result = {
    grantId: string(input.grantId, 'grantId', /^[A-Za-z0-9._-]{16,160}$/),
    pluginId: string(input.pluginId, 'pluginId', /^[a-z0-9][a-z0-9._-]{0,95}$/),
    offerId: string(input.offerId, 'offerId', /^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$/),
    devicePublicKeyHash: string(input.devicePublicKeyHash, 'devicePublicKeyHash', /^sha256:[a-f0-9]{64}$/),
    nonce: string(input.nonce, 'nonce', /^[A-Za-z0-9_-]{32,128}$/),
    notBefore: time(input.notBefore, 'notBefore'),
    expiresAt: time(input.expiresAt, 'expiresAt'),
    refreshAfter: time(input.refreshAfter, 'refreshAfter'),
    offlineGraceSeconds: input.offlineGraceSeconds as number,
    versionRange: string(input.versionRange, 'versionRange'),
    features: Object.freeze([...features]),
  }
  if (
    !Number.isInteger(result.offlineGraceSeconds) || result.offlineGraceSeconds < 0
    || result.offlineGraceSeconds > 2_592_000
  ) throw new Error('offlineGraceSeconds is invalid')
  const start = Date.parse(result.notBefore)
  const refresh = Date.parse(result.refreshAfter)
  const expiry = Date.parse(result.expiresAt)
  if (!(start < expiry && start <= refresh && refresh < expiry)) {
    throw new Error('PublisherGrant authorization timing is invalid')
  }
...
export function parsePublisherGrantStatement(value: unknown): PublisherGrantStatement {
  ...
  exact(
    input,
    ['$schema', 'schemaVersion', 'kind', 'issuer', 'statementId', 'issuedAt', 'payload', 'signature'],
    'PublisherGrant statement',
  )
  if (input.$schema !== CORDISX_PUBLISHER_GRANT_SCHEMA_V1 || input.schemaVersion !== 1) {
    throw new Error('PublisherGrant schema is unsupported')
  }
  const kind = string(input.kind, 'kind')
  if (!(['grant', 'renew', 'revoke', 'transfer'] as const).includes(kind as StatementKind)) {
    throw new Error('PublisherGrant kind is unsupported')
  }
  const issuerInput = record(input.issuer, 'issuer')
  exact(issuerInput, ['id', 'keyId', 'environment'], 'issuer')
  ...
  const issuer: PublisherIssuer = Object.freeze({
    id: string(issuerInput.id, 'issuer.id', /^[a-z0-9][a-z0-9._-]{0,127}$/),
    keyId: string(issuerInput.keyId, 'issuer.keyId', /^[A-Za-z0-9._-]{1,128}$/),
    environment,
  })
  const signatureInput = record(input.signature, 'signature')
  exact(signatureInput, ['algorithm', 'value'], 'signature')
  if (signatureInput.algorithm !== 'Ed25519') throw new Error('PublisherGrant signature algorithm is unsupported')
  const signature = Object.freeze({
    algorithm: 'Ed25519' as const,
    value: string(signatureInput.value, 'signature.value', /^[A-Za-z0-9_-]{86}$/),
  })
  const base = {
    ...
    issuer,
    statementId: string(input.statementId, 'statementId', /^[A-Za-z0-9._-]{16,128}$/),
    issuedAt: time(input.issuedAt, 'issuedAt'),
  }
  if (kind === 'grant' || kind === 'renew') {
    return Object.freeze({ ...base, payload: authorization(input.payload), signature })
  }
  const payload = record(input.payload, `${kind} payload`)
  if (kind === 'revoke') {
    exact(payload, ['grantId', 'effectiveAt', 'reason'], 'revoke payload')
    const effectiveAt = time(payload.effectiveAt, 'effectiveAt')
    if (Date.parse(effectiveAt) < Date.parse(base.issuedAt)) throw new Error('revoke effectiveAt predates issuedAt')
    return Object.freeze({
      ...base,
      payload: Object.freeze({
        grantId: string(payload.grantId, 'grantId', /^[A-Za-z0-9._-]{16,160}$/),
        effectiveAt,
        ...(payload.reason === undefined ? {} : { reason: string(payload.reason, 'reason') }),
      }),
      signature,
    })
  }
  exact(
    payload,
    ['grantId', 'fromDevicePublicKeyHash', 'toDevicePublicKeyHash', 'nonce', 'notBefore', 'expiresAt'],
    'transfer payload',
  )
  const transfer = Object.freeze({
    grantId: string(payload.grantId, 'grantId', /^[A-Za-z0-9._-]{16,160}$/),
    fromDevicePublicKeyHash: string(
      payload.fromDevicePublicKeyHash,
      'fromDevicePublicKeyHash',
      /^sha256:[a-f0-9]{64}$/,
    ),
    toDevicePublicKeyHash: string(payload.toDevicePublicKeyHash, 'toDevicePublicKeyHash', /^sha256:[a-f0-9]{64}$/),
    nonce: string(payload.nonce, 'nonce', /^[A-Za-z0-9_-]{32,128}$/),
    notBefore: time(payload.notBefore, 'notBefore'),
    expiresAt: time(payload.expiresAt, 'expiresAt'),
  })
  if (
    transfer.fromDevicePublicKeyHash === transfer.toDevicePublicKeyHash
    || Date.parse(transfer.notBefore) >= Date.parse(transfer.expiresAt)
  ) throw new Error('transfer is invalid')
  return Object.freeze({ ...base, payload: transfer, signature })
}

export async function verifyPublisherGrantStatement(
  value: unknown,
  registry: PublisherKeyRegistry,
): Promise<PublisherGrantStatement> {
  const statement = parsePublisherGrantStatement(value)
  const key = await registry.resolve(statement.issuer)
  ...
  if (
    !verify(
      null,
      canonicalPublisherGrantSigningInput(unsigned),
      key,
      Buffer.from(statement.signature.value, 'base64url'),
    )
  ) throw new Error('PublisherGrant signature is invalid')
  return statement
}
export function devicePublicKeyHash(publicKey: Uint8Array): string {
  return `sha256:${createHash('sha256').update(publicKey).digest('hex')}`
}
function grantPayload(statement: PublisherGrantStatement): PublisherGrantAuthorization {
  if (statement.kind !== 'grant' && statement.kind !== 'renew') throw new Error('statement does not authorize a plugin')
  return statement.payload as PublisherGrantAuthorization
}
export async function buildPublisherGrantActivationRequest(
  statement: PublisherGrantStatement,
  deviceProvider: DeviceKeyProvider,
): Promise<PublisherGrantActivationRequest> {
  const grant = grantPayload(statement)
  const device = await deviceProvider.current()
  if (device === undefined) throw new Error('a secure device key is unavailable')
  const hash = devicePublicKeyHash(device.publicKey)
  if (hash !== grant.devicePublicKeyHash) throw new Error('PublisherGrant belongs to a different device key')
  ...
  return Object.freeze({
    ...
    devicePublicKey: Buffer.from(device.publicKey).toString('base64url'),
    nonce: grant.nonce,
    proof: Buffer.from(await device.sign(proofInput)).toString('base64url'),
    idempotencyKey,
  })
}
export function evaluatePublisherGrantTime(
  grant: PublisherGrantAuthorization,
  state: TrustedTimeState,
  now = new Date(),
): PublisherGrantDecision {
  const systemNow = now.getTime()
  const trusted = state.lastTrustedAt === undefined ? undefined : Date.parse(state.lastTrustedAt)
  if (trusted !== undefined && !Number.isFinite(trusted)) {
    return { state: 'invalid', reason: 'stored trusted time is invalid' }
  }
  const effective = Math.max(systemNow, trusted ?? Number.NEGATIVE_INFINITY)
  const notBefore = Date.parse(grant.notBefore)
  const expiresAt = Date.parse(grant.expiresAt)
  if (effective < notBefore) {
    return { state: 'not-yet-valid', effectiveNow: new Date(effective).toISOString(), reason: 'grant is not yet valid' }
  }
  const clockRollbackDetected = trusted !== undefined && systemNow < trusted
  if (effective <= expiresAt) {
    return {
      state: 'active',
      effectiveNow: new Date(effective).toISOString(),
      refreshDue: effective >= Date.parse(grant.refreshAfter),
      clockRollbackDetected,
    }
  }
  if (effective <= expiresAt + grant.offlineGraceSeconds * 1000) {
    return { state: 'grace', effectiveNow: new Date(effective).toISOString(), refreshDue: true, clockRollbackDetected }
  }
  return {
    state: 'expired',
    effectiveNow: new Date(effective).toISOString(),
    reason: 'grant expired beyond offline grace',
  }
}
...
export function publisherGrantVersionMatches(range: string, version: string): boolean {
  ...
  const clauses = range.trim().split(/\s+/u)
  if (clauses.length === 0) return false
  return clauses.every(clause => {
    const match = /^(>=|>|<=|<|=)?(.+)$/.exec(clause)
    const bound = match === null ? undefined : versionTuple(match[2]!)
    if (match === null || bound === undefined) return false
    const result = compareVersion(candidate, bound)
...
export class PublisherGrantLifecycleGate {
  constructor(
    private readonly keys: PublisherKeyRegistry,
    private readonly device: DeviceKeyProvider,
    private readonly trustedTime: TrustedTimeStore,
    private readonly registry?: PublisherGrantActivationRegistry,
  ) {}
  async activate(
    value: unknown,
    target: PublisherGrantTarget,
    now = new Date(),
  ): Promise<
    { readonly state: 'activated' | 'grace' | 'rejected' | 'unavailable'; readonly features: readonly string[] }
  > {
    let statement: PublisherGrantStatement
    try {
      statement = await verifyPublisherGrantStatement(value, this.keys)
    } catch {
      return { state: 'rejected', features: [] }
    }
    const grant = grantPayload(statement)
    if (grant.pluginId !== target.pluginId || !publisherGrantVersionMatches(grant.versionRange, target.version)) {
      return { state: 'rejected', features: [] }
    }
    const features = target.requestedFeatures === undefined
      ? grant.features
      : target.requestedFeatures.filter(feature => grant.features.includes(feature))
    const local = evaluatePublisherGrantTime(grant, await this.trustedTime.read(), now)
    if (local.state === 'expired' || local.state === 'not-yet-valid' || local.state === 'invalid') {
      return { state: 'rejected', features: [] }
    }
    if (this.registry === undefined) {
      ...
      return evaluated.state === 'active'
        ? { state: 'activated', features }
        : evaluated.state === 'grace'
        ? { state: 'grace', features }
        : { state: 'rejected', features: [] }
    }
    let response: PublisherGrantActivationResponse
    try {
      response = await this.registry.activate(await buildPublisherGrantActivationRequest(statement, this.device))
    } catch {
      return { state: 'unavailable', features: [] }
    }
    if (response.status !== 'activated' && response.status !== 'already-activated') {
      return { state: response.status === 'unavailable' ? 'unavailable' : 'rejected', features: [] }
    }
    if (response.trustedAt === undefined || !Number.isFinite(Date.parse(response.trustedAt))) {
      return { state: 'rejected', features: [] }
    }
    const before = await this.trustedTime.read()
    const prior = before.lastTrustedAt === undefined ? Number.NEGATIVE_INFINITY : Date.parse(before.lastTrustedAt)
    const received = Date.parse(response.trustedAt)
