...
export class PermissionBrokerChannelTaskAuthorizer implements ChannelTaskPermissionBroker {
  ...
  async authorize(
    input: Parameters<ChannelTaskPermissionBroker['authorize']>[0],
  ): Promise<{ readonly decision: 'allow' | 'ask' | 'deny'; readonly scopeFingerprint: `sha256:${string}` }> {
  ...
  private permits(
    record: CordisXPersistedPermissionPolicyRecord,
    input: Parameters<ChannelTaskPermissionBroker['authorize']>[0],
  ): boolean {
    const key = record.key
    if (
      key.profileId !== this.profileId || key.identity.source !== this.source || key.identity.pluginId !== 'channel'
...
interface LaunchGrant {
  ...
  readonly target: {
    readonly profileId: string
    readonly model: { readonly providerId: string; readonly modelId: string }
    readonly workspace: ChannelWorkspaceRegistration
  }
...
function failure(code: string, retryable: boolean) {
  return {
    code: code.replace(/[^A-Za-z0-9._-]/g, '_').toUpperCase().slice(0, 128) || 'CHANNEL_GATEWAY_REJECTED',
    retryable,
  }
}
...
export class LauncherChannelTaskGateway implements ChannelTaskGateway {
  ...
  private async create(
    ...
    if (workspace === undefined) {
      return { dispatch: rejected(context.operationId, 'create', 'CHANNEL_WORKSPACE_REJECTED') }
    }
    const first = await this.options.permissions.authorize({
      operationId: context.operationId,
      source: context.input.source.event,
    ...
    if (
      first.decision !== 'allow' || second.decision !== 'allow' || first.scopeFingerprint !== second.scopeFingerprint
    ) {
    ...
    const dispatched = await this.options.fleet.dispatchCreate({
      operationId: context.operationId,
      model: consumed.target.model,
      cwd: consumed.target.workspace.cwd,
      message: text(context.input) ?? '',
    })
    return {
  ...
  private async resolveModel(
    ...
    if (
      provider === undefined || selectedModel === undefined || 'useDefault' in provider || 'useDefault' in selectedModel
    ) return undefined
    if (
      operation.profile !== undefined
      && ('useDefault' in operation.profile || operation.profile.id !== this.options.profileId)
    ) return undefined
    const providerId = provider.id
    const modelId = selectedModel.id
    const models = await this.options.fleet.listModels({ providerIds: [providerId] })
    return models.ok
        && models.value.models.some(item => item.ref.providerId === providerId && item.ref.modelId === modelId)
      ? { providerId, modelId }
      : undefined
  }
  ...
  private async resolveWorkspace(alias: string): Promise<ChannelWorkspaceRegistration | undefined> {
    ...
    if (registered === undefined || !path.isAbsolute(registered.root) || !path.isAbsolute(registered.cwd)) {
      return undefined
    }
    try {
      const [root, cwd, info] = await Promise.all([
        realpath(registered.root),
        realpath(registered.cwd),
        stat(registered.cwd),
      ])
  ...
  private issue(
    ...
    const grant: LaunchGrant = {
      ...
      sourceKey: sourceKey(context.input.source.event),
      target: { profileId: this.options.profileId, model, workspace },
      scopeFingerprint,
      expiresAt: (this.options.now ?? Date.now)() + (this.options.grantTtlMs ?? 60_000),
      consumed: false,
    }
  ...
  private consume(token: string, context: ChannelTaskContext): LaunchGrant | undefined {
    ...
    if (
      grant === undefined || grant.consumed || grant.expiresAt <= (this.options.now ?? Date.now)()
      || grant.operationId !== context.operationId || grant.routeId !== context.routeId
      || grant.serviceGeneration !== context.serviceGeneration
      || grant.configurationRevision !== context.configurationRevision
      || grant.sourceKey !== sourceKey(context.input.source.event) || grant.target.profileId !== this.options.profileId
    ) return undefined
    grant.consumed = true
    return grant
  }
...
export function channelTaskScopeFingerprint(
  input: Omit<Parameters<ChannelTaskPermissionBroker['authorize']>[0], 'capability'>,
): `sha256:${string}` {
  const { operationId, source, profileId, model, cwd } = input
  return `sha256:${
    createHash('sha256').update(JSON.stringify({ operationId, source, profileId, model, cwd })).digest('hex')
  }`
}
