...
import type {
  CordisXAgentLoopAuthorizationRequest,
  CordisXAgentLoopHost,
  CordisXBoundAgentLoopClientOptions,
...
import {
  AllowedCreateAuthorization,
  AllowedSendAuthorization,
  canonicalAgentLoopTaskDetailsUrl,
  clone,
  Command,
  CordisXAgentLoopBrokerV2Persistence,
  CreateCommand,
  EventPayload,
  fingerprint,
  freeze,
  ...
  sameDefinition,
  SendCommand,
} from './agent-loop-v2-contract.js'
...
export class CordisXAgentLoopBrokerV2 {
  ...
  definitionPresentation(identity: { readonly agentId: string; readonly revision: string }): {
    ...
    const definition =
      [...this.records.values()].map(record => record.definition).find(item => sameDefinition(item.identity, identity))
        ?? [...this.ledger.values()].map(entry => entry.definition).find(item =>
          item !== undefined && sameDefinition(item.identity, identity)
        )
  ...
  bind(options: CordisXBoundAgentLoopClientOptions): BoundAgentLoopClient {
    ...
    const client = Object.freeze({
      ...
      createOrBind: (command: CreateCommand) => {
        ...
        return this.executeDurable<AgentLoopCreateOrBindResult>(
          options,
          command,
          () =>
            this.unavailable(
          ...
          async () => await this.createOrBind(options, owned, command, live),
        ).then(result => {
      ...
      send: (command: SendCommand) => {
        ...
        if (this.persistence === undefined) {
          return Promise.resolve(this.unavailable(command, 'turns.submit', 'reconciliation-required'))
        }
        return this.executeDurable<AgentLoopSendResult>(
          options,
          command,
      ...
      subscribe: async (binding: AgentLoopTaskBinding, afterSequence: number) => {
        ...
        if (scoped.state !== 'allowed') {
          return { status: scoped.state, authorization: scoped } as AgentLoopSubscribeRuntimeResult
        }
  ...
  private executeDurable<Result extends AgentLoopCreateOrBindResult | AgentLoopSendResult>(
    ...
    if (existing !== undefined) {
      ...
      if (command.type === 'create-or-bind') {
        return this.replayCreate(options, command, existing as LedgerEntry<AgentLoopCreateOrBindResult>) as Promise<
          Result
        >
      }
    ...
    const entry: LedgerEntry<Result> = {
  ...
  private async replayCreate(
    ...
    if (current !== undefined && current.binding.state === 'active') {
      ...
      const currentAuthorization = await options.authorize({
        capability: 'tasks.content.read',
    ...
    if (detailsUrl === undefined) {
      this.host.release?.(bound.value)
      return this.unavailable(command, capability, 'details-unavailable')
    }
    const record = this.record(
      options,
      entry.definition,
      { ...bound.value, detailsUrl },
      detailsUrl,
  ...
  private async createOrBind(
    ...
    try {
      definition = resolveAgentDefinition(command as unknown as Parameters<typeof resolveAgentDefinition>[0])
    } catch {
    ...
    if (command.target.mode === 'create') {
      ...
      const outcome = await options.authorize({
        capability,
      ...
      if (current !== undefined && current.binding.state === 'active') {
        ...
        if (outcome.state !== 'allowed') return this.refusalFrom(command, outcome)
        if (!sameDefinition(current.definition.identity, definition.identity)) {
          return this.refusal(command, capability, 'unavailable', 'unsupported')
        }
      ...
      if (outcome.state !== 'allowed') {
        this.host.release?.(bound.value)
        return this.refusalFrom(command, outcome)
      }
      authorization = { capability, state: 'allowed', code: 'allowed' }
      task = bound.value
    }
    ...
    const record = this.record(
      ...
      command.commandId,
      command.target.mode === 'create' ? 'binding.created' : 'binding.bound',
    )
  ...
  private async send(
    ...
    const acceptedAuthorization: AllowedSendAuthorization = {
    ...
    try {
      sent = await this.host.send(record.task, command.content)
    } catch {
      return this.unavailable(command, 'turns.submit', 'reconciliation-required')
    }
    if (!sent.ok) {
      return this.refusal(
        command,
  ...
  private refusalFrom<CommandType extends Command>(
    command: CommandType,
  ...
  private startPolling(record: RecordState): void {
    ...
    const poll = async () => {
      ...
      try {
        ...
        /* transient provider gaps remain retryable */
      } finally {
        record.polling = false
      }
      if (record.binding.state === 'active' && !this.disposed) {
  ...
  private subscription(record: RecordState, afterSequence: number): AgentLoopSubscription {
    ...
    const nextPage = (): AgentLoopEventPage | undefined => {
      ...
      const page = clone({
        ...
        contract: 'cordisx.agent-loop-event-page/v2' as const,
        schemaVersion: 2 as const,
  ...
  private close(record: RecordState | undefined): void {
    ...
    for (const entry of this.ledger.values()) {
      if (entry.task?.task === record.task.task && entry.ownerKey === record.ownerKey) {
  ...
  private restore(): void {
    try {
      ...
      if (
        state.version !== 1 || state.providerKey !== this.persistence?.providerKey
        || !Number.isSafeInteger(state.nextBinding) || (state.nextBinding ?? -1) < 0
        || !Array.isArray(state.taskGenerations) || !Array.isArray(state.ledger)
      ) return
      ...
      for (const persisted of state.ledger) {
        if (
          ...
          || persisted.result === null || typeof persisted.result !== 'object'
        ) return
