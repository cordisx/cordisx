...
function clone<Value>(value: Value): Value {
  const result = typeof globalThis.structuredClone === 'function' ? globalThis.structuredClone(value) : JSON.parse(JSON.stringify(value)) as Value
...
function handle(value: unknown): value is string { return typeof value === 'string' && value.length > 0 && value.length <= 512 }
function identifier(value: unknown): value is string { return typeof value === 'string' && /^[A-Za-z0-9][A-Za-z0-9._:-]{0,127}$/.test(value) }
function identityKey(value: CordisXAgentDefinitionIdentity): string { return JSON.stringify([value.agentId, value.revision]) }
function sameIdentity(left: CordisXAgentDefinitionIdentity, right: CordisXAgentDefinitionIdentity): boolean { return left.agentId === right.agentId && left.revision === right.revision }

function mergeArray<Value>(parent: readonly Value[] | undefined, child: readonly Value[] | undefined, mode: CordisXAgentInheritanceMode, key: (value: Value) => string): readonly Value[] | undefined {
  const unique = (values: readonly Value[] | undefined): readonly Value[] | undefined => {
    if (values === undefined) return undefined
    const identities = values.map(key)
    if (new Set(identities).size !== identities.length) throw new Error('Agent definition produces duplicate effective identities')
    return values
  }
  ...
  for (const value of values) {
    ...
    if (index === undefined) { indices.set(id, output.length); output.push(value) } else output[index] = value
  }
  return output
}

function mergeFilter(parent: CordisXAgentFilter | undefined, child: CordisXAgentFilter | undefined, mode: CordisXAgentInheritanceMode): CordisXAgentFilter | undefined {
  ...
  return {
    ...([...new Set([...(first.include ?? []), ...(second.include ?? [])])].length === 0 ? {} : { include: [...new Set([...(first.include ?? []), ...(second.include ?? [])])] }),
    ...([...new Set([...(first.exclude ?? []), ...(second.exclude ?? [])])].length === 0 ? {} : { exclude: [...new Set([...(first.exclude ?? []), ...(second.exclude ?? [])])] }),
  }
}

function mergeObject<Value extends object>(parent: Value | undefined, child: Value | undefined, mode: CordisXAgentInheritanceMode): Value | undefined {
  if (mode === 'none') return child
  if (mode === 'replace') return child ?? parent
  if (parent === undefined) return child
  if (child === undefined) return parent
...
function assertDefinition(definition: CordisXAgentDefinition): void {
  if (definition.$schema !== CORDISX_AGENT_DEFINITION_SCHEMA_V1 || definition.contract !== 'cordisx.agent-definition/v1' || definition.schemaVersion !== 1) throw new Error('Agent definition contract is invalid')
  if (!identifier(definition.identity.agentId) || !handle(definition.identity.revision)) throw new Error('Agent definition identity is invalid')
...
export function resolveAgentDefinition(command: CreateCommand): CordisXResolvedAgentDefinition {
  ...
  if (!catalog.has(identityKey(command.definition))) throw new Error('Target Agent definition is missing from the catalog')
...
function mergeResolved(parent: CordisXResolvedAgentDefinition, child: CordisXResolvedAgentDefinition, modes: CordisXAgentDefinition['inherit']): CordisXResolvedAgentDefinition {
  const promptSections = mergeArray(parent.promptSections, child.promptSections, modes.promptSections, value => value.sectionId)
  ...
  return {
    ...
    ...(promptSections === undefined ? {} : { promptSections }), ...(rules === undefined ? {} : { rules }),
    ...(skills === undefined ? {} : { skills }), ...(tools === undefined ? {} : { tools }),
    ...(mcpServers === undefined ? {} : { mcpServers }), ...(runtimeDefaults === undefined ? {} : { runtimeDefaults }),
    sourceDefinitions: [...new Map([...parent.sourceDefinitions, ...child.sourceDefinitions].map(identity => [identityKey(identity), identity])).values()],
  }
}

export function renderAgentDeveloperInstructions(definition: CordisXResolvedAgentDefinition): string | undefined {
  const sections = (definition.promptSections ?? []).map(section => `## ${section.kind}:${section.sectionId}\n\n${section.text.trim()}`)
  if ((definition.rules?.length ?? 0) > 0) sections.push(`## rules\n\n${definition.rules!.map(item => `- ${item}`).join('\n')}`)
  if ((definition.skills?.length ?? 0) > 0) sections.push(`## skills\n\n${definition.skills!.map(item => `- ${item}`).join('\n')}`)
...
export class BindingAgentLoopHost implements CordisXAgentLoopHost {
  ...
  async prepare(definition: CordisXResolvedAgentDefinition) {
    ...
    if (requested !== undefined) return { ok: true as const, value: { model: clone(requested), cwd: this.workspaceCwd } }
  ...
  async send(task: HostTask, content: readonly [CordisXAgentLoopContentPart, ...CordisXAgentLoopContentPart[]]) {
    ...
    if (message === '') return { ok: false as const, error: { code: 'invalid-request' as const, message: 'AgentLoop message has no text content' } }
    const turn = await this.platform.submitTurn({ session: task.session, message })
    return turn.ok ? { ok: true as const, value: { messageId: `cxloop-message:${crypto.randomUUID()}`, turn: turn.value.turnId } } : turn
  }
  async lifecycle(task: HostTask, afterSequence: number) { return await this.platform.readAgentLoopLifecycle(task.session, afterSequence) }
}
...
export class CordisXAgentLoopBroker {
  ...
  bind(options: CordisXBoundAgentLoopClientOptions): CordisXBoundAgentLoopClient {
    ...
    const client: CordisXBoundAgentLoopClient = Object.freeze({
      ...
      send: (command: SendCommand): Promise<CordisXAgentLoopSendResult> => idempotent(command,
        ...
        this.append(record, { type: 'message', turn: sent.value.turn, message: { messageId: sent.value.messageId, role: 'user', content: command.content } })
      ...
      subscribe: async (binding: CordisXAgentLoopTaskBinding, afterSequence: number): Promise<CordisXAgentLoopSubscribeRuntimeResult> => {
        ...
        if (subscriptionAuthorization.state !== 'allowed') return { status: subscriptionAuthorization.state, authorization: subscriptionAuthorization } as CordisXAgentLoopSubscribeRuntimeResult
        const handle = this.subscription(record, afterSequence)
        subscriptions.add(handle); this.startPolling(record)
  ...
  dispose(): void { if (this.disposed) return; this.disposed = true; for (const record of this.records.values()) this.close(record); this.records.clear(); this.boundTasks.clear() }

  private acceptCreate(command: CreateCommand, binding: CordisXAgentLoopTaskBinding): CordisXAgentLoopCreateOrBindResult { return clone({ $schema: CORDISX_AGENT_LOOP_RESULT_SCHEMA_V1, contract: 'cordisx.agent-loop-result/v1', schemaVersion: 1, commandId: command.commandId, type: 'create-or-bind', status: 'accepted', authorization: { capability: command.target.mode === 'create' ? 'tasks.create' : 'tasks.content.read', state: 'allowed', code: 'allowed' }, binding }) }
  private refusal<Command extends CreateCommand | SendCommand>(command: Command, capability: CordisXAgentLoopAuthorizationOutcome['capability'], state: 'denied' | 'unavailable', code: Refused['code']): Command extends CreateCommand ? CordisXAgentLoopCreateOrBindResult : CordisXAgentLoopSendResult { return clone({ $schema: CORDISX_AGENT_LOOP_RESULT_SCHEMA_V1, contract: 'cordisx.agent-loop-result/v1', schemaVersion: 1, commandId: command.commandId, type: command.type, status: state, authorization: { capability, state, code } } as Command extends CreateCommand ? CordisXAgentLoopCreateOrBindResult : CordisXAgentLoopSendResult) }
  private refusalFrom<Command extends CreateCommand | SendCommand>(command: Command, authorization: Refused): Command extends CreateCommand ? CordisXAgentLoopCreateOrBindResult : CordisXAgentLoopSendResult { return clone({ $schema: CORDISX_AGENT_LOOP_RESULT_SCHEMA_V1, contract: 'cordisx.agent-loop-result/v1', schemaVersion: 1, commandId: command.commandId, type: command.type, status: authorization.state, authorization } as Command extends CreateCommand ? CordisXAgentLoopCreateOrBindResult : CordisXAgentLoopSendResult) }
  private owned(ownerKey: string, binding: CordisXAgentLoopTaskBinding): BindingRecord | undefined { const record = this.records.get(binding.binding.bindingId); return record?.ownerKey === ownerKey && record.binding.state === 'active' && record.binding.binding.generation === binding.binding.generation && record.binding.task === binding.task && sameIdentity(record.binding.definition, binding.definition) ? record : undefined }
  ...
  private append(record: BindingRecord, payload: EventPayload): void {
    const event = clone({ $schema: CORDISX_AGENT_LOOP_EVENT_SCHEMA_V1, contract: 'cordisx.agent-loop-event/v1', schemaVersion: 1, eventId: `cxloop-event:${record.binding.binding.bindingId}:${record.events.length}`, binding: record.binding.binding, sequence: record.events.length, occurredAt: this.now().toISOString(), ...payload } as CordisXAgentLoopEvent)
    record.events.push(event); for (const listener of record.listeners) listener([event])
  }
  ...
  private projectLifecycle(record: BindingRecord, event: CordisXAgentLoopLifecycleEvent): void {
    if (event.type === 'turn.completed') {
      if ((event.output?.length ?? 0) > 0) this.append(record, { type: 'message', turn: event.turnId, message: { messageId: `cxloop-assistant:${event.turnId}`, role: 'assistant', content: event.output!.map(item => ({ kind: 'text' as const, text: item.text })) as [CordisXAgentLoopContentPart, ...CordisXAgentLoopContentPart[]] } })
      this.append(record, { type: 'lifecycle', turn: event.turnId, lifecycle: { phase: 'turn.completed' } }); return
    }
    if (event.type === 'turn.failed') { this.append(record, { type: 'lifecycle', turn: event.turnId, lifecycle: { phase: 'turn.failed', failure: event.failure ?? { code: 'AGENT_LOOP_FAILED', retryable: false } } }); return }
    if (event.type === 'turn.started') { if (!record.events.some(item => item.type === 'lifecycle' && item.turn === event.turnId && item.lifecycle.phase === 'turn.started')) this.append(record, { type: 'lifecycle', turn: event.turnId, lifecycle: { phase: 'turn.started' } }); return }
    if (event.approval === undefined) return
    this.append(record, { type: 'approval', turn: event.turnId, approval: event.approval.state === 'pending' ? { approvalId: event.approval.approvalId, kind: event.approval.kind, state: 'pending' } : { approvalId: event.approval.approvalId, kind: event.approval.kind, state: 'resolved', outcome: event.approval.outcome ?? 'cancelled' } })
  }
