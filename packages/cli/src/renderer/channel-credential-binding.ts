...
export class BrowserChannelCredentialBridge {
  private readonly pending = new Map<string, { resolve: (value: HostServiceConfigMutationResult) => void; reject: (error: Error) => void; timer: ReturnType<typeof setTimeout> }>()
  private constructor(private readonly token: string) { globalThis[RECEIVER] = this.receive }
  static connect(token: string): BrowserChannelCredentialBridge { return new BrowserChannelCredentialBridge(token) }
  async create(input: { readonly account: { readonly adapterId: string; readonly accountId: string; readonly tenantId: string }; readonly secret: string; readonly mutation: HostServiceConfigMutation }): Promise<HostServiceConfigMutationResult> {
    ...
    return await new Promise<HostServiceConfigMutationResult>((resolve, reject) => {
      const timer = setTimeout(() => { this.pending.delete(requestId); reject(new Error('channel-credential-timeout')) }, 5_000)
  ...
  private readonly receive = (payload: string): void => {
    ...
    const pending = this.pending.get(response.requestId); if (pending === undefined) return
