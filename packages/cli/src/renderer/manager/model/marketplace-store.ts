...
function bridgeFetcher(view: Window | null): { readonly fetcher?: MarketplaceFetcher; readonly dispose: () => void } {
  ...
  const pending = new Map<
    ...
    {
      readonly resolve: (
        value: { readonly ok: boolean; readonly status: number; readonly text: () => Promise<string> },
      ) => void
  ...
  const receiver = (payloadText: string) => {
    try {
      ...
      view.clearTimeout(request.timeout)
      pending.delete(payload.requestId)
      if (typeof payload.status === 'number' && typeof payload.text === 'string') {
        request.resolve({
          ok: payload.ok === true,
          status: payload.status,
          text: async () => payload.text as string,
        })
      } else {request.reject(
          new Error(typeof payload.error === 'string' ? payload.error : 'marketplace launcher bridge failed'),
        )}
    } catch { /* malformed bridge messages are ignored until timeout */ }
  }
  bridge.__cordisxMarketplaceReceiveV1 = receiver
  const fetcher: MarketplaceFetcher = async url =>
  ...
  return {
    ...
    dispose: () => {
      ...
      for (const request of pending.values()) {
        view.clearTimeout(request.timeout)
        request.reject(new Error('CordisX Manager 已关闭'))
      }
      pending.clear()
    },
  }
