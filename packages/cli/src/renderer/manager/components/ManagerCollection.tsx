...
import { type KeyboardEvent, useEffect, useId, useLayoutEffect, useRef, useState, useSyncExternalStore } from 'react'
...
function ActionIcon({ action }: { readonly action: ManagerCollectionAction }) {
  ...
  if (semantic === 'pin' || semantic === 'unpin') {
    return <HostSurfaceIcon token="host:pin" state={action.pressed || semantic === 'unpin' ? 'active' : 'default'} />
  }
  ...
  if (action.icon !== undefined) {
    return <HostSurfaceIcon token={action.icon as CordisXIconToken} state={action.pressed ? 'active' : 'default'} />
  }
...
function LeadingVisual({ item }: { readonly item: ManagerCollectionItem }) {
  ...
  if (visual.kind === 'semantic-icon') {
    return (
      <span className="cxr-manager-collection-visual" data-kind="semantic-icon">
        <HostSurfaceIcon token={visual.icon} />
      </span>
    )
  }
  return (
    <span className="cxr-manager-collection-visual" data-kind="semantic-fallback">
...
function OverflowMenu({
  ...
  useEffect(() => {
    ...
    const closeOnOutsidePointer = (event: PointerEvent) => {
      ...
      if (
        target instanceof registry.options.document.defaultView!.Node && !menu.current?.contains(target)
  ...
  return (
    <span
      className="cxr-manager-collection-overflow"
    ...
    >
      <button
        ref={trigger}
        type="button"
        className="cxr-manager-collection-action"
        aria-label={registry.options.hostCopy('more-actions')}
        aria-haspopup="menu"
        aria-expanded={open}
        disabled={busy}
      ...
      {!open
        ...
        : (
          <div
            ref={menu}
            className="cxr-manager-collection-menu"
            role="menu"
            aria-label={registry.options.hostCopy('more-actions')}
            onKeyDown={onKeyDown}
          >
            {actions.map(action => (
              <button
                ...
                type="button"
                role="menuitem"
                disabled={busy || action.disabled.value}
                aria-pressed={action.pressed}
                data-tone={action.tone}
                title={action.disabled.reason === undefined
                  ? undefined
                  : registry.localized(action.disabled.reason, `item:${item.id}:action:${action.id}:disabled`)}
                onClick={() => {
              ...
              >
                ...
                <span>{registry.localized(action.label, `item:${item.id}:action:${action.id}:label`)}</span>
              </button>
            ))}
          </div>
        )}
...
function Row(
  { item, registry, busy, rememberTrigger }: {
    readonly item: ManagerCollectionItem
    readonly registry: HostManagerCollectionPageRegistry
    readonly busy: boolean
    readonly rememberTrigger: (element: HTMLElement | null) => void
  ...
  return (
    <article
      className="cxr-manager-collection-row"
      role="listitem"
    ...
    >
      <button
        type="button"
        className="cxr-manager-collection-open"
        disabled={item.disabled.value}
        aria-label={title}
        aria-description={disabledReason ?? summary}
        onClick={() => void registry.open(item.id)}
      >
        <LeadingVisual item={item} />
        <span className="cxr-manager-collection-copy">
          <strong>{title}</strong>
          <span>{summary}</span>
        </span>
      </button>
      {direct.length === 0 && overflow.length === 0
        ? null
        : (
          <span
            className="cxr-manager-collection-actions"
            onPointerDown={event => event.stopPropagation()}
            onClick={event => event.stopPropagation()}
          >
            {direct.map(action => (
              <button
                ...
                data-tone={action.tone}
                disabled={busy || action.disabled.value}
                aria-label={registry.localized(
                  action.ariaLabel ?? action.label,
                  `item:${item.id}:action:${action.id}:aria`,
                )}
                aria-pressed={action.pressed}
                title={action.disabled.reason === undefined
...
function Dialog(
  ...
  const onKeyDown = (event: KeyboardEvent<HTMLDivElement>) => {
    ...
    if (
      event.key === 'Enter' && valid && !event.nativeEvent.isComposing
  ...
  return (
    <div
      className="cxr-manager-collection-dialog-backdrop"
      onPointerDown={event => {
    ...
    >
      <div
        ref={panel}
        className="cxr-manager-collection-dialog"
        data-tone={dialog.tone}
        role="dialog"
        aria-modal="true"
        aria-labelledby={titleId}
        aria-describedby={dialog.description === undefined ? undefined : descriptionId}
        onKeyDown={onKeyDown}
      >
        <h3 id={titleId}>{registry.localized(dialog.title, `dialog:${dialog.actionId}:title`)}</h3>
        {dialog.description === undefined
          ? null
          : <p id={descriptionId}>{registry.localized(dialog.description, `dialog:${dialog.actionId}:description`)}</p>}
        {dialog.input === undefined ? null : (
          <label>
            <span>{registry.localized(dialog.input.label, `dialog:${dialog.actionId}:label`)}</span>
            <input
              type="text"
              value={value}
              placeholder={dialog.input.placeholder === undefined
                ? undefined
                : registry.localized(dialog.input.placeholder, `dialog:${dialog.actionId}:placeholder`)}
              onChange={event => setValue(event.currentTarget.value)}
              aria-invalid={!valid}
            />
          </label>
        )}
        <div className="cxr-manager-collection-dialog-actions">
          <button type="button" onClick={close}>{registry.options.hostCopy('cancel')}</button>
          <button
            type="button"
            data-tone={dialog.tone === 'danger' ? 'danger' : 'primary'}
            disabled={!valid}
            onClick={() => {
              registry.submitDialog(normalized)
              restoreFocus()
          ...
          >
            {registry.localized(dialog.confirmLabel, `dialog:${dialog.actionId}:confirm`)}
          </button>
        </div>
      </div>
    </div>
...
function ViewSelector(
  { registry, views, active, panelId }: {
    readonly registry: HostManagerCollectionPageRegistry
    readonly views: NonNullable<ReturnType<HostManagerCollectionPageRegistry['snapshot']>['registration']>['views']
  ...
  return (
    <div className="cxr-manager-collection-views" role="tablist" aria-label={registry.options.hostCopy('views')}>
      {views.map((view, index) => (
        <button
          key={view.id}
          id={`${panelId}-${view.id}`}
          ref={node => {
            if (node === null) refs.current.delete(view.id)
            else refs.current.set(view.id, node)
          }}
          type="button"
          role="tab"
        ...
        >
          {registry.localized(view.label, `view:${view.id}:label`)}
        </button>
      ))}
    </div>
  )
...
function ManagerCollectionHost({ registry }: { readonly registry: HostManagerCollectionPageRegistry }) {
  ...
  return (
    <section
      ref={section}
      className="cxr-manager-collection"
      aria-label={registry.localized(registration.label, 'label')}
      aria-busy={snapshot.state === 'loading' || snapshot.busy !== undefined}
    >
      {!multipleViews
        ? null
      ...
      >
        {snapshot.state === 'loading'
          ? <div className="cxr-manager-collection-state" role="status">{registry.options.hostCopy('loading')}</div>
          : snapshot.state === 'error'
          ? (
            <div className="cxr-manager-collection-state" role="alert">
              <strong>{registry.options.hostCopy('error-title')}</strong>
              <span>{registry.options.hostCopy('error-description')}</span>
              <button type="button" onClick={() => registry.retry()}>{registry.options.hostCopy('retry')}</button>
            </div>
          )
          : items.length === 0
          ? (
            <div className="cxr-manager-collection-state" role="status">
              <strong>
                {empty === undefined ? registry.options.hostCopy('empty') : registry.localized(empty, 'empty:title')}
              </strong>
              {emptyDescription === undefined
                ? null
                : <span>{registry.localized(emptyDescription, 'empty:description')}</span>}
            </div>
          )
          : (
            <div
              className="cxr-manager-collection-list"
              role="list"
              aria-label={registry.localized(registration.label, 'list:label')}
            >
              {items.map(item => (
                <Row
                  key={item.id}
      ...
      {feedback === undefined
        ...
        : (
          <div
            className="cxr-manager-collection-feedback"
            data-tone={feedback.tone}
            role={feedback.tone === 'error' ? 'alert' : 'status'}
          >
            <span>{registry.localized(feedback.message, `feedback:${feedback.id}`)}</span>
            <button
              type="button"
              aria-label={registry.options.hostCopy('clear-feedback')}
              onClick={() => registry.clearFeedback()}
            >
              <HostIcon token="close" />
            </button>
          </div>
      ...
      {snapshot.dialog === undefined
        ? null
        : <Dialog dialog={snapshot.dialog} registry={registry} restoreFocus={restoreFocus} />}
    </section>
  )
}
...
export function mountManagerCollectionHost(
  container: HTMLElement,
  options: HostManagerCollectionPageOptions,
): MountedManagerCollectionHost {
