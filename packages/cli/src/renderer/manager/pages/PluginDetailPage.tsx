...
function RuntimePanel(
  { model, plugin, permissionCount, pointCount, routeCount, locale }: {
    readonly model: ManagerModel
    readonly plugin: ManagerPluginSnapshot
  ...
  return (
    <div className="cxm-runtime-overview">
      {plugin.development === undefined
        ? null
        : (
          <section className="cxr-section" data-plugin-development={plugin.development.state}>
            <h3>{managerCopy(locale, 'plugins.local-development')}</h3>
            <dl className="cxr-facts">
              <div>
                <dt>{managerCopy(locale, 'plugins.source-path')}</dt>
                <dd>
                  <code>{plugin.development.sourcePath}</code>
                </dd>
              </div>
              <div>
                <dt>{managerCopy(locale, 'plugins.build-status')}</dt>
                <dd>{plugin.development.state}</dd>
              </div>
              {plugin.development.lastSuccessfulAt === undefined ? null : (
                <div>
                  <dt>{managerCopy(locale, 'plugins.last-success')}</dt>
                  <dd>{plugin.development.lastSuccessfulAt}</dd>
                </div>
              )}
            </dl>
            {plugin.development.error === undefined
              ? null
              : <div className="cxr-notice cxr-danger" role="alert">{plugin.development.error}</div>}
          </section>
        )}
      <section data-plugin-runtime-status={plugin.id}>
        <strong>{plugin.status}</strong>
        {plugin.error === undefined ? null : <span>{plugin.error}</span>}
      </section>
      <div className="cxr-metrics">
        {metrics.map(([label, value]) => (
          <section className="cxr-metric" key={label}>
            <span>{label}</span>
            <strong>{value}</strong>
          </section>
        ))}
      </div>
      <section className="cxm-runtime-console-summary" data-runtime-console-summary={plugin.id}>
        {consoleMetrics.map(([label, value]) => (
          <div className="cxm-runtime-console-metric" key={label}>
            <strong>{value}</strong>
            <span>{label}</span>
          </div>
        ))}
      ...
      <section className="cxr-section" style={{ marginTop: 10 }}>
        <h3>{managerCopy(locale, 'plugins.injected-capabilities')}</h3>
        <div className="cxr-token-list">
          {plugin.inject.map(item => <code key={item}>{item}</code>)}
          {plugin.inject.length === 0 ? <span>{managerCopy(locale, 'plugins.none')}</span> : null}
        </div>
      </section>
    </div>
  )
...
export function PluginDetailPage(
  ...
  const visiblePoints = pointUsage.filter(point =>
    `${point.titleProjection.text} ${point.descriptionProjection.text} ${point.id}`.toLocaleLowerCase().includes(
      normalized,
    )
  )
  const visibleRoutes = routes.filter(item =>
    `${item.productMetadata.title ?? ''} ${
  ...
  return (
    <section className="cxr-page" data-plugin-detail={plugin.id}>
      <section
        className="cxr-plugin-identity"
        aria-label={managerCopy(snapshot.localization.locale, 'plugins.details-actions')}
      >
            ...
            {sourceLink === undefined
              ...
              : (
                <a href={sourceLink} target="_blank" rel="noopener noreferrer">
                  {managerCopy(snapshot.localization.locale, 'plugins.project-link')}
                </a>
              )}
          </span>
        </span>
        <span className="cxr-plugin-identity-actions">
          <Button
            shape="square"
            variant="outline"
            aria-label={managerCopy(snapshot.localization.locale, 'plugins.reload')}
          ...
          {plugin.status === 'configured-disabled'
            ? (
              <Button
                shape="square"
                theme="primary"
                aria-label={managerCopy(snapshot.localization.locale, 'plugins.enable')}
            ...
            : (
              <Button
                shape="square"
                variant="outline"
                aria-label={managerCopy(snapshot.localization.locale, 'plugins.disable')}
          ...
          <Button
            ...
            variant="outline"
            theme="danger"
            aria-label={managerCopy(snapshot.localization.locale, 'plugins.uninstall')}
      ...
      {route.page === 'config' && (
        <div
          className="cxr-plugin-config-panel"
          role="tabpanel"
          aria-label={managerCopy(snapshot.localization.locale, 'plugin-tab.configuration')}
        >
          ...
          <div className="cxr-list">
            {visibleRoutes.map(item => (
              <button
                ...
                onClick={() => router.navigate({ kind: 'route', qualifiedId: item.qualifiedId })}
              >
                <span className="cxr-card-body">
                  <span className="cxr-card-title">{item.productMetadata.title ?? item.qualifiedId}</span>
                  <span className="cxr-card-description">{item.productMetadata.description}</span>
                  <code className="cxr-card-code">{item.definition.path}</code>
                </span>
              </button>
            ))}
