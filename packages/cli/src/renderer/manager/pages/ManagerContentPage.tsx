...
export function ManagerContentPage(
  { model, router, locale }: { readonly model: ManagerModel; readonly router: ManagerRouter; readonly locale: string },
) {
  ...
  return (
    <section className="cxr-page cxr-manager-content-page" data-manager-content-page="true">
      {presentation?.recordSummary === undefined ? null : (
        <section
        ...
        >
          <span
            className="cxr-manager-record-avatar"
            role="img"
            aria-label={presentation.recordSummary.title}
            data-manager-content-record-avatar-kind={presentation.recordSummary.leadingVisual.kind}
          >
            <HostAgentAvatar
              participant={{
          ...
          <span className="cxr-manager-record-copy">
            ...
            {presentation.recordSummary.description === undefined
              ? null
              : <span>{presentation.recordSummary.description}</span>}
          </span>
        </section>
      )}
      {presentation === undefined || contributionId === undefined || presentation.tabs.length === 0
        ? null
        : (
          <div className="cxr-tabs" role="tablist" aria-label={presentation.title} data-manager-content-tabs="true">
            {presentation.tabs.map((tab, index) => (
              <button
                key={tab.id}
                id={`${panelId}-${tab.id}`}
                ref={node => {
                  if (node === null) tabs.current.delete(tab.id)
                  else tabs.current.set(tab.id, node)
                }}
                type="button"
                role="tab"
                data-manager-content-tab={tab.id}
      ...
      <div
        className="cxr-manager-content-panel"
        id={panelId}
        role={activeTab === undefined ? undefined : 'tabpanel'}
