  ...
  const plugins = useMemo(
    () =>
      snapshot.plugins.filter(plugin =>
        normalized === ''
        || `${plugin.name} ${plugin.id} ${plugin.description ?? ''}`.toLocaleLowerCase().includes(normalized)
      ),
  ...
  return (
    <section className="cxr-page" aria-label={managerCopy(snapshot.localization.locale, 'plugins.heading')}>
      <SearchField
        ...
        value={query}
        aria-label={managerCopy(snapshot.localization.locale, 'plugins.search-label')}
        placeholder={managerCopy(snapshot.localization.locale, 'plugins.search-placeholder')}
        onChange={setQuery}
      />
      <div className="cxr-list" role="list">
        ...
        {plugins.length === 0 && pendingDevelopment.length === 0
