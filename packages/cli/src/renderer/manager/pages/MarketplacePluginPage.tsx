...
const COPY = {
  'zh-CN': {
    ...
    required: '必需',
    optional: '可选',
    identityLabel: '插件信息与操作',
    detailsLabel: 'Marketplace 身份与审核信息',
    official: '官方',
    certified: '已认证',
    evidence: '查看受保护审核证据',
    officialSummary:
    ...
    certifiedSummary:
      'CordisX 已按策略 {policy} 审核当前 {version} 版本的明确制品，并认定其代码符合该版本策略。新版本或制品摘要变化后必须重新认证。',
    certifiedBoundary:
  ...
  en: {
    ...
    searchPermissions: 'Search permissions, request reasons, or type…',
    searchPermissionsLabel: 'Search plugin permissions',
    ...
    source: 'Source',
    homepage: 'Homepage',
    feed: 'Marketplace source',
    author: 'Author',
    opensInNewWindow: 'opens in a new window',
    missing: 'This Marketplace record no longer exists',
    required: 'Required',
    optional: 'Optional',
    identityLabel: 'Plugin information and actions',
    detailsLabel: 'Marketplace identity and review information',
    official: 'Official',
    certified: 'Certified',
    evidence: 'View protected review evidence',
    officialSummary:
      'Official means CordisX creates and maintains this plugin through a trusted publisher, source repository, and package namespace. It affects Marketplace identity, filters, and product ordering among equally relevant results only. It never changes PermissionBroker decisions or automatically means Certified.',
    certifiedSummary:
      'CordisX reviewed the exact {version} artifact under policy {policy} and determined that its code conforms to that policy version. A new version or artifact digest requires a new certification.',
    certifiedBoundary:
...
export function MarketplacePluginPage({ marketplace, snapshot: managerSnapshot, router }: {
  ...
  const permissions = installed === undefined
    ...
    : managerSnapshot.permissions.filter(item =>
      item.identity.id === installed.id && item.identity.source === installed.source
    )
  const permissionName = (capability: typeof permissions[number]['capability']): string =>
    projectPermissionCapabilityName(capability, managerSnapshot.localization.locale)
  const normalizedPermissionQuery = permissionQuery.trim().toLocaleLowerCase()
  const visiblePermissions = normalizedPermissionQuery.length === 0
    ? permissions
  ...
  const toggleFavorite = () => {
    setFavorites(current => {
      ...
      if (next.has(plugin.identity)) next.delete(plugin.identity)
      else next.add(plugin.identity)
      writeMarketplaceFavorites(next)
      return next
    })
  }
  const externalLabel = (label: string): string =>
    locale === 'zh-CN' ? `${label}（${copy.opensInNewWindow}）` : `${label} (${copy.opensInNewWindow})`
  const links = [
    ...projection.authors.flatMap(author =>
      author.url === undefined
        ? []
        : [{ label: `${copy.author} · ${author.name}`, value: author.url, href: author.url }]
    ),
  ...
  return (
    <section className="cxr-page" data-marketplace-plugin-detail={plugin.id}>
      <section
        className="cxr-plugin-identity cxr-marketplace-identity"
        aria-label={[copy.identityLabel, ...trustLabels].join(' · ')}
      >
        <span className="cxr-card-icon">
          {plugin.icon === undefined
            ? projection.name.slice(0, 2).toLocaleUpperCase()
            : <img src={plugin.icon} alt="" referrerPolicy="no-referrer" />}
        </span>
        <span className="cxr-plugin-identity-copy">
          <span className="cxr-marketplace-title-row">
            <strong>{projection.name}</strong>
            <MarketplaceTrustBadges plugin={plugin} locale={managerSnapshot.localization.locale} />
          </span>
          <span className="cxr-plugin-identity-description">{projection.description}</span>
          <span className="cxr-plugin-identity-meta">
            <span>{plugin.version}</span>
            <code>{plugin.id}</code>
            <span>{projection.feedName}</span>
          </span>
        </span>
      ...
      {plugin.official === undefined && plugin.certification === undefined
        ? null
        : (
          <section className="cxr-marketplace-trust-details" aria-label={copy.detailsLabel}>
            {plugin.official === undefined ? null : (
              <div data-marketplace-trust-dimension="official">
                <h3>
                ...
                <p>
                  <span className="cxr-marketplace-trust-record-label">{copy.fallbackDescription}：</span>
                  {plugin.official.description.fallback}
                </p>
                <a href={plugin.official.reviewer.evidenceRef} target="_blank" rel="noopener noreferrer">
                  {copy.evidence}
                  <HostIcon token="external-link" />
                </a>
              </div>
            )}
            {plugin.certification === undefined ? null : (
              <div data-marketplace-trust-dimension="certified">
                <h3>
                ...
                <p>
                  <span className="cxr-marketplace-trust-record-label">{copy.fallbackDescription}：</span>
                  {plugin.certification.description.fallback}
                </p>
                <p>{copy.certifiedBoundary}</p>
                <p>{copy.mergeChain}</p>
                <a href={plugin.certification.reviewer.evidenceRef} target="_blank" rel="noopener noreferrer">
                  {copy.evidence}
                  <HostIcon token="external-link" />
                </a>
              </div>
        ...
        <div role="tabpanel" aria-label={copy.tabs.permissions}>
          ...
          <div className="cxr-list cxr-permission-list" role="list">
            {visiblePermissions.map(item => (
              <article className="cxr-card cxr-permission-summary" role="listitem" key={item.capability}>
                <span className="cxr-card-icon">
                  <HostIcon token="permissions" />
                </span>
                <span className="cxr-card-body">
                  <span className="cxr-card-title">{permissionName(item.capability)}</span>
                  <span className="cxr-card-description">{item.reasonText}</span>
                  <code className="cxr-card-code">{item.capability}</code>
                </span>
                <span className="cxr-status">{item.required ? copy.required : copy.optional}</span>
              </article>
            ))}
      ...
      {tab === 'authors-source' && (
        <div role="tabpanel" aria-label={copy.tabs.authorsSource}>
          <div className="cxr-marketplace-detail-grid">
            <section className="cxr-metric">
              <span>{copy.version}</span>
              <strong>{plugin.version}</strong>
            </section>
            <section className="cxr-metric">
              <span>{copy.compatibility}</span>
              <strong>{plugin.compatibility.cordisx}</strong>
            </section>
            <section className="cxr-metric">
            ...
            {links.map(link => (
              <a
                ...
                className="cxr-card cxr-marketplace-source-link"
                href={link.href}
                target="_blank"
                rel="noopener noreferrer"
                aria-label={externalLabel(link.label)}
              >
                <span className="cxr-card-icon">
                  <HostIcon token="authors-source" />
                </span>
                <span className="cxr-card-body">
                  <span className="cxr-card-title">{link.label}</span>
                  <code className="cxr-card-code">{link.value}</code>
                </span>
                <HostIcon token="external-link" />
              </a>
            ))}
