...
export function MarketplacePluginPage({ marketplace, snapshot: managerSnapshot, router }: {
  ...
  return <section className="cxr-page" data-marketplace-plugin-detail={plugin.id}>
    <section className="cxr-plugin-identity cxr-marketplace-identity" aria-label="插件信息与操作">
      ...
      <span className="cxr-plugin-identity-copy">
        ...
        <span className="cxr-plugin-identity-meta"><span>{plugin.version}</span><code>{plugin.id}</code><span>{projection.feedName}</span>{plugin.official === undefined ? null : <span>Official</span>}{plugin.certification === undefined ? null : <span>Certified</span>}</span>
      </span>
