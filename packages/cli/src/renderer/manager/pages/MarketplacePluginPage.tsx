...
const COPY = {
  'zh-CN': {
    ...
    certifiedBoundary: '认证不是绝对安全保证，也不放宽沙箱、生命周期或安装审核。通过当前 Marketplace Certified 条目安装或启用时，插件声明的权限会因 Certified 自动授权；Host 仍按当前范围和运行实例创建可撤销、可审计的授权。',
  ...
  en: {
    ...
    certifiedBoundary: 'Certification is not an absolute safety guarantee and does not relax sandbox, lifecycle, or installation review. Installing or enabling through the current Marketplace Certified entry auto-authorizes the plugin\'s declared permissions. The Host still creates revocable, audited authorization for the current scope and runtime instance.',
