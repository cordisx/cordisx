...
interface DocumentWatch {
  ...
  readonly listeners: Set<{ readonly listener: (result: CordisXOwnerDocumentLoadResultV1) => void; readonly active: () => boolean }>
...
function unavailable(code: Extract<CordisXOwnerDocumentLoadResultV1, { status: 'unavailable' }>['code'], diagnostic: string, recoverable = true): Extract<CordisXOwnerDocumentLoadResultV1, { status: 'unavailable' }> {
  return Object.freeze({ status: 'unavailable', code, diagnostic: diagnostic.slice(0, 512), recoverable })
}
function assertDocumentId(documentId: string): void { if (!/^[a-z0-9][a-z0-9._-]{0,95}$/u.test(documentId)) throw new Error('documentId is invalid') }
...
export class BrowserOwnerDocumentBridge {
  ...
  #receive = (payload: string): void => {
    ...
    try { const parsed = JSON.parse(payload) as unknown; if (parsed === null || typeof parsed !== 'object' || Array.isArray(parsed)) return; response = parsed as Record<string, unknown> } catch { return }
  ...
  constructor() { globalThis[OWNER_DOCUMENT_RECEIVER] = this.#receive }

  async request(token: string, value: Omit<Record<string, unknown>, 'version' | 'requestId' | 'token'>): Promise<unknown> {
    ...
    return await new Promise<unknown>((resolve, reject) => {
      const timer = setTimeout(() => { const pending = this.#pending.get(requestId); if (pending === undefined) return; this.#pending.delete(requestId); pending.reject(new Error('owner document bridge request timed out')) }, REQUEST_TIMEOUT_MS)
      this.#pending.set(requestId, { resolve, reject, timer })
      try { binding(JSON.stringify({ version: 1, requestId, token, ...value })) } catch (error) { clearTimeout(timer); this.#pending.delete(requestId); reject(error instanceof Error ? error : new Error(String(error))) }
    })
  }

  #watchKey(token: string, documentId: string): string { return `${token}\0${documentId}` }
  ...
  #publish(watch: DocumentWatch, result: CordisXOwnerDocumentLoadResultV1): void {
    ...
    const key = resultKey(result); if (key === watch.lastKey) return; watch.lastKey = key
    for (const entry of watch.listeners) {
      if (!entry.active()) continue
      try { entry.listener(immutable(result)) } catch { /* isolate consumers */ }
    }
  }

  async #poll(watch: DocumentWatch): Promise<void> {
    if (watch.polling || this.#disposed || this.#watches.get(this.#watchKey(watch.token, watch.documentId)) !== watch) return
    watch.polling = true
    try {
      const result = immutable(await this.request(watch.token, { operation: 'load', documentId: watch.documentId }) as CordisXOwnerDocumentLoadResultV1)
      this.#publish(watch, result)
    } catch { /* next bounded poll retries launcher availability */ } finally { watch.polling = false }
  }

  watch(token: string, documentId: string, listener: (result: CordisXOwnerDocumentLoadResultV1) => void, active: () => boolean): Disposable<void> {
    if (this.#disposed) throw new Error('owner document bridge is disposed')
    ...
    if (watch === undefined) {
      ...
      watch = { token, documentId, listeners: new Set(), polling: false, timer: undefined as unknown as ReturnType<typeof setInterval> }
      watch.timer = setInterval(() => { void this.#poll(watch!) }, POLL_INTERVAL_MS)
      this.#watches.set(key, watch); void this.#poll(watch)
    }
    const entry = { listener, active }; watch.listeners.add(entry); this.#listenerCount += 1
    let subscribed = true
    return () => {
      if (!subscribed) return; subscribed = false
      const current = this.#watches.get(key); if (current === undefined) return
      if (current.listeners.delete(entry)) this.#listenerCount -= 1
      if (current.listeners.size === 0) { clearInterval(current.timer); this.#watches.delete(key) }
    }
  }

  publish(token: string, documentId: string, result: CordisXOwnerDocumentLoadResultV1): void {
    const watch = this.#watches.get(this.#watchKey(token, documentId)); if (watch !== undefined) this.#publish(watch, result)
  }
  ...
  dispose(): void {
    ...
    this.#watches.clear(); this.#listenerCount = 0
    if (globalThis[OWNER_DOCUMENT_RECEIVER] === this.#receive) globalThis[OWNER_DOCUMENT_RECEIVER] = undefined
    for (const pending of this.#pending.values()) { clearTimeout(pending.timer); pending.reject(new Error('owner document bridge is disposed')) }
    this.#pending.clear()
  }
}
...
class BoundOwnerDocuments implements CordisXOwnerDocumentsV1 {
  ...
  constructor(bridge: BrowserOwnerDocumentBridge | undefined, token: string | undefined, active: () => boolean, onDispose: () => void) { this.#bridge = bridge; this.#token = token; this.#active = active; this.#onDispose = onDispose }
  #live(): boolean { return !this.#disposed && this.#active() }

  async load(documentId: string): Promise<CordisXOwnerDocumentLoadResultV1> {
    try { assertDocumentId(documentId) } catch (error) { return unavailable('invalid-request', error instanceof Error ? error.message : 'documentId is invalid', false) }
    if (!this.#live()) return unavailable('stale-generation', 'plugin generation is stale')
    if (this.#bridge === undefined || this.#token === undefined) return unavailable('bridge-unavailable', 'owner document bridge is unavailable')
    try { return immutable(await this.#bridge.request(this.#token, { operation: 'load', documentId }) as CordisXOwnerDocumentLoadResultV1) } catch { return unavailable('host-unavailable', 'owner document request failed') }
  }
  ...
  async replace(command: CordisXOwnerDocumentReplaceCommandV1): Promise<CordisXOwnerDocumentReplaceResultV1> {
    try {
      ...
      const result = immutable(await this.#bridge.request(this.#token, { operation: 'replace', documentId: command.documentId, expectedRevision: command.expectedRevision, schemaVersion: command.schemaVersion, value }) as CordisXOwnerDocumentReplaceResultV1)
      if (result.status === 'accepted') this.#bridge.publish(this.#token, command.documentId, { status: 'loaded', snapshot: result.snapshot })
      return result
    } catch (error) { return unavailable('invalid-request', error instanceof Error ? error.message : 'request is invalid', false) }
  }

  async transaction(command: CordisXOwnerDocumentReplaceCommandV1): Promise<CordisXOwnerDocumentReplaceResultV1> { return await this.replace(command) }
  ...
  subscribe(documentId: string, listener: (result: CordisXOwnerDocumentLoadResultV1) => void): Disposable<void> {
    ...
    unsubscribe = () => { release(); this.#subscriptions.delete(unsubscribe) }
  ...
  dispose(): void {
    ...
    this.#subscriptions.clear(); this.#token = undefined; this.#bridge = undefined
    this.#onDispose?.(); this.#onDispose = undefined
  }
}
...
export class CordisXOwnerDocumentBroker {
  ...
  dispose(): void { for (const client of [...this.#clients]) client.dispose(); this.#clients.clear(); this.#bridge?.dispose(); this.#bridge = undefined }
}
