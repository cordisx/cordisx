...
import type { CordisXAgentLoopHost, CordisXBoundAgentLoopClientOptions } from '../agent-loop.js'
...
import {
  AgentLoopV4Transport,
  ApprovalCommand,
  authorizationFor,
  bindingFor,
  CancelIntroductionCommand,
  Capability,
  clone,
  ...
  InternalLifecycleResult,
  internalRefusal,
  InternalResult,
  IntroductionCommand,
  presentationForDefinition,
  PromptRegistration,
  record,
  refusal,
  scope,
  SendCommand,
  shellSafeAssistantMessageId,
  string,
  tupleKey,
} from './agent-loop-v4-contract.js'
...
export class CordisXAgentLoopBrokerV4 {
  ...
  private readonly introductions = new Map<
    string,
    { operationId: string; messageId: string; participantId: string; memberId: string; runId: string }
  >()
  ...
  > {
    ...
    try {
      const value = record(
        await this.transport.resolveAgentLoopV4Session({
  ...
  bind(options: CordisXBoundAgentLoopClientOptions): BoundAgentLoopClient {
    ...
    const mutationKey = (binding: AgentLoopTaskBinding) =>
      tupleKey(
    ...
    const mutateLifecycle = async <Value>(
      binding: AgentLoopTaskBinding,
    ...
    const registerPrompt = (
      task: string,
      definition: Parameters<NonNullable<typeof options.registerPrompt>>[1],
    ): boolean => {
    ...
    const authorize = async (
      ...
      binding?: AgentLoopTaskBinding,
    ): Promise<AgentLoopAuthorizationOutcome> => {
    ...
    const client: BoundAgentLoopClient = Object.freeze({
      ...
      createOrBind: async (command: CreateCommand): Promise<AgentLoopCreateOrBindResult> => {
        ...
        let definition
        try {
          definition = resolveAgentDefinition(command as never)
        } catch {
        ...
        let detailsUrl
        try {
          detailsUrl = validateAgentLoopTaskDetailsUrl(clone(internal.detailsUrl) as never)
        } catch {
        ...
        this.definitionPresentations.set(
          definitionPresentationKey(definition.identity),
          presentationForDefinition(definition),
        )
      ...
      decideApproval: async (command: ApprovalCommand): Promise<AgentLoopApprovalDecisionResult> => {
        ...
        const internal = await mutateLifecycle(
          command.binding,
          async () =>
            await this.transport!.decideAgentLoopApprovalV4({
              scope: authorityScope,
              command,
              operationId: command.commandId,
              task: command.binding.task,
        ...
        this.approvals.set(
          correlationKey(authorityScope, command.binding.task, command.turn, command.approvalId),
          command.commandId,
        )
        return Object.freeze({
          $schema: CORDISX_AGENT_LOOP_RESULT_SCHEMA_V4,
      ...
      requestMemberSelfIntroduction: async (
        command: IntroductionCommand,
      ): Promise<AgentLoopRequestMemberSelfIntroductionResult> => {
        if (!live() || this.transport === undefined) {
        ...
        const internal = await mutateLifecycle(
          command.binding,
          async () =>
            await this.transport!.requestAgentLoopIntroductionV4({
              scope: authorityScope,
              command,
              operationId: command.commandId,
              task: command.binding.task,
      ...
      cancelMemberSelfIntroduction: async (
        command: CancelIntroductionCommand,
      ): Promise<AgentLoopCancelMemberSelfIntroductionResult> => {
        if (!live() || this.transport === undefined) {
          return refusal(command, 'turns.introduce', 'unavailable', 'host-unavailable')
        }
        const allowed = await authorize('turns.introduce', command.binding)
        ...
        const internal = await mutateLifecycle(
          command.binding,
          async () =>
            await this.transport!.cancelAgentLoopIntroductionV4({
              scope: authorityScope,
              command,
              operationId: command.commandId,
      ...
      subscribe: async (
      ...
      ): Promise<AgentLoopSubscribeRuntimeResult> => {
        if (!live() || this.transport === undefined) {
          return {
            status: 'unavailable',
        ...
        if (allowed.state !== 'allowed') {
          return { status: allowed.state, authorization: allowed } as AgentLoopSubscribeRuntimeResult
        }
        ...
        const descriptor = Object.freeze({
          ...
          contract: 'cordisx.agent-loop-event-subscription/v4' as const,
  ...
  private async *pages(
    ...
    while (active()) {
      ...
      const events = all.filter(event =>
        ...
        && (!replaying || event.sequence <= descriptor.snapshotSequence)
      ).slice(0, 256)
      if (events.length > 0) {
  ...
  private async publicEvents(
    ...
    snapshotKey: string,
    active: () => boolean,
  ): Promise<readonly AgentLoopEvent[] | undefined> {
  ...
  private projectLifecycle(
    authorityScope: CordisXAgentLoopV4Scope,
    binding: AgentLoopTaskBinding,
    input: InternalLifecycleEvent,
  ): readonly AgentLoopEvent[] {
    ...
    const base = {
      ...
      contract: 'cordisx.agent-loop-event/v4' as const,
      schemaVersion: 4 as const,
      ...
      const message = output.length === 0 || cancelled !== undefined ? [] : [{
        ...
        message: validIntroduction === undefined
          ? {
            ...
            content: output as [typeof output[number], ...typeof output],
          }
          : {
    ...
    if ((input.type === 'approval.required' || input.type === 'approval.resolved') && approvalId !== undefined) {
      const kind = ['file-change', 'external-action', 'other'].includes(String(approval?.kind))
        ? approval!.kind as 'file-change' | 'external-action' | 'other'
        : 'command'
