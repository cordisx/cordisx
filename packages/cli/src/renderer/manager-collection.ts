...
import {
  managerCollectionCodePointIncludes,
  normalizeManagerCollectionSearch,
} from './manager-collection-normalization.js'
...
function assertManagerLocalizedText(value: CordisXLocalizedText, label: string): void {
  ...
  if (value.fallback !== undefined && [...value.fallback].length > 4_000) {
    throw new Error(`${label}.fallback is too long`)
  }
  if (value.params !== undefined && Object.keys(value.params).length > 32) {
    throw new Error(`${label}.params has too many entries`)
  }
...
function assertRoute(reference: ManagerCollectionRouteReference, label: string): void {
  ...
  for (const [key, value] of Object.entries(reference.params)) {
    ...
    if (value !== null && !['string', 'number', 'boolean'].includes(typeof value)) {
      throw new Error(`${label}.params.${key} is not scalar`)
    }
...
function assertInputAction(action: ManagerCollectionTextInputAction, label: string): void {
  ...
  if (action.kind !== 'text-input-command' || action.tone !== 'neutral') {
    throw new Error(`${label} has an invalid text-input action kind or tone`)
  }
  ...
  if (action.command.arguments !== undefined) {
    ...
    if (Object.keys(action.command.arguments).length > 32) {
      throw new Error(`${label}.command.arguments has too many entries`)
    }
    for (const [key, value] of Object.entries(action.command.arguments)) {
      assertJsonValue(value, `${label}.command.arguments.${key}`)
    }
  }
  assertRecord(action.input, `${label}.input`)
  assertKeys(action.input, [
    'argument',
    'title',
    'description',
    'label',
  ...
  if (Object.hasOwn(action.command.arguments ?? {}, action.input.argument)) {
    throw new Error(`${label}.input.argument collides with an existing command argument`)
  }
  ...
  if (!Number.isInteger(action.input.minLength) || action.input.minLength < 0 || action.input.minLength > 4_000) {
    throw new Error(`${label}.input.minLength is invalid`)
  }
  if (
    !Number.isInteger(action.input.maxLength) || action.input.maxLength < 1 || action.input.maxLength > 4_000
    || action.input.maxLength < action.input.minLength
  ...
  if (
    action.input.initialValue !== undefined
    && ([...action.input.initialValue].length > 4_000 || CONTROL_CHARACTER.test(action.input.initialValue))
  ) {
...
function assertAction(action: ManagerCollectionAction, label: string): void {
  ...
  if (action.kind === 'text-input-command') assertInputAction(action, label)
    ...
    if (action.tone === 'danger' && action.confirmation === undefined) {
      throw new Error(`${label} requires confirmation for a danger command`)
    }
    ...
    if (
      typeof action.text.value !== 'string' || [...action.text.value].length < 1
      || [...action.text.value].length > 4_096 || action.text.value.includes('\0')
    ) {
...
function assertRegistration(input: ManagerCollectionRegistrationV1): ManagerCollectionRegistrationV1 {
  ...
  assertKeys(input, [
    ...
    'defaultView',
    'search',
  ], 'manager collection registration')
  if (
    input.$schema !== REGISTRATION_SCHEMA || input.contract !== 'cordisx.manager-collection-registration/v1'
    || input.schemaVersion !== 1
  ) throw new Error('manager collection registration has an unsupported schema tuple')
  ...
  if (!Array.isArray(input.views) || input.views.length < 1 || input.views.length > 8) {
    throw new Error('manager collection registration views are invalid')
  }
  ...
  assertKeys(
    input.search,
    ['fields', 'normalization', 'label', 'placeholder', 'noMatchTitle', 'noMatchDescription'],
    'manager collection search',
  )
  if (
    input.search.fields.length !== 2 || input.search.fields[0] !== 'title' || input.search.fields[1] !== 'summary'
    || input.search.normalization !== 'nfkc-casefold'
  ) throw new Error('manager collection search descriptor is invalid')
...
function assertSnapshot(
  input: ManagerCollectionSnapshotV1,
  query: ManagerCollectionQueryV1,
): ManagerCollectionSnapshotV1 {
  ...
  assertKeys(input, [
    ...
    'revision',
    'items',
  ], 'manager collection snapshot')
  if (
    input.$schema !== SNAPSHOT_SCHEMA || input.contract !== 'cordisx.manager-collection-snapshot/v1'
    || input.schemaVersion !== 1
  ) throw new Error('manager collection snapshot has an unsupported schema tuple')
  if (
    input.collectionId !== query.collectionId || input.queryRevision !== query.queryRevision
    || input.view !== query.view || input.normalizedSearch !== query.search.normalized
  ) throw new Error('manager collection snapshot query fence mismatch')
  if ([...input.normalizedSearch].length > 8_192 || CONTROL_CHARACTER.test(input.normalizedSearch)) {
    throw new Error('manager collection snapshot normalizedSearch is invalid')
  }
  if (!Number.isSafeInteger(input.revision) || input.revision < 0) {
    throw new Error('manager collection snapshot revision is invalid')
  }
  if (!Array.isArray(input.items) || input.items.length > 1_000) {
    throw new Error('manager collection snapshot items are invalid')
  }
  const ids = new Set<string>()
  for (const [index, item] of input.items.entries()) {
    ...
    assertKeys(
      item,
      ['id', 'title', 'summary', 'leadingVisual', 'route', 'order', 'disabled', 'actions'],
      `manager collection item ${index}`,
    )
    ...
    if (item.leadingVisual.kind === 'semantic-icon') {
      ...
      if (
        !Array.isArray(item.leadingVisual.entries) || item.leadingVisual.entries.length < 1
        || item.leadingVisual.entries.length > 16
      ) throw new Error(`manager collection item ${index} avatar stack is invalid`)
      ...
      for (const [entryIndex, entry] of item.leadingVisual.entries.entries()) {
        ...
        if (!OPAQUE_ID.test(entry.id) || entryIds.has(entry.id)) {
          throw new Error(`manager collection item ${index} avatar ${entryIndex} id is invalid`)
        }
    ...
    if (!Number.isSafeInteger(item.order) || item.order < -1_000_000_000 || item.order > 1_000_000_000) {
      throw new Error(`manager collection item ${index} order is invalid`)
    }
    assertDisabled(item.disabled, `manager collection item ${index} disabled`)
    if (!Array.isArray(item.actions) || item.actions.length > 8) {
      throw new Error(`manager collection item ${index} actions are invalid`)
    }
    ...
    for (const [actionIndex, action] of item.actions.entries()) {
      ...
      if (actionIds.has(action.id)) {
        throw new Error(`manager collection item ${index} has duplicate action ${action.id}`)
      }
...
function commandReference(
  action: NavigationCollectionCommandAction | ManagerCollectionTextInputAction,
  value?: string,
...
function validActionResult(
  ...
  assertKeys(input, [
    ...
    'code',
    'revision',
  ], 'manager collection action result')
  if (
    input.$schema !== RESULT_SCHEMA || input.contract !== 'cordisx.manager-collection-action-result/v1'
    || input.schemaVersion !== 1
  ) throw new Error('manager collection action result has an unsupported schema tuple')
  if (input.collectionId !== collectionId || input.itemId !== itemId || input.actionId !== actionId) {
    throw new Error('manager collection action result identity mismatch')
  }
  ...
  if (!['applied', 'rejected', 'conflict', 'unavailable'].includes(String(input.status))) {
    throw new Error('manager collection action result status is invalid')
  }
  if (input.status === 'applied') {
    if (!Number.isSafeInteger(input.revision) || Number(input.revision) < 0) {
      throw new Error('manager collection applied result revision is invalid')
    }
  } else if (input.revision !== undefined) {
    throw new Error('manager collection non-applied result cannot carry revision')
  }
...
export class HostManagerCollectionPageRegistry implements ManagerCollectionRegistryV1 {
  ...
  private version = 0
  private currentSnapshot: HostManagerCollectionSnapshot = Object.freeze({
    version: 0,
    state: 'unregistered',
    search: '',
  })
  ...
  register(
    ...
    if (this.registration !== undefined) {
      throw new Error('manager collection page registry already has an active registration')
    }
    if (
      source === null || typeof source !== 'object' || typeof source.snapshot !== 'function'
      || typeof source.subscribe !== 'function'
    ) throw new Error('manager collection source requires snapshot and subscribe functions')
    ...
    try {
      ...
      if (typeof unsubscribe !== 'function') {
        throw new Error('manager collection source subscribe must return an unsubscribe function')
      }
  ...
  snapshot(): HostManagerCollectionSnapshot {
  ...
  setView(view: string): void {
    if (
      this.registration === undefined || this.view === view
      || !this.registration.views.some(candidate => candidate.id === view)
    ) return
    this.view = view
    this.publish()
    void this.load()
  ...
  private async invoke(itemId: string, actionId: string, inputValue?: string): Promise<void> {
    ...
    const action = this.action(itemId, actionId)
    if (
      registration === undefined || source === undefined || item === undefined || action === undefined
      || item.disabled.value || action.disabled.value || this.busy !== undefined || this.disposed
    ) return
    if (action.kind === 'text-input-command') {
      const value = action.input.trim === 'both' ? (inputValue ?? '').trim() : inputValue ?? ''
      if (
        [...value].length < action.input.minLength || [...value].length > action.input.maxLength
    ...
    this.publish()
    try {
      if (this.registration !== registration || this.source !== source || this.action(itemId, actionId) === undefined) {
        return
      }
  ...
  private async load(minimumRevision?: number): Promise<boolean> {
    ...
    try {
      if (CONTROL_CHARACTER.test(this.search)) {
        throw new Error('manager collection search input contains a control character')
      }
    ...
    try {
      ...
      if (
        abort.signal.aborted || this.disposed || this.registration !== registration || this.source !== source
        || this.queryAbort !== abort
      ) return false
      const snapshot = assertSnapshot(candidate, query)
      if (this.accepted !== undefined && snapshot.revision < this.accepted.revision) {
        throw new Error('manager collection snapshot revision moved backwards')
      }
      if (minimumRevision !== undefined && snapshot.revision < minimumRevision) {
        throw new Error('manager collection action result revision is not yet authoritative')
      }
      const defensive = normalized === '' ? snapshot.items : snapshot.items.filter(item => {
        ...
        return managerCollectionCodePointIncludes(title, normalized)
          || managerCollectionCodePointIncludes(summary, normalized)
      })
      this.clearLocalizationSites()
      this.accepted = Object.freeze({
        ...snapshot,
        items: Object.freeze(
          [...defensive].sort((left, right) =>
