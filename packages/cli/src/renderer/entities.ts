...
const CLOSE_SCHEMA = 'https://raw.githubusercontent.com/cordisx/cordisx-protocol/main/schemas/entity-registry-subscription-close.v1.schema.json' as const
const PAGE_SCHEMA = 'https://raw.githubusercontent.com/cordisx/cordisx-protocol/main/schemas/entity-registry-change-page.v1.schema.json' as const
const POLL_MS = 200
...
class BrowserEntitySubscription {
  ...
  private async *iterate(): AsyncGenerator<EntityChangePage> {
    while (this.#terminal === undefined) {
      if (!this.active()) { await this.close('plugin-generation-replaced'); return }
      let result: EntityReadProjection
      try {
        result = await this.bridge.request(this.token, {
          operation: 'entity-read', subscriptionId: this.descriptor.subscriptionId,
          afterRevision: this.#cursor, replayThrough: this.descriptor.replayThrough,
        }) as EntityReadProjection
      } catch { await this.close('connection-replaced'); return }
      if (result.changes.length === 0) { await wait(); continue }
      ...
      if (changes.length === 0) { await wait(); continue }
      this.#cursor = changes[changes.length - 1]!.sequence
      yield Object.freeze({
        $schema: PAGE_SCHEMA, contract: 'cordisx.entity-registry-change-page/v1', schemaVersion: 1,
        subscription: this.descriptor, phase: replay ? 'replay' : 'live', changes: clone(changes),
        nextRevision: this.#cursor, hasMore: this.#cursor < limit,
      })
    }
  }

  async unsubscribe(): Promise<EntitySubscriptionClosed> { return await this.close('unsubscribed') }
  ...
  async close(code: EntitySubscriptionClosed['code']): Promise<EntitySubscriptionClosed> {
    ...
    const terminal = Object.freeze({
      $schema: CLOSE_SCHEMA, contract: 'cordisx.entity-registry-subscription-close/v1', schemaVersion: 1,
      subscriptionId: this.descriptor.subscriptionId, binding: clone(this.descriptor.binding), status: 'closed' as const, code,
    })
    this.#terminal = terminal
    this.#resolveClosed(terminal)
    try { await this.bridge.request(this.token, { operation: 'entity-unsubscribe', subscriptionId: this.descriptor.subscriptionId }) } catch { /* first terminal wins */ }
    return terminal
  }
}
...
export class CordisXEntityRegistryServiceV1 extends Service {
  ...
  snapshot = async (): Promise<EntityRegistrySnapshot> => {
    ...
    return clone(await this.bridge.request(this.#token, { operation: 'entity-snapshot', binding: this.binding }) as EntityRegistrySnapshot)
  }

  get = async (identity: AgentDefinitionIdentity): Promise<EntityGetResult> => {
    if (this.bridge === undefined || !this.#active()) return { status: 'unavailable', code: 'plugin-generation-replaced' }
    try { return clone(await this.bridge.request(this.#token, { operation: 'entity-get', binding: this.binding, identity }) as EntityGetResult) }
    catch { return { status: 'unavailable', code: 'host-unavailable' } }
  }

  save = async (request: EntitySaveRequest): Promise<EntitySaveResult> => {
    if (this.bridge === undefined || !this.#active()) return { status: 'unavailable', code: 'plugin-generation-replaced' }
    try { structuredClone(request); return clone(await this.bridge.request(this.#token, { operation: 'entity-save', binding: this.binding, request }) as EntitySaveResult) }
    catch { return { status: 'unavailable', code: 'host-unavailable' } }
  }
  ...
  subscribe = async (afterRevision: number): Promise<EntitySubscribeResult> => {
    if (this.bridge === undefined || !this.#active()) return { status: 'unavailable', code: 'plugin-generation-replaced' }
    try {
      const descriptor = await this.bridge.request(this.#token, { operation: 'entity-subscribe', binding: this.binding, afterRevision }) as EntitySubscription['descriptor']
