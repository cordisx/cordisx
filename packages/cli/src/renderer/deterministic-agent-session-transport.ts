...
export class DeterministicAgentSessionTransport implements CordisXPrivateAgentDriver {
  private readonly sessions = new Map<string, { nextTurn: number; active?: PlaygroundRun }>()
  ...
  async create(input: { readonly sessionId: string; readonly options: AgentOptions }): Promise<{ readonly status: 'accepted'; readonly detail: { readonly kind: 'host'; readonly ref: string } }> {
    ...
    this.sessions.set(input.sessionId, { nextTurn: 0 })
  ...
  async resume(input: { readonly sessionId: string }): Promise<{ readonly status: 'accepted'; readonly detail: { readonly kind: 'host'; readonly ref: string } } | { readonly status: 'unavailable'; readonly code: 'unsupported' }> {
    if (this.disposed || !this.sessions.has(input.sessionId)) return { status: 'unavailable', code: 'unsupported' }
    return { status: 'accepted', detail: { kind: 'host', ref: `deterministic-agent-session:${input.sessionId}` } }
  }

  async submit(input: { readonly sessionId: string; readonly message: UserMessage }): Promise<'accepted' | 'unavailable'> {
    const session = this.sessions.get(input.sessionId)
    if (this.disposed || session === undefined || session.active !== undefined) return 'unavailable'
    const run: PlaygroundRun = { turn: ++session.nextTurn, cancelled: false }
    session.active = run
  ...
  async discard(input: { readonly sessionId: string; readonly messageId: MessageId }): Promise<'accepted' | 'not-found'> {
    const session = this.sessions.get(input.sessionId)
    if (this.disposed || session?.active === undefined) return 'not-found'
    delete session.active
    this.emitStatus({ sessionId: input.sessionId, status: 'idle' })
    return 'accepted'
  }

  async cancel(input: { readonly sessionId: string; readonly cause: AgentCancelCause }): Promise<'accepted' | 'unavailable'> {
    const active = this.sessions.get(input.sessionId)?.active
    if (this.disposed || active === undefined) return 'unavailable'
    active.cancelled = true
    this.emit({ sessionId: input.sessionId, type: 'turn/end', data: { turn: active.turn, reason: { kind: 'interrupted' } } })
    delete this.sessions.get(input.sessionId)!.active
    this.emitStatus({ sessionId: input.sessionId, status: 'idle' })
    return 'accepted'
  }
  ...
  dispose(): void {
    ...
    this.disposed = true
    this.sessions.clear(); this.replacements.clear(); this.eventListeners.clear(); this.approvalListeners.clear(); this.statusListeners.clear()
  }
  ...
  private async complete(sessionId: string, run: PlaygroundRun, message: UserMessage): Promise<void> {
    ...
    if (this.sessions.get(sessionId)?.active === run) delete this.sessions.get(sessionId)!.active
