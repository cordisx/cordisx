...
const DETAILS_URL_PATTERN =
  /^(?!.*%(?:0[0-9A-F]|1[0-9A-F]|7F))(?!.*%(?![0-9A-F]{2}))(?![A-Za-z][A-Za-z0-9+.-]*:\/\/[^/?#]*@)[^\u0000-\u0020\u007F\\?#]+$/u
...
export function validateAgentLoopTaskDetailsUrl(input: AgentLoopTaskDetailsUrl): AgentLoopTaskDetailsUrl {
  ...
  if (keys.length !== 2 || keys[0] !== 'target' || keys[1] !== 'url') {
    throw new TypeError('Task details URL contains unknown fields')
  }
  if (
    typeof input.url !== 'string' || input.url.length < 5 || input.url.length > 2_048
    || !DETAILS_URL_PATTERN.test(input.url)
  ) {
  ...
  try {
    ...
    throw new TypeError('Task details URL is invalid')
  }
  ...
  if (parsed.username !== '' || parsed.password !== '') {
    throw new TypeError('Task details URL credentials are forbidden')
  }
  ...
  if (input.target === 'host') {
    ...
    if (parsed.protocol === 'https:' && parsed.hostname === '') {
      throw new TypeError('External https task details require a host')
    }
    if (
      (parsed.protocol === 'codex:' || parsed.protocol === 'claude:') && parsed.hostname === ''
      && parsed.pathname === ''
    ) {
...
export class HostAgentTaskDetailsNavigator {
  ...
  navigateAgentDetail(input: AgentDetailReference, sessionId: SessionId): void | Promise<void> {
    if (
      input === null || typeof input !== 'object' || Object.keys(input).sort().join(',') !== 'kind,ref'
