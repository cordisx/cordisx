...
export function AgentConversationRenderer(
  ...
  const identityCopy: HostAgentIdentityPanelCopy = {
    ...
    activeSessions: chinese ? '会话' : 'Sessions',
    noActiveSessions: chinese ? '暂无会话' : 'No sessions',
    sessionCount: count => chinese ? `${count} 个会话` : `${count} session${count === 1 ? '' : 's'}`,
    lifecycle: {
      active: chinese ? '激活' : 'Active',
      running: chinese ? '工作中' : 'Working',
