...
import {
  ...
  resolveOfficialOneWorksAvatarAsset,
} from './OfficialOneWorksAvatarAssets.js'
...
function acquireAgentAvatarStyles(document: Document): () => void {
  ...
  let style = document.querySelector<HTMLStyleElement>(
    `style[data-cordisx-agent-avatar-style="${AVATAR_STYLE_MARKER}"]`,
  )
  if (style === null) {
...
export class HostAgentAvatarResolver {
  ...
  constructor(
    ...
    private readonly createDefinition: (seed: string) => AvatarDefinition = seed =>
      createSeededAvatarDefinition({ name: 'CordisX Agent', seed }),
  ) {
...
export function HostAgentAvatar(
  { participant, resolver = defaultHostAgentAvatarResolver, fallback = 'initials' }: HostAgentAvatarProps,
) {
