...
type PlaygroundTaskDetailsResolverWindow = Window & {
  [PLAYGROUND_TASK_DETAILS_SESSION_RESOLVER]?: (
    input: PlaygroundTaskDetailsSessionResolverInput,
  ) => Extract<AgentConversationActiveRun, { readonly detailsUrl: unknown }>['detailsUrl']
}
...
function cloneIdentity(
  value: HostAgentDefinitionIdentityPresentation | undefined,
): HostAgentDefinitionIdentityPresentation | undefined {
  if (value === undefined) return undefined
  if (
    value === null || typeof value !== 'object' || !AGENT_ID_PATTERN.test(value.agentId)
    || !HANDLE_PATTERN.test(value.revision)
  ) {
    throw new TypeError('Agent identity is invalid')
  }
  if (Object.keys(value).sort().join(',') !== 'agentId,revision') {
    throw new TypeError('Agent identity contains unknown fields')
  }
  return Object.freeze({ agentId: value.agentId, revision: value.revision })
}

export function createHostAgentIdentityPresentation(
  input: HostAgentIdentityPresentation,
): HostAgentIdentityPresentation {
  if (input === null || typeof input !== 'object') throw new TypeError('Agent identity presentation must be an object')
  if (input.participant === null || typeof input.participant !== 'object') {
    throw new TypeError('Agent participant is invalid')
  }
  ...
  const activeSessions = input.activeSessions.map((session, index) => {
    ...
    if (session.run === null || typeof session.run !== 'object') {
      throw new TypeError(`Active session ${index} run is invalid`)
    }
    const sessionParticipantId = boundedHandle(session.run.participantId, `Active session ${index} participant id`)
    if (sessionParticipantId !== participantId) {
      throw new TypeError(`Active session ${index} crosses participant identity`)
    }
    ...
    keys.add(key)
    if (!['active', 'running', 'waiting', 'attention'].includes(session.run.lifecycle.phase)) {
      throw new TypeError(`Active session ${index} lifecycle is invalid`)
    }
    const run = !('sessionId' in session.run)
      ? Object.freeze({
        ...
        lifecycle: Object.freeze({
          ...
          ...(session.run.lifecycle.updatedAt === undefined
            ? {}
            : {
              updatedAt: boundedText(session.run.lifecycle.updatedAt, `Active session ${index} lifecycle time`, 64),
            }),
        }),
      ...
      : Object.freeze({
        ...
        ...(session.run.details === undefined
          ? {}
          : {
            details: Object.freeze({
              kind: session.run.details.kind,
              ref: boundedHandle(session.run.details.ref, `Active session ${index} detail ref`),
            }),
          }),
      })
...
export function projectHostAgentIdentityFromShell(
  ...
  if (
    participant.agentIdentity.agentId !== identity.agentId || participant.agentIdentity.revision !== identity.revision
  ) {
...
export function HostAgentIdentityAvatarButton(
  { presentation: input, label, onOpen }: HostAgentIdentityAvatarButtonProps,
) {
  ...
  return (
    <button type="button" className="cx-agent-identity-avatar-button" aria-label={label} onClick={onOpen}>
      {avatar}
    </button>
  )
}
    ...
    <>
      ...
      <div className="cx-agent-identity-body">
        <section className="cx-agent-identity-hero" aria-labelledby={`${contentId}-name`}>
          <span className="cx-agent-identity-avatar-seat">
          ...
          <button
            ...
            aria-describedby={!settingsAvailability.available && settingsAvailability.reason !== undefined
              ? settingsReasonId
              : undefined}
            onClick={() => {
