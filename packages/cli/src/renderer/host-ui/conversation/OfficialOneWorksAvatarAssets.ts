...
const OFFICIAL_ONEWORKS_AVATAR_DEFINITIONS = new Map<string, AvatarDefinition>([
  [HOST_ONEWORKS_RED_FOX_AVATAR_ASSET_REF, parseAvatarDefinition(JSON.parse(RED_FOX_EDITOR_EXPORT))],
  [HOST_ONEWORKS_ARCTIC_FOX_AVATAR_ASSET_REF, parseAvatarDefinition(JSON.parse(ARCTIC_FOX_EDITOR_EXPORT))],
])
...
export function resolveOfficialOneWorksAvatarAsset(ref: string, revision: string | undefined): AvatarDefinition | undefined {
  const expectedRevision = ref === HOST_ONEWORKS_RED_FOX_AVATAR_ASSET_REF
    ...
    : ref === HOST_ONEWORKS_ARCTIC_FOX_AVATAR_ASSET_REF
      ...
      : undefined
  return expectedRevision === revision ? OFFICIAL_ONEWORKS_AVATAR_DEFINITIONS.get(ref) : undefined
}
