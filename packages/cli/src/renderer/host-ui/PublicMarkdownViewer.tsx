...
export function PublicMarkdownViewer({ source, className, 'aria-label': ariaLabel }: PublicMarkdownViewerProps) {
  ...
  const components = useMemo(() => ({
    ...
    code: ({ className: codeClassName, children, node: _node, ...props }: ComponentPropsWithoutRef<'code'> & { readonly node?: unknown }) => {
      const language = /(?:^|\s)language-([^\s]+)/u.exec(codeClassName ?? '')?.[1]
      return <code {...props} className={codeClassName} {...(language === undefined ? {} : { 'data-language': language })}>{children}</code>
    },
  ...
  return <article ref={article} className={joinClassName('cxr-ui-markdown', 'cxm-readme', className)} aria-label={ariaLabel}>
    <ReactMarkdown
