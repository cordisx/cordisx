...
import { hostSurfaceIconKey, normalizedVectorCommandData, resolveBuiltinHostIcon } from '../icons.js'
...
export function HostSurfaceIcon({ token }: { readonly token: CordisXIconToken }) {
  const { descriptor, resolution } = resolveBuiltinHostIcon(hostSurfaceIconKey(token) ?? token)
