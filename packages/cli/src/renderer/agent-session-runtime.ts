...
const PAGE_TARGET_ORIGIN_SCHEMA =
...
const PAGE_TARGET_RECEIPT_SCHEMA =
...
const PAGE_ROUTE_CONTINUATION_SCHEMA =
...
const PAGE_ROUTE_CLAIM_RECEIPT_SCHEMA =
...
const PAGE_FRESH_NAVIGATION_SCHEMA =
...
interface PageAdmissionCommandRecord {
  readonly owner: PluginOwnerIdentity
...
interface PageAdmissionTargetRecord {
  readonly owner: PluginOwnerIdentity
  readonly command: PageAdmissionCommand
  readonly origin: AgentPageComposerOrigin
...
export class CordisXAgentSessionRuntime {
  ...
  private readonly entityMutations = new Map<
    string,
  ...
  private readonly legacyResolvers = new Map<
    string,
    { readonly token: object; readonly resolve: CordisXLegacyAgentLoopBindingResolver }
  >()
  private readonly legacyMutations = new Map<
    string,
  ...
  private readonly targetAdmissionOrigins = new Map<
    string,
    Readonly<{
      owner: PluginOwnerIdentity
      origin: AgentCommandOrigin
  ...
  private readonly bootstrapAdmissionTargets = new Map<
    string,
    Readonly<{
      owner: PluginOwnerIdentity
      origin: AgentBootstrapCommandOrigin
  ...
  private readonly bootstrapAdmissionRoomTargets = new Map<
    string,
    Readonly<{
      owner: PluginOwnerIdentity
      origin: AgentBootstrapCommandOrigin
  ...
  private readonly bootstrapAdmissionRouteContinuations = new Map<
    string,
    Readonly<{
      owner: PluginOwnerIdentity
      origin: AgentBootstrapCommandOrigin
  ...
  /** Page-scoped admission is distinct from frozen Shell v3-v6 capabilities. */
  private readonly pageAdmissionCommands = new Map<string, PageAdmissionCommandRecord>()
  private readonly pageAdmissionTargets = new Map<string, PageAdmissionTargetRecord>()
  ...
  private async reservePageAdmission(
    owner: PluginOwnerIdentity,
  ...
  private validPageTarget(target: AgentPageAdmissionTarget | undefined): target is AgentPageAdmissionTarget {
    return target !== undefined && opaque(target.roomId) && opaque(target.participantId)
      && opaque(target.memberId) && opaque(target.runId)
  }

  private validPageRouteTarget(
  ...
  private pageDestination(route: AgentPageRoomRoute): HostPageAdmissionDestinationRoute {
    return {
      outlet: route.outlet,
  ...
  private samePageRoomRoute(route: AgentPageRoomRoute, command: PageAdmissionCommandRecord): boolean {
  ...
  private pageDeniedDeliveryCode(
    code: string,
  ...
  private validBootstrapAdmissionOrigin(
  ...
  private bootstrapTargetIssueKey(
    owner: PluginOwnerIdentity,
    origin: AgentBootstrapCommandOrigin,
    target: AgentAdmissionTarget,
  ): string {
  ...
  private validBootstrapRoomTarget(
  ...
  private sameBootstrapRoomTarget(
  ...
  private validBootstrapRouteTarget(
  ...
  private sameBootstrapRouteTarget(
  ...
  private bootstrapRouteIssueKey(
    owner: PluginOwnerIdentity,
    origin: AgentBootstrapCommandOrigin,
    target: AgentAdmissionBootstrapRouteTarget,
  ): string {
  ...
  /** Host-only destination activation hook invoked by NavigationRegistry before page mount. */
  ...
  private async append<K extends SessionEvent['type']>(
  ...
  private emitLive<K extends AgentLiveEvent['type']>(
    record: AgentRecord,
    type: K,
  ...
  private answererKey(record: AgentRecord): string {
    return `${record.id}\u0000${record.generation}`
  }
  private acquireDenied(operation: 'create' | 'resume', mutationId?: string): AgentAcquireResult {
    return {
      $schema: ACQUIRE_SCHEMA,
  ...
  private remember(key: string | undefined, fingerprint: string, result: AgentAcquireResult): AgentAcquireResult {
  ...
  private approvalBinding(record: AgentRecord): ApprovalAgentBinding {
    return Object.freeze({
      agentId: record.id,
      sessionId: record.id,
      agentGeneration: record.generation,
...
/** Public page-scoped exact target issuer; it never accepts a Shell origin. */
...
/** Public page-scoped one-shot reservation for one exact existing Room target. */
...
/** Public fresh-Room declaration. Only the Host may later claim the destination page binding. */
...
/** Public fresh-Room one-shot reservation; no driver or route claim leaks to the plugin. */
...
/** Fresh Room navigation is a Host-bound permit emitted only in a v2 page command context. */
