...
export const CORDISX_MANAGER_EXTENSION_POINT_CATALOG = Object.freeze({
  ...
  points: Object.freeze([
    Object.freeze({
      ...
      description: Object.freeze({ namespace: DESCRIPTOR_NAMESPACE, key: 'manager.settings.tabs.description', fallback: 'Switches Marketplace and plugin settings content inside the Host-owned Settings page.' }),
      icon: 'host:settings',
    ...
    Object.freeze({
      ...
      description: Object.freeze({ namespace: DESCRIPTOR_NAMESPACE, key: 'manager.settings.navigation-items.description', fallback: 'Adds an independent Host-rendered plugin destination immediately before or after Settings in the Manager navigation.' }),
      icon: 'host:layers',
...
const ZH_MESSAGES: Readonly<Record<string, string>> = {
  ...
  'manager.settings.tabs.description': '在宿主拥有的配置页内切换插件商店与插件配置内容。',
  ...
  'manager.settings.navigation-items.description': '在管理器左侧配置入口前后添加由宿主渲染的独立插件页面入口。',
