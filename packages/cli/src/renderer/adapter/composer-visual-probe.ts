...
export function probeComposerVisual(document: Document): NativeComposerVisualProbe {
  ...
  if (footers.length !== 1) return { status: 'unavailable', reason: 'Composer footer is ambiguous or absent' }
  ...
  const editors = [...frame.querySelectorAll<HTMLElement>('[contenteditable="true"][role="textbox"], textarea')]
    .filter(visible)
