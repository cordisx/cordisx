...
function clone<Value>(value: Value): Value { return freeze(structuredClone(value)) }

function bindingV4(binding: AgentLoopTaskBindingV3): AgentLoopTaskBindingV4 {
  return clone({ ...binding, $schema: CORDISX_AGENT_LOOP_TASK_BINDING_SCHEMA_V4, contract: 'cordisx.agent-loop-task-binding/v4' as const, schemaVersion: 4 as const })
}

function bindingV3(binding: AgentLoopTaskBindingV4): AgentLoopTaskBindingV3 {
  return clone({ ...binding, $schema: CORDISX_AGENT_LOOP_TASK_BINDING_SCHEMA_V3, contract: 'cordisx.agent-loop-task-binding/v3' as const, schemaVersion: 3 as const })
}

function commandV4(command: AgentLoopCommandV3): AgentLoopCommandV4 {
  const base = { ...command, $schema: CORDISX_AGENT_LOOP_COMMAND_SCHEMA_V4, contract: 'cordisx.agent-loop-command/v4' as const, schemaVersion: 4 as const }
  if (command.type === 'create-or-bind') return clone(base as AgentLoopCommandV4)
  if (command.type === 'approval-decision') return clone({
    ...base,
...
function resultV3(result: AgentLoopResultV4): AgentLoopResultV3 {
  const output: Record<string, unknown> = { ...clone(result), $schema: CORDISX_AGENT_LOOP_RESULT_SCHEMA_V3, contract: 'cordisx.agent-loop-result/v3', schemaVersion: 3 }
...
function eventV3(event: AgentLoopEventV4): AgentLoopEventV3 {
  return clone({ ...event, $schema: CORDISX_AGENT_LOOP_EVENT_SCHEMA_V3, contract: 'cordisx.agent-loop-event/v3' as const, schemaVersion: 3 as const } as AgentLoopEventV3)
}
...
function pageV3(page: AgentLoopEventPageV4): AgentLoopEventPageV3 {
  return clone({
    ...
    subscription: { ...page.subscription, $schema: CORDISX_AGENT_LOOP_EVENT_SUBSCRIPTION_SCHEMA_V3, contract: 'cordisx.agent-loop-event-subscription/v3' as const, schemaVersion: 3 as const },
...
function subscribeV3(result: AgentLoopSubscribeRuntimeResultV4): AgentLoopSubscribeRuntimeResultV3 {
  ...
  return Object.freeze({
    ...
    handle: Object.freeze({
      subscription: clone({ ...source.subscription, $schema: CORDISX_AGENT_LOOP_EVENT_SUBSCRIPTION_SCHEMA_V3, contract: 'cordisx.agent-loop-event-subscription/v3' as const, schemaVersion: 3 as const }),
      pages: Object.freeze({ async *[Symbol.asyncIterator]() { for await (const page of source.pages) yield pageV3(page) } }),
...
export function adaptAgentLoopV3(client: BoundAgentLoopClientV4): BoundAgentLoopClientV3 {
  const adapted: BoundAgentLoopClientV3 = {
    ...
    createOrBind: async command => resultV3(await client.createOrBind(commandV4(command) as Extract<AgentLoopCommandV4, { type: 'create-or-bind' }>)) as Awaited<ReturnType<BoundAgentLoopClientV3['createOrBind']>>,
    send: async command => resultV3(await client.send(commandV4(command) as Extract<AgentLoopCommandV4, { type: 'send' }>)) as Awaited<ReturnType<BoundAgentLoopClientV3['send']>>,
    decideApproval: async command => resultV3(await client.decideApproval(commandV4(command) as Extract<AgentLoopCommandV4, { type: 'approval-decision' }>)) as Awaited<ReturnType<BoundAgentLoopClientV3['decideApproval']>>,
    requestMemberSelfIntroduction: async command => resultV3(await client.requestMemberSelfIntroduction(commandV4(command) as Extract<AgentLoopCommandV4, { type: 'request-member-self-introduction' }>)) as Awaited<ReturnType<BoundAgentLoopClientV3['requestMemberSelfIntroduction']>>,
    cancelMemberSelfIntroduction: async command => resultV3(await client.cancelMemberSelfIntroduction(commandV4(command) as Extract<AgentLoopCommandV4, { type: 'cancel-member-self-introduction' }>)) as Awaited<ReturnType<BoundAgentLoopClientV3['cancelMemberSelfIntroduction']>>,
