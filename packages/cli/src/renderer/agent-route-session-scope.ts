...
export interface AgentRuntimePermissionDeclaration {
  ...
  readonly scope: Readonly<{ readonly sessionIds?: readonly string[] | AgentRouteScopeBinding; readonly authorityRequester?: AgentApprovalAuthorityRequesterScope }>
}
...
export class AgentRouteSessionScopeAuthority {
  ...
  async mintApprovalAuthorityLease(owner: PluginOwnerIdentity, request: AgentApprovalAuthorityRouteRequest): Promise<PluginApprovalAuthorityLeaseV8 | undefined> {
    ...
    const active = this.options.activeRoute()
    if (active === undefined || !sameOwner(active.owner, owner) || active.routeId !== scope.requester.routeId) return undefined
    const routeSessionId = active.params[scope.requester.param]
    if (typeof routeSessionId !== 'string' || routeSessionId !== request.requester.sessionId || request.requester.agentId !== routeSessionId
      || request.authority.sessionId === request.requester.sessionId || request.authority.agentId !== request.authority.sessionId) return undefined
    ...
    const decision = await this.decide(owner, 'approvals.answer', [request.authority.sessionId], {
      kind: 'host-route', routeId: scope.requester.routeId, routeInstanceId: active.instanceId, path: route.path, params: { sessionId: routeSessionId },
    })
    if (!decision.authorized) return undefined
    if (this.options.isLeaseActive?.(owner, decision.leaseId ?? '') === false) return undefined
    const correlation = Object.freeze({
      $schema: 'https://raw.githubusercontent.com/cordisx/cordisx-protocol/main/schemas/approval-authority-route-correlation.v1.schema.json' as const,
      contract: 'cordisx.approval-authority-route-correlation/v1' as const, schemaVersion: 1 as const,
      routingId: request.routingId, registrationId: request.registrationId, owner: Object.freeze({ ...owner }),
      route: Object.freeze({ routeId: scope.requester.routeId, param: 'sessionId' as const, sessionId: routeSessionId }),
      requester: Object.freeze(structuredClone(request.requester)), authority: Object.freeze(structuredClone(request.authority)),
    })
    const lease = Object.freeze({
      $schema: 'https://raw.githubusercontent.com/cordisx/cordisx-protocol/main/schemas/approval-authority-route-lease.v1.schema.json' as const,
      contract: 'cordisx.approval-authority-route-lease/v1' as const, schemaVersion: 1 as const,
      leaseId: crypto.randomUUID(), correlation,
    })
    this.approvalAuthorityLeases.set(lease.leaseId, { lease, owner: Object.freeze({ ...owner }), routeParam: 'sessionId', routeInstanceId: active.instanceId, connectionGeneration: this.options.connectionGeneration() })
    return lease
  }

  requiresApprovalAuthorityLease(owner: PluginOwnerIdentity): boolean {
    const installed = this.declarations.get(owner.pluginId)?.find(item => item.declaration.name === 'approvals.answer')
    return installed?.owner.generation === owner.generation && installed.declaration.scope.authorityRequester !== undefined
  }

  approvalAuthorityLeaseActive(owner: PluginOwnerIdentity, lease: PluginApprovalAuthorityLeaseV8, requester: ApprovalAgentBinding, authority: ApprovalAgentBinding): boolean {
    const stored = this.approvalAuthorityLeases.get(lease.leaseId)
    if (stored === undefined || stored.lease !== lease || !sameOwner(stored.owner, owner)
      ...
      || stored.connectionGeneration !== this.options.connectionGeneration()) return false
    const active = this.options.activeRoute()
    return active !== undefined && sameOwner(active.owner, owner) && active.instanceId === stored.routeInstanceId
      && active.routeId === lease.correlation.route.routeId && active.params[stored.routeParam] === lease.correlation.route.sessionId
  }

  releaseApprovalAuthorityLease(lease: PluginApprovalAuthorityLeaseV8): void { this.approvalAuthorityLeases.delete(lease.leaseId) }
  ...
  private validate(owner: PluginOwnerIdentity, declaration: AgentRuntimePermissionDeclaration): InstalledDeclaration {
    ...
    return Object.freeze({ owner: Object.freeze({ ...owner }), declaration: Object.freeze({ ...declaration, scope: Object.freeze({
      ...(scope === undefined ? {} : { sessionIds: Array.isArray(scope) ? Object.freeze([...scope]) : Object.freeze({ ...scope }) }),
      ...(authorityRequester === undefined ? {} : { authorityRequester: Object.freeze({ kind: authorityRequester.kind, requester: Object.freeze({ ...authorityRequester.requester }) }) }),
    }) }) })
  }
  ...
  private fence(owner: string, code: Exclude<AgentRouteFenceCode, 'route-replaced'>): void {
    ...
    for (const [leaseId, record] of this.approvalAuthorityLeases) if (record.owner.pluginId === owner) this.approvalAuthorityLeases.delete(leaseId)
  }

  private sameBindingOwner(owner: PluginOwnerIdentity, request: AgentApprovalAuthorityRouteRequest): boolean {
    return request.requester.agentId === request.requester.sessionId && request.authority.agentId === request.authority.sessionId
