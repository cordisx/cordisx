...
export const ICON_STATES = ['default', 'hover', 'active', 'selected', 'disabled', 'danger', 'success', 'warning'] as const
...
export type NormalizedVectorCommand =
  ...
  | { readonly op: 'cubic'; readonly x1: number; readonly y1: number; readonly x2: number; readonly y2: number; readonly x: number; readonly y: number }
...
export type NormalizedVectorPath =
  | { readonly paint: 'fill'; readonly fillRule?: 'nonzero' | 'evenodd'; readonly opacity?: number; readonly commands: readonly NormalizedVectorCommand[] }
  | { readonly paint: 'stroke'; readonly strokeWidth: number; readonly lineCap: 'butt' | 'round' | 'square'; readonly lineJoin: 'miter' | 'round' | 'bevel'; readonly opacity?: number; readonly commands: readonly NormalizedVectorCommand[] }
...
export function isNormalizedVectorDescriptor(value: unknown): value is NormalizedVectorDescriptor {
  ...
  if (!exactKeys(descriptor, ['format', 'formatVersion', 'viewBox', 'paths'])
    ...
    || !exactKeys(descriptor.viewBox as object, ['minX', 'minY', 'width', 'height'])) return false
  const viewBox = descriptor.viewBox as Record<string, unknown>
  if (viewBox.minX !== 0 || viewBox.minY !== 0 || viewBox.width !== 24 || viewBox.height !== 24
    || !Array.isArray(descriptor.paths) || descriptor.paths.length < 1 || descriptor.paths.length > 16) return false
  return descriptor.paths.every(pathValue => {
    ...
    if (!fill && (!boundedNumber(path.strokeWidth, 0.25, 4)
      || !['butt', 'round', 'square'].includes(String(path.lineCap))
      || !['miter', 'round', 'bevel'].includes(String(path.lineJoin)))) return false
    ...
    return commands.every((commandValue, index) => {
      ...
      if (command.op === 'move' || command.op === 'line') {
        return exactKeys(command, ['op', 'x', 'y']) && boundedNumber(command.x, -64, 64) && boundedNumber(command.y, -64, 64)
      }
      ...
      if (command.op === 'cubic') {
        return exactKeys(command, ['op', 'x', 'x1', 'x2', 'y', 'y1', 'y2'])
          && [command.x1, command.y1, command.x2, command.y2, command.x, command.y].every(item => boundedNumber(item, -64, 64))
      }
      return false
    })
  })
