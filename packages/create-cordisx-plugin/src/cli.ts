...
async function renderPluginSource(
  ...
  await writeFile(path.join(destination, 'README.md'), `# ${id}\n\nThe local-development lifecycle entry is \`src/index.tsx\`. React page components live in component-only modules such as \`src/overview-page.tsx\` so Vite can apply React Fast Refresh. Production builds write one immutable Vite ESM graph with \`module.js\`, content-addressed chunks, CSS, assets, and Vite metadata at \`dist/manifest.json\`. A portable package entry points at \`dist/module.js\`; the Host synthesizes the separate store-level \`artifact.json\`.\n\nThis plugin starts private and \`UNLICENSED\`. Its generated source is Marked Template Material under the CordisX Independent Plugin Exception; choose a license before distribution.\n`, { encoding: 'utf8', flag: 'wx' })
  await writeFile(path.join(destination, 'README.zh-Hans.md'), `# ${id}\n\n本地开发的生命周期入口为 \`src/index.tsx\`。React 页面组件位于 \`src/overview-page.tsx\` 等纯组件模块中，因此 Vite 可以应用 React Fast Refresh。生产构建会在 \`dist/\` 下生成独立、不可变的 Vite ESM graph，包括 \`module.js\`、带内容摘要的 chunk、CSS、静态资源和 Vite 元数据 \`manifest.json\`。可移植 package 的入口指向 \`dist/module.js\`；Host 会另行生成存储层 \`artifact.json\`。\n\n此插件默认私有且使用 \`UNLICENSED\`。生成源码属于 CordisX 独立插件例外中的已标记模板材料；分发前请选择许可证。\n`, { encoding: 'utf8', flag: 'wx' })
}
