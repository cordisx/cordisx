...
const FIXTURE_STATUS: TraceAdapterStatus = Object.freeze({
  ...
  diagnostics: Object.freeze([
    ...
    'Public protocol is pinned; the compatible host provider is still pending.',
...
function contractTypeForSemantic(semanticType: string): TraceContractEventType {
  ...
  if (semanticType.startsWith('agent.pre-step') || semanticType.startsWith('system-prompt.')) return 'diagnostic'
...
function buildFixtureLedger(sessionId: string): TraceEvent[] {
  ...
  return [
    ...
    base(2, {
      turnId: 'turn-7', turnNumber: 7, stepId: 'turn-7-step-1', stepNumber: 1,
      requestId: 'pre-step-7', lane: 'injection', semanticType: 'agent.pre-step.append', truth: 'cordisx', phase: 'requested',
      summary: 'Pre-step append requested by Agent Trace Showcase.', source: PLUGIN_SOURCE, plugin: SHOWCASE_PLUGIN,
    ...
    base(5, {
      turnId: 'turn-7', turnNumber: 7, stepId: 'turn-7-step-1', stepNumber: 1,
      requestId: 'prompt-section-7', lane: 'injection', semanticType: 'system-prompt.section', truth: 'cordisx', phase: 'projected',
      summary: 'Named release-safety system prompt section projected.', source: PLUGIN_SOURCE, plugin: SHOWCASE_PLUGIN,
      modelConsumption: 'unproved', payload: { section: 'release-safety', persistence: 'generation' },
    }),
    base(6, {
      turnId: 'turn-7', turnNumber: 7, stepId: 'turn-7-step-1', stepNumber: 1,
...
export class FixtureTraceShowcaseStore implements TraceShowcaseStore {
  ...
  async clearQueued(): Promise<number> {
    ...
    const queued = [...latestByRequest.values()].filter(event => event.phase === 'queued' && event.plugin?.id === SHOWCASE_PLUGIN.id)
    for (const event of queued) {
      const kind = demoKindForSemanticType(event.semanticType)
      if (kind === undefined) continue
      this.append(event.requestId!, kind, 'cancelled', 'Queued fixture contribution cleared by the plugin owner.', {
  ...
  private append(
    ...
    const turnNumber = 9
    const semanticType = kind === 'pre-step'
