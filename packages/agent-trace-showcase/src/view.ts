...
export function mountTraceShowcase(
  ...
  const renderData = (): void => {
    ...
    if (snapshot.status.completeness === 'unavailable') {
      ...
      for (const turn of groupTraceEvents(rendered)) {
        ...
        const turnCell = create(document, 'td', undefined, turn.turnNumber === undefined ? 'Between turns' : `Turn ${turn.turnNumber}`)
        turnCell.colSpan = 4
        ...
        for (const step of turn.steps) {
          ...
          const stepCell = create(document, 'td', undefined, step.stepNumber === undefined ? 'Unscoped' : `Step ${step.stepNumber}`)
          stepCell.colSpan = 4
    ...
    if (selected === undefined) {
      ...
      addDetail(document, list, 'Plugin', selected.plugin === undefined ? undefined : `${selected.plugin.id}@${selected.plugin.version}`)
  ...
  demos.append(create(document, 'span', 'cxt-demos-label', 'Explicit fixture actions'))
  for (const demo of DEMOS) {
    ...
    button.title = snapshot.status.mode === 'fixture'
      ...
      : 'Requires the pinned public Agent contract and Permission Broker'
    button.addEventListener('click', () => {
  ...
  clear.type = 'button'
  clear.disabled = snapshot.status.mode !== 'fixture'
