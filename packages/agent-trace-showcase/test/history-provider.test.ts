...
function page(
  ...
  return {
    ...
    sessionId: 'session-a',
    snapshotId: 'snapshot-opaque-0001',
    limit: 500,
    requestedPayloadPolicy: 'summarized',
    effectivePayloadPolicy: 'summarized',
    source: {
      kind: 'historical',
      adapterId: 'codex-jsonl',
      adapterVersion: '1.0.0',
      hostId: 'codex-desktop-history',
    ...
    coverage: {
      ...
      oversizedLines: 0,
      redactedFields: 2,
      tailAvailable: input.tailCursor !== undefined,
    },
  ...
  getSnapshot(): TraceSnapshot {
    return {
      ...
      status: {
        ...
        diagnostics: ['Current connection unavailable.'],
        supportedOperations: [],
...
describe('Agent Trace historical provider', () => {
  ...
  it('prefers a native-id live observation and assigns one continuous display sequence', () => {
    const historical = projectAgentEvent(
      item({
        id: 'historical-id',
        seq: 99,
        time: 1_000,
  ...
  it('enforces one 500-row merged UI boundary for oversized imported windows', () => {
    const imported = Array.from({ length: 620 }, (_, index) =>
      projectAgentEvent(
        item({
          id: `large-${index}`,
          seq: index,
  ...
  it('moves through opaque earlier pages and incrementally tails without duplicates', async () => {
    const newest = page([item({ id: 'hist-new', seq: 2, time: 3_000, itemId: 'new' })], {
      nextCursor: 'opaque-earlier-cursor-0001',
    })
    ...
    const tailStart = page([item({ id: 'tail-old', seq: 0, time: 1_000, itemId: 'old' })], {
      tailCursor: 'opaque-tail-cursor-0001',
    })
    const tailNext = page([
  ...
  it('fails closed on history denial while retaining the independently sourced live status', async () => {
    ...
    expect(store.getSnapshot()).toMatchObject({
      events: [],
      status: { origins: ['live'], mode: 'partial' },
    })
