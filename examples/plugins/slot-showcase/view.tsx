...
export function ShowcasePage(props: CordisXReactPageProps<Messages>) {
    ...
    return (
      ...
      >
        <div style={{ display: 'grid', justifyItems: 'center', gap: '22px' }}>
          ...
          >
            <img
              className="cordisx-welcome-mark-dark"
              src={cordisxMarkDarkUri}
              alt=""
              style={{ gridArea: '1 / 1', width: '100%', height: '100%' }}
            />
            <img
              className="cordisx-welcome-mark-light"
              src={cordisxMarkLightUri}
              alt=""
              style={{ gridArea: '1 / 1', width: '100%', height: '100%' }}
            />
          </span>
        ...
        `}
          ...
          <div style={{ display: 'grid', gap: '10px' }}>
            <span style={{ color: 'var(--color-text-secondary, #8490a3)', fontSize: '14px' }}>
              {props.t('page.welcome.eyebrow')}
            </span>
  ...
  const body = props.routeId.endsWith(':app.overview')
    ? props.t('page.app.body')
    : props.routeId.endsWith(':main.analytics')
    ? props.t('page.main.body')
    : props.t('page.session.body', { sessionId: String(props.params.sessionId) })
  return (
    <Card as="article" data-cordisx-demo-marker={props.outlet}>
      <Stack gap="medium">
