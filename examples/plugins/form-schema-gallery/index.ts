...
const group = (id: string, title: string, titleZh: string, icon: string) => ({
  group: { id, title: { en: title, 'zh-CN': titleZh }, icon },
})
...
const fields = {
  ...
  welcomeNote: Schema.string().default('Keep shared work clear, current, and respectful.').max(400).role('multiline')
    ...
    .i18n(
      description('Optional guidance shown when someone joins the workspace.', '新成员加入工作区时显示的可选说明。'),
    ),
  handoffNote: Schema.string().max(240).role('multiline')
    ...
    .i18n(
      description(
        'Optional and intentionally empty until the workspace is handed over.',
        '可选字段，在工作区交接前保持为空。',
      ),
    ),
  ...
  exportDirectory: Schema.string().default('/Users/demo/Documents/Northstar exports').max(240).role('directory')
    .extra(
      'extra',
      label('Export folder', '导出文件夹', {
        icon: 'host:folder',
        ...group('storage', 'Storage', '存储', 'host:folder'),
      }),
    )
    .description('Choose where privacy-safe exports are prepared on this device.')
    .i18n(
      description(
        'Choose where privacy-safe exports are prepared on this device.',
        '选择在此设备上准备隐私安全导出的位置。',
      ),
    ),
  maxParallelJobs: Schema.natural().default(4).min(1).max(16).step(1)
    .extra(
      'extra',
      label('Parallel tasks', '并行任务数', {
        presenter: { version: 1, kind: 'number.stepper', options: { density: 'compact' } },
        ...group('performance', 'Performance', '性能', 'host:settings'),
      }),
    )
    .description('Limit background work so the workspace stays responsive.')
    .i18n(
      description('Limit background work so the workspace stays responsive.', '限制后台任务数量，让工作区保持流畅。'),
    ),
  reviewThreshold: Schema.number().default(0.75).min(0.5).max(1).step(0.05).role('slider')
    .extra(
      'extra',
      label('Review threshold', '审核阈值', {
        presenter: { version: 1, kind: 'number.slider', options: { density: 'compact' } },
        ...group('performance', 'Performance', '性能', 'host:settings'),
      }),
    )
    ...
    .i18n(
      description(
        'Items at or above this confidence level can be queued for review.',
        '达到此置信度的项目可进入审核队列。',
      ),
    ),
  showMemberAvatars: Schema.boolean().default(true)
    .extra(
      'extra',
      label('Show member avatars', '显示成员头像', group('appearance', 'Appearance', '外观', 'host:palette')),
    )
  ...
  releaseTrack: Schema.union([Schema.const('stable'), Schema.const('preview'), Schema.const('early-access')]).default(
    'stable',
  )
    .extra(
      'extra',
      label('Release track', '发布通道', {
        presenter: { version: 1, kind: 'choice.select' },
        ...group('workspace', 'Workspace', '工作区', 'host:settings'),
      }),
    )
  ...
  approvalMode: Schema.union([Schema.const('manual'), Schema.const('team-lead'), Schema.const('automatic')]).default(
    'manual',
  ).role('radio')
    .extra(
      'extra',
      label('Approval mode', '审批方式', {
        presenter: { version: 1, kind: 'choice.radio', options: { density: 'compact' } },
        ...group('collaboration', 'Collaboration', '协作', 'host:tags'),
      }),
    )
  ...
  reviewMode: Schema.union([Schema.const('guided'), Schema.const('focused'), Schema.const('automatic')]).default(
    'guided',
  ).role('radio')
    .extra(
      'extra',
      label('Review mode', '审核模式', {
    ...
    .i18n(
      description(
        'Choose the same finite enum through the compact segmented presenter.',
        '使用紧凑分段展示器选择同一类有限枚举。',
      ),
    ),
  preferredReviewDate: Schema.string().default('2026-09-01').role('date')
    .extra(
      'extra',
      label('Preferred review date', '首选审核日期', {
        icon: 'host:calendar',
        ...group('schedule', 'Schedule', '日程', 'host:clock'),
      }),
    )
  ...
  dailyQuietTime: Schema.string().default('18:30').role('time')
    .extra(
      'extra',
      label('Daily quiet time', '每日免打扰时间', {
        icon: 'host:clock',
        ...group('schedule', 'Schedule', '日程', 'host:clock'),
      }),
    )
  ...
  accentColor: Schema.string().default('#476a9c').role('color')
    .extra(
      ...
      label('Accent color', '强调色', {
        icon: 'host:palette',
        ...group('appearance', 'Appearance', '外观', 'host:palette'),
      }),
    )
    .description('Choose a recognizable accent, or enter an exact HEX value.')
    .i18n(
      description(
        'Choose a recognizable accent, or enter an exact HEX value.',
        '选择常用强调色，或输入精确的 HEX 值。',
      ),
    ),
  audienceTags: Schema.array(
    Schema.union([
      Schema.const('design'),
      Schema.const('research'),
      Schema.const('operations'),
      Schema.const('community'),
    ]),
  )
    .default(['design', 'research']).min(1).max(5).role('multi-select')
    .extra(
      'extra',
      label('Audience tags', '受众标签', {
        icon: 'host:tags',
        ...group('collaboration', 'Collaboration', '协作', 'host:tags'),
      }),
    )
    ...
    .i18n(
      description('Choose the audiences that can receive this workspace update.', '选择可接收此工作区更新的受众。'),
    ),
  quickLabels: Schema.array(Schema.string().min(1).max(24)).default(['weekly', 'planning']).min(0).max(6)
    .extra(
      'extra',
      label('Quick labels', '快捷标签', {
        icon: 'host:tags',
        presenter: { version: 1, kind: 'array.scalar-tags', options: { maxInlineItems: 6 } },
        ...group('collaboration', 'Collaboration', '协作', 'host:tags'),
      }),
    )
  ...
  reminderChannels: Schema.array(Schema.boolean()).default([true]).min(1).max(2)
    .extra(
      'extra',
      label('Reminder channels', '提醒渠道', {
        icon: 'host:tags',
        presenter: { version: 1, kind: 'array.scalar-rows', options: { maxInlineItems: 2 } },
        ...group('collaboration', 'Collaboration', '协作', 'host:tags'),
      }),
    )
    ...
    .i18n(
      description(
        'Choose the bounded on/off delivery channels for this workspace.',
        '选择此工作区可用的有限开关投递渠道。',
      ),
    ),
  notificationRules: Schema.array(Schema.object({
    ...
    .extra(
      'extra',
      label('Notification rules', '通知规则', {
        presenter: { version: 1, kind: 'array.object-page', options: { allowReorder: true } },
        ...group('delivery', 'Delivery', '投递', 'host:info'),
      }),
    )
    .description('Repeatable delivery rules open in the shared Host editor page.')
    .i18n(
      description(
        'Repeatable delivery rules open in the shared Host editor page.',
        '可重复的投递规则在共享 Host 编辑子页面中打开。',
      ),
    ),
  ...
  appearance: Schema.object({
    density: Schema.union([Schema.const('comfortable'), Schema.const('compact')]).default('comfortable')
      ...
      .i18n(
        description('Choose a comfortable or compact layout for this workspace.', '为此工作区选择舒适或紧凑的布局。'),
      ),
    showActivity: Schema.boolean().default(true)
      .extra(
        'extra',
        label('Show recent activity', '显示最近动态', group('appearance', 'Appearance', '外观', 'host:palette')),
      )
