...
export function apply(ctx: Context): void {
  ctx.i18n.define({
    ...
    messages: {
      ...
      'page.description': 'Exercises one exact Host-owned DOM permission scope after update.',
      'route.title': 'Open permission v3 smoke',
      'route.description': 'Requests the exact controlled main outlet through the Host broker after update.',
    },
  })
  ctx.i18n.define({
    namespace: 'permission-v3-smoke',
    ...
    messages: {
      ...
      'page.description': '更新后验证一个由 Host 管理的精确 DOM 权限范围。',
      'route.title': '打开权限 v3 冒烟测试',
      'route.description': '更新后通过 Host Broker 请求精确的受控主区域。',
    },
  })
  ...
  ctx.pages.register({
    ...
    description: message('page.description', 'Exercises one exact Host-owned DOM permission scope after update.'),
    icon: 'host:info',
    chrome: 'body-only',
  }, domSmokePage)
  ctx.routes.register({
